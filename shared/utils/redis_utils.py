...
async def get_redis_connection_pool() -> aioredis.ConnectionPool:
    ...
    if _redis_pool is None:
        try:
            ...
            logger.error(
                f"Failed to initialize Redis connection pool: {e}", exc_info=True
            )
...
async def publish_task_event(
    redis_client: aioredis.Redis, channel: str, event_data: Dict[str, Any]
) -> None:
    """
    ...
    if not isinstance(event_data, dict):
        logger.error(
            f"Attempted to publish non-dict event_data to {channel}: {type(event_data)}"
        )
        return
    ...
    try:
        ...
        logger.debug(
            f"Published to {channel}: TaskID {event_data.get('task_id')} Status {event_data.get('status')}"
        )
    except TypeError as e:  # Handles non-serializable data in event_data
        logger.error(
            f"TypeError serializing event_data for Redis publish on {channel}: {e}. Data: {event_data}",
            exc_info=True,
        )
    except Exception as e:
        logger.error(
            f"Failed to publish event to Redis channel {channel}: {e}", exc_info=True
        )
