...
class TaskResponse(BaseModel):
    ...
    message: str = Field(
        "Task successfully submitted.", description="Informational message."
    )
...
class TaskStatusResponse(BaseModel):
    ...
    status: TaskStatusEnum = Field(
        ..., description="Current high-level status of the task."
    )
    progress: Optional[int] = Field(
        None, description="Percentage progress of the task (if available)."
    )  # <<< NEW
    status_message: Optional[str] = Field(
        None, description="Detailed status message from the worker (if available)."
    )  # <<< NEW
    result: Any | None = Field(
        None, description="Final result of the task if completed successfully."
    )
