...
from shared.schemas.enums import ModelTypeEnum # Use this for type_name
...
class HyperparameterDefinitionSchema(BaseModel):
    ...
    options: Optional[List[Dict[str, Any]]] = None # e.g., [{"value": "gini", "label": "Gini Impurity"}]
    range: Optional[Dict[str, Optional[float]]] = None # e.g., {"min": 0.1, "max": 1.0, "step": 0.01}
    required: Optional[bool] = False


class MLModelTypeDefinitionBase(BaseModel):
    type_name: str = Field(..., description="Internal name from ModelTypeEnum as string")
    display_name: str = Field(..., description="User-friendly display name")
    description: Optional[str] = Field(None, description="Description of the model type")
    ...
    is_enabled: bool = Field(True, description="If this model type is available for selection")
...
class AvailableModelTypeResponse(BaseModel):
    type_name: str = Field(..., description="Model type name as string from ModelTypeEnum")
    display_name: str
    description: Optional[str] = None
