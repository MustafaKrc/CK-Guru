...
class MLModelBase(BaseModel):
    ...
    dataset_id: Optional[int] = Field(None, description="ID of the dataset used for training/evaluation.")
...
class MLModelCreate(MLModelBase):
    ...
    s3_artifact_path: Optional[str] = Field(None, description="Initial path before final upload (if needed).")
...
class MLModelRead(MLModelBase):
    id: int
    version: int
    s3_artifact_path: Optional[str] = Field(None, description="URI to the saved model artifact.")
