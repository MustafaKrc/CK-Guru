...
class EventPublishingTask(Task):
    ...
    async def _publish_event(self, event_data: dict):
        ...
        try:
            ...
            logger.error(f"Redis connection error in _publish_event for task {self.request.id}: {e}", exc_info=True)
        except Exception as e:
            logger.error(f"Failed to publish task event via _publish_event for task {self.request.id}: {e}", exc_info=True)
    ...
    async def update_task_state(self, *, # Make arguments keyword-only for clarity
                                state: str, # Should ideally be JobStatusEnum or TaskStatusEnum
                                meta: dict = None,
                                job_type: Optional[str] = None, # Application-specific e.g., "repository_ingestion"
        ...
        elif "status" not in current_meta and state: # fallback status_message to state if not provided
        ...
        if state == JobStatusEnum.RUNNING.value and progress is not None and progress < 100:
        ...
        logger.debug(f"Task {self.request.id} state updated to {celery_state_to_set} with meta: {current_meta}")
        ...
        event_payload = {
            ...
            "error_details": current_meta.get("error_details") if state == JobStatusEnum.FAILED.value else None,
            "result_summary": current_meta.get("result_summary") if state == JobStatusEnum.SUCCESS.value else None,
        }
