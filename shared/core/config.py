...
class Settings(BaseSettings):
    ...
    S3_SECRET_ACCESS_KEY: Optional[SecretStr] = Field(
        None, validation_alias="S3_SECRET_ACCESS_KEY"
    )
    ...
    def OPTUNA_DB_URL(self) -> str:
        ...
        if not sync_db_url.startswith("postgresql+psycopg2://"):
            logger.warning(
                f"Could not reliably convert DATABASE_URL ('{db_url_str}') to Optuna sync URL. Using original."
            )
            return db_url_str
    ...
    DEFAULT_WEBHOOK_MODEL_ID: int = Field(
        1, validation_alias="DEFAULT_WEBHOOK_MODEL_ID"
    )
...
logger.info(
    f"Database URL: {'Configured' if settings.DATABASE_URL else 'Not Configured'}"
)
...
if settings.GITHUB_WEBHOOK_SECRET:
    logger.info("GitHub Webhook Secret: Loaded")
else:
    logger.warning(
        "GitHub Webhook Secret: Not configured. Webhook verification disabled."
    )
