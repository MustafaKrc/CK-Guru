...
class JobStatusUpdater(IJobStatusUpdater):
    ...
    def _get_job_model_class(self, job_type_or_model: Union[str, Type[JobModel]]) -> Type[JobModel]:
        ...
        if isinstance(job_type_or_model, str):
            ...
            if job_type_lower == 'dataset': return Dataset # Add Dataset mapping
            raise ValueError(f"Unknown job_type string '{job_type_or_model}'")
        # Include Dataset in the issubclass check
        elif isinstance(job_type_or_model, type) and issubclass(job_type_or_model, (TrainingJob, HyperparameterSearchJob, InferenceJob, Dataset)):
    ...
    def _get_and_update_job(
        ...
        with self.session_factory() as session:
            try:
                ...
                if not job:
                    logger.error(f"JobStatusUpdater: Job/Dataset {job_id} (Type: {model_class.__name__}) not found in database for update.")
                    return False

                logger.debug(f"JobStatusUpdater: Updating {model_class.__name__} {job_id} with: {updates}")
                ...
                for field, value in updates.items():
                    if hasattr(job, field):
                        ...
                        logger.warning(f"JobStatusUpdater: Field '{field}' not found on model {model_class.__name__} for ID {job_id}.")

                session.add(job) # Add updated object to session
                session.commit()
                logger.info(f"JobStatusUpdater: Successfully updated {model_class.__name__} ID {job_id}. Status -> {updates.get('status')}")
                success = True
            except SQLAlchemyError as db_err:
                logger.error(f"JobStatusUpdater: DB error updating {model_class.__name__} {job_id}: {db_err}", exc_info=True)
                session.rollback()
            except Exception as e:
                logger.error(f"JobStatusUpdater: Unexpected error updating {model_class.__name__} {job_id}: {e}", exc_info=True)
    ...
    def update_job_start(self, job_id: int, job_type: Union[str, Type[JobModel]], task_id: str) -> bool:
        ...
        running_status = DatasetStatusEnum.GENERATING if model_class is Dataset else JobStatusEnum.RUNNING
    ...
    def update_job_completion(
        ...
        valid_job_statuses = [JobStatusEnum.SUCCESS, JobStatusEnum.FAILED, JobStatusEnum.REVOKED]
        ...
        if model_class is Dataset:
            if status not in valid_dataset_statuses:
                 logger.error(f"JobStatusUpdater: Invalid final status '{status}' for Dataset job {job_id}.")
                 return False
        else: # Training, HP, Inference jobs
             if status not in valid_job_statuses:
                  logger.error(f"JobStatusUpdater: Invalid final status '{status}' for Job {job_id}.")
                  return False
        ...
        if results:
            is_success = (model_class is Dataset and status == DatasetStatusEnum.READY) or \
                         (model_class is not Dataset and status == JobStatusEnum.SUCCESS)
            ...
            if is_success:
                if model_class == TrainingJob:
                    ...
                    updates['background_data_path'] = results.get('background_data_path')
    ...
    def update_inference_feature_path(self, job_id: int, feature_path: str) -> bool:
         ...
         updates = {"feature_artifact_path": feature_path} # Field needs adding to InferenceJob model
         # Check if the field exists before updating
         if not hasattr(InferenceJob, "feature_artifact_path"):
              logger.error("Field 'feature_artifact_path' does not exist on InferenceJob model. Cannot update.")
              return False
    ...
    def update_dataset_completion(
        ...
        if status == DatasetStatusEnum.READY:
            if storage_path: results['storage_path'] = storage_path
            if background_data_path: results['background_data_path'] = background_data_path
