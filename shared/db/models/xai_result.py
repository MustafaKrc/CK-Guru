...
from sqlalchemy import (ForeignKey, Text, JSON, DateTime, Enum, Index, String, Integer, UniqueConstraint)
...
class XAIResult(Base):
    ...
    inference_job_id: Mapped[int] = mapped_column(
        ForeignKey('inference_jobs.id', ondelete="CASCADE"), # Cascade delete if inference job is removed
        nullable=False,
        index=True
    )
    xai_type: Mapped[XAITypeEnum] = mapped_column(
        Enum(XAITypeEnum, name="xai_type_enum"),
        nullable=False,
        index=True
    )
    ...
    celery_task_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True) # Task ID for this specific XAI job
    ...
    # --- Relationships ---
    inference_job: Mapped["InferenceJob"] = relationship() # No back-pop needed on InferenceJob unless listing XAIResults

    # --- Constraints ---
    __table_args__ = (
        UniqueConstraint('inference_job_id', 'xai_type', name='uq_inference_job_xai_type'),
        Index('ix_xai_results_job_id_type', 'inference_job_id', 'xai_type'), # Combined index
    )
