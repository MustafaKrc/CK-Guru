...
class HyperparameterSearchJob(Base):
    ...
    config: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=False, comment="Search configuration (model type, HP space, Optuna settings)")

    dataset_id: Mapped[int] = mapped_column(ForeignKey('datasets.id', ondelete="CASCADE"), nullable=False, index=True)
    optuna_study_name: Mapped[str] = mapped_column(String, nullable=False, index=True, unique=True,
                                                   comment="Unique name for the Optuna study")

    # Results from Optuna
    best_trial_id: Mapped[int | None] = mapped_column(Integer, nullable=True,
                                                       comment="Optuna's internal best trial ID")
