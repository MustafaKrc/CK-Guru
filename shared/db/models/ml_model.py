...
class MLModel(Base):
    ...
    s3_artifact_path: Mapped[str | None] = mapped_column(
        String,
        nullable=True,
        unique=True,
    ...
    hp_search_job_id: Mapped[int | None] = mapped_column(
        ForeignKey("hp_search_jobs.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    ...
    performance_metrics: Mapped[Dict[str, Any] | None] = mapped_column(
        JSON, nullable=True, comment="Key performance metrics (e.g., accuracy, f1)"
    )
    ...
    dataset: Mapped[Optional["Dataset"]] = (
        relationship()
    )  # No back_populates needed unless Dataset needs to list models
    ...
    inference_jobs: Mapped[List["InferenceJob"]] = relationship(
        "InferenceJob", back_populates="ml_model"
    )  # Models used in many inferences
