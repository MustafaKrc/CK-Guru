...
class GitHubIssueRepository(BaseRepository[GitHubIssue]):
    ...
    def update_or_create_from_api(
        ...
        """
        with self._session_scope() as session:
            db_issue = self.get_by_number(repo_id, issue_number) # Use own method for consistency
            ...
            if db_issue:
                ...
                if api_response.status_code == 304: # Not Modified
                    ...
                    except (TypeError, ValueError, AttributeError): logger.warning(f"Could not parse create date '{created_at_str}'")
                    ...
                    except (TypeError, ValueError, AttributeError): logger.warning(f"Could not parse close date '{closed_at_str}'")
                    ...
                    logger.error(f"GitHubIssueRepository: API error {api_response.status_code} for existing issue #{issue_number}. Using stale data.")
                ...
                if api_response.status_code == 200 and api_response.json_data:
                    ...
                    new_issue = GitHubIssue(
                        repository_id=repo_id, issue_number=issue_number,
                    ...
                    logger.warning(f"GitHubIssueRepository: Issue #{issue_number} not found on GitHub ({api_response.status_code}). Cannot create.")
                    managed_issue = None
                else: # API Error
                    logger.error(f"GitHubIssueRepository: API error {api_response.status_code} for new issue #{issue_number}. Cannot create. Error: {api_response.error_message}")
                    managed_issue = None

            if managed_issue:
                try:
                    session.commit() # Commit changes for this single issue
                    session.refresh(managed_issue) # Ensure state is up-to-date after commit
                    return managed_issue
                except SQLAlchemyError as e:
                    logger.error(f"GitHubIssueRepository: DB error updating/creating issue #{issue_number}: {e}", exc_info=True)
                    session.rollback()
                    return None # Return None on DB error
                except Exception as e:
                    logger.error(f"GitHubIssueRepository: Unexpected error updating/creating issue #{issue_number}: {e}", exc_info=True)
                    session.rollback()
                    return None
            else:
