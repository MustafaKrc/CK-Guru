...
from shared.db.models import TrainingJob, JobStatusEnum # Correct model and enum
...
class TrainingJobRepository(BaseRepository[TrainingJob]):
    ...
    def get_by_task_id(self, celery_task_id: str) -> Optional[TrainingJob]:
        ...
        with self._session_scope() as session:
            stmt = select(TrainingJob).where(TrainingJob.celery_task_id == celery_task_id).limit(1)
    ...
    def list_jobs(
        ...
        logger.debug(f"TrainingJobRepo: Listing jobs (dataset_id={dataset_id}, status={status})")
        with self._session_scope() as session:
