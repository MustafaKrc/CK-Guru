...
class MLModelTypeDefinitionRepository(BaseRepository):
    ...
    def get_by_type_name(self, type_name: str) -> Optional[MLModelTypeDefinitionDB]:
        with self._session_scope() as session:
            return session.execute(
                select(MLModelTypeDefinitionDB).filter(
                    MLModelTypeDefinitionDB.type_name == type_name
                )
            ).scalar_one_or_none()

    def get_all_enabled(
        self, skip: int = 0, limit: int = 100
    ) -> Sequence[MLModelTypeDefinitionDB]:
        with self._session_scope() as session:
            return (
                session.execute(
                    select(MLModelTypeDefinitionDB)
                    .filter(MLModelTypeDefinitionDB.is_enabled)
    ...
    def upsert_model_type_definitions(
        self, definitions: List[Dict[str, Any]], worker_identifier: str
    ):
        """
        ...
        """
        if (
            not definitions and not worker_identifier
        ):  # Allow empty definitions if worker_identifier is provided for cleanup
            logger.warning(
                "Upsert called with no definitions and no worker_identifier. Skipping DB update."
            )
            return
        ...
        with self._session_scope() as session:
            if definitions:
                logger.info(
                    f"Upserting {len(definitions)} model type definitions from {worker_identifier} into DB..."
                )
                ...
                update_columns["is_enabled"] = (
                    True  # Assume discovered types should be enabled by default
                )
            ...
            logger.info(
                f"Marking model types as disabled if previously managed by '{worker_identifier}' and not in current discovery set..."
            )
            update_stmt_disable_old = (
                ...
                .values(
                    is_enabled=False, last_updated_by=worker_identifier
                )  # Mark as disabled and update who did it
                .execution_options(synchronize_session=False)
            )
            result = session.execute(update_stmt_disable_old)
            if result.rowcount > 0:
                logger.info(
                    f"Marked {result.rowcount} old model types as disabled for worker '{worker_identifier}'."
                )
