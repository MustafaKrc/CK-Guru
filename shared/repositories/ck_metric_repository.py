...
class CKMetricRepository(BaseRepository[CKMetric]):
        ...
        with self._session_scope() as session:
            try:
                ...
                stmt = pg_insert(CKMetric).on_conflict_do_update(
                    ...
                    set_={
                        c.name: getattr(pg_insert(CKMetric).excluded, c.name)
                ...
                logger.info(
                    f"CKMetricRepository: Bulk UPSERT processed {processed_count} records."
                )

            except SQLAlchemyError as e:
                logger.error(
                    f"CKMetricRepository: Database error during bulk UPSERT: {e}",
                    exc_info=True,
                )
                raise  # Re-raise to be handled by caller
            except Exception as e:
                logger.error(
                    f"CKMetricRepository: Unexpected error during bulk UPSERT: {e}",
                    exc_info=True,
                )
                raise
    ...
    def get_metrics_dataframe_for_commit(
        ...
        data = [
            {
                ...
                "tcc": m.tcc,
                "lcc": m.lcc,
                "totalMethodsQty": m.totalMethodsQty,
                "staticMethodsQty": m.staticMethodsQty,
                "publicMethodsQty": m.publicMethodsQty,
                "privateMethodsQty": m.privateMethodsQty,
                "protectedMethodsQty": m.protectedMethodsQty,
                "defaultMethodsQty": m.defaultMethodsQty,
                "visibleMethodsQty": m.visibleMethodsQty,
                "abstractMethodsQty": m.abstractMethodsQty,
                "finalMethodsQty": m.finalMethodsQty,
                "synchronizedMethodsQty": m.synchronizedMethodsQty,
                "totalFieldsQty": m.totalFieldsQty,
                "staticFieldsQty": m.staticFieldsQty,
                "publicFieldsQty": m.publicFieldsQty,
                "privateFieldsQty": m.privateFieldsQty,
                "protectedFieldsQty": m.protectedFieldsQty,
                "defaultFieldsQty": m.defaultFieldsQty,
                "finalFieldsQty": m.finalFieldsQty,
                "synchronizedFieldsQty": m.synchronizedFieldsQty,
                "nosi": m.nosi,
                "loc": m.loc,
                "returnQty": m.returnQty,
                "loopQty": m.loopQty,
                "comparisonsQty": m.comparisonsQty,
                "tryCatchQty": m.tryCatchQty,
                "parenthesizedExpsQty": m.parenthesizedExpsQty,
                "stringLiteralsQty": m.stringLiteralsQty,
                "numbersQty": m.numbersQty,
                "assignmentsQty": m.assignmentsQty,
                "mathOperationsQty": m.mathOperationsQty,
                "variablesQty": m.variablesQty,
                "maxNestedBlocksQty": m.maxNestedBlocksQty,
                "anonymousClassesQty": m.anonymousClassesQty,
                "innerClassesQty": m.innerClassesQty,
                "lambdasQty": m.lambdasQty,
                "uniqueWordsQty": m.uniqueWordsQty,
                "modifiers": m.modifiers,
                "logStatementsQty": m.logStatementsQty,
            }
            for m in metrics
        ]
        df = pd.DataFrame(data)
        logger.debug(
            f"CKMetricRepository: Retrieved {len(df)} CK metrics as DataFrame for commit {commit_hash[:7]}."
        )
        return df
    ...
    def check_metrics_exist_for_commit(self, repo_id: int, commit_hash: str) -> bool:
        ...
        with self._session_scope() as session:
            stmt = (
                ...
                .limit(1)
            )  # Optimization: only need to find one record
            ...
            logger.debug(
                f"CKMetricRepository: Check existence for commit {commit_hash[:7]} -> {exists_flag}"
            )
            return exists_flag
