...
class BotPatternRepository(BaseRepository[BotPattern]):
    ...
    def build_bot_filter_condition(
        ...
        try:
            ...
            logger.error(
                f"Model alias {model_alias} does not have an 'author_name' attribute."
            )
        ...
        is_excluded_bot = (
            sa.or_(*exclusion_filters) if exclusion_filters else sa.false()
        )
        is_included_bot = (
            sa.or_(*inclusion_filters) if inclusion_filters else sa.false()
        )
