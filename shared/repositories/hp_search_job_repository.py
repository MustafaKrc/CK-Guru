...
from shared.db.models import (  # Correct model and enum
...
class HPSearchJobRepository(BaseRepository[HyperparameterSearchJob]):
    ...
    def get_by_task_id(self, celery_task_id: str) -> Optional[HyperparameterSearchJob]:
        ...
        with self._session_scope() as session:
            stmt = (
                ...
                .where(HyperparameterSearchJob.celery_task_id == celery_task_id)
                .limit(1)
            )
    ...
    def list_jobs(
        ...
        logger.debug(
            f"HPSearchJobRepo: Listing jobs (dataset_id={dataset_id}, status={status}, study={study_name})"
        )
        with self._session_scope() as session:
            from sqlalchemy.orm import selectinload
            ...
            if study_name:
                stmt = stmt.filter(
                    HyperparameterSearchJob.optuna_study_name == study_name
                )
