...
class CommitGuruMetricRepository(BaseRepository[CommitGuruMetric]):
    ...
    def bulk_upsert(self, commit_metrics: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        upsert_stmt = (
            insert_stmt
            .on_conflict_do_update(index_elements=index_elements, set_=update_columns)
        ...
        with self._session_scope() as session:
            try:
                ...
                logger.info(f"CommitGuruMetricRepository: Bulk UPSERT processed {len(db_ids_map)} rows.")

            except SQLAlchemyError as e:
                logger.error(f"CommitGuruMetricRepository: Database error during bulk UPSERT: {e}", exc_info=True)
                raise # Re-raise to be handled by caller
            except Exception as e:
                logger.error(f"CommitGuruMetricRepository: Unexpected error during bulk UPSERT: {e}", exc_info=True)
                raise
        ...
        with self._session_scope() as session:
            try:
                ...
                if not commit_metric_obj:
                    logger.error(f"CommitGuruMetricRepository: Cannot link issues, CommitGuruMetric {commit_db_id} not found.")
                    return

                # Fetch issue objects that need linking
                issues_to_link = session.execute(
                    select(GitHubIssue).where(GitHubIssue.id.in_(issue_db_ids))
                ).scalars().all()

                existing_linked_issue_ids = {issue.id for issue in commit_metric_obj.github_issues}
                ...
                if newly_linked_count > 0:
                    session.add(commit_metric_obj) # Add to session to track relationship changes
                    session.commit()
                    logger.info(f"CommitGuruMetricRepository: Linked {newly_linked_count} new issues to commit {commit_db_id}.")
                else:
                    logger.debug(f"CommitGuruMetricRepository: No new issues to link for commit {commit_db_id}.")

            except SQLAlchemyError as e:
                logger.error(f"CommitGuruMetricRepository: Database error linking issues to commit {commit_db_id}: {e}", exc_info=True)
                raise
            except Exception as e:
                logger.error(f"CommitGuruMetricRepository: Unexpected error linking issues to commit {commit_db_id}: {e}", exc_info=True)
                raise

    def update_bug_links(self, bug_introducing_commit_ids: Set[int], fixing_commit_map: Dict[int, List[str]]):
            ...
            try:
                ...
                if bug_introducing_commit_ids:
                    update_buggy_stmt = (
                        ...
                        .where(CommitGuruMetric.id.in_(list(bug_introducing_commit_ids)))
                        .values(is_buggy=True)
                        .execution_options(synchronize_session=False)
                    )
                    result = session.execute(update_buggy_stmt)
                    updated_buggy_count = result.rowcount
                    logger.info(f"CommitGuruMetricRepository: Updated is_buggy=True for {updated_buggy_count} commits.")
                ...
                for buggy_db_id, fixing_hashes in fixing_commit_map.items():
                    try:
                        update_fixing_stmt = (
                            ...
                            .values(fixing_commit_hashes={"hashes": fixing_hashes}) # Store as JSON object
                            .execution_options(synchronize_session=False)
                        )
                        session.execute(update_fixing_stmt)
                        updated_fixing_count += 1
                    except Exception as ind_update_err:
                         logger.error(f"CommitGuruMetricRepository: Failed updating fixing_commit_hashes for commit ID {buggy_db_id}: {ind_update_err}", exc_info=False)
                         # Continue with others

                session.commit()
                logger.info(f"CommitGuruMetricRepository: Updated fixing_commit_hashes for {updated_fixing_count} commits.")

            except SQLAlchemyError as e:
                logger.error(f"CommitGuruMetricRepository: Database error updating bug links: {e}", exc_info=True)
                raise
            except Exception as e:
                 logger.error(f"CommitGuruMetricRepository: Unexpected error updating bug links: {e}", exc_info=True)
                 raise

    def get_earliest_linked_issue_timestamp(self, commit_metric_id: int) -> Optional[int]:
        """
        ...
        with self._session_scope() as session:
            try:
                stmt = (
                    ...
                    .join(commit_github_issue_association_table, CommitGuruMetric.id == commit_github_issue_association_table.c.commit_guru_metric_id)
                    .join(GitHubIssue, GitHubIssue.id == commit_github_issue_association_table.c.github_issue_id)
                ...
                logger.error(f"CommitGuruMetricRepository: DB error getting earliest issue timestamp for commit {commit_metric_id}: {e}", exc_info=True)
                return None # Return None on error
            except Exception as e:
                logger.error(f"CommitGuruMetricRepository: Unexpected error getting earliest issue timestamp for commit {commit_metric_id}: {e}", exc_info=True)
                return None