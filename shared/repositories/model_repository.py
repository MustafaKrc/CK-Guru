...
class ModelRepository(BaseRepository[MLModel]): # Specify ModelType
    ...
    def create_model_record(self, model_data: Dict[str, Any]) -> int:
        ...
        logger.info(f"ModelRepo: Creating MLModel record for {model_data.get('name')} v{model_data.get('version')}")
        with self._session_scope() as session:
    ...
    def set_model_artifact_path(self, model_id: int, s3_path: str):
        ...
        logger.info(f"ModelRepo: Setting artifact path for MLModel {model_id} to {s3_path}")
        with self._session_scope() as session:
            ...
            if db_obj:
                ...
                logger.debug(f"ModelRepo: MLModel {model_id} artifact path updated and committed.")
            else:
                logger.error(f"ModelRepo: Cannot set artifact path - MLModel {model_id} not found.")
