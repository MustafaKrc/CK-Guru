...
from typing import Callable, Dict, List, Optional  # Added Callable
...
class XaiResultRepository(BaseRepository[XAIResult]):  # Specify ModelType
    ...
    def find_existing_xai_result_id_sync(
        self, inference_job_id: int, xai_type: XAITypeEnum
    ) -> Optional[int]:
    ...
    def create_pending_xai_result_sync(
        self, inference_job_id: int, xai_type: XAITypeEnum
    ) -> Optional[int]:
        ...
        try:
            with self._session_scope() as session:
                logger.info(
                    f"XaiRepo: Creating PENDING XAIResult record for Job {inference_job_id}, Type: {xai_type.value}"
                )
                xai_create = schemas.XAIResultCreate(
                    inference_job_id=inference_job_id, xai_type=xai_type
                )
            ...
            if xai_result_id:
                ...
                logger.error(
                    f"XaiRepo: Failed to get ID after flush for Job {inference_job_id}, Type {xai_type.value}"
                )
        except Exception as e:
            logger.error(
                f"XaiRepo: Failed to create pending XAIResult for Job {inference_job_id}, Type {xai_type.value}: {e}",
                exc_info=True,
            )
    ...
    def update_xai_result_sync(
        ...
        try:
            with self._session_scope() as session:
                ...
                if not xai_record:
                    logger.error(
                        f"XaiRepo: Cannot update XAIResult: ID {xai_result_id} not found."
                    )
                    raise ValueError(
                        f"XAIResult record {xai_result_id} not found for update."
                    )

                logger.info(
                    f"XaiRepo: Updating XAIResult {xai_result_id} status to {status.value}"
                )
                xai_record.status = status
                if message:
                    xai_record.status_message = message[:1000]
                if result_data is not None:
                    xai_record.result_data = result_data
                if task_id:
            ...
            logger.error(
                f"XaiRepo: Failed to update XAIResult DB entry for ID {xai_result_id}: {e}",
                exc_info=True,
            )
    ...
    def update_xai_task_id_sync(self, xai_result_id: int, task_id: str):
        ...
        try:
            with self._session_scope() as session:
                ...
                if not xai_record:
                    logger.error(
                        f"XaiRepo: Cannot update task ID for XAIResult: ID {xai_result_id} not found."
                    )
                    raise ValueError(
                        f"XAIResult record {xai_result_id} not found for task ID update."
                    )
                xai_record.celery_task_id = task_id
                session.add(xai_record)
                # Commit happens automatically
            logger.debug(
                f"XaiRepo: Set task ID {task_id} for XAIResult {xai_result_id} and committed."
            )
        except Exception as e:
            logger.error(
                f"XaiRepo: Failed to update task ID for XAIResult {xai_result_id}: {e}",
                exc_info=True,
            )
            raise  # Re-raise
    ...
    def mark_xai_results_failed_sync(self, xai_result_ids: List[int], message: str):
        ...
        if not xai_result_ids:
            return
        try:
            with self._session_scope() as session:
                logger.warning(
                    f"XaiRepo: Marking XAIResults {xai_result_ids} as FAILED. Reason: {message}"
                )
                stmt = (
                    update(XAIResult)
            ...
            logger.error(
                f"XaiRepo: Failed to bulk mark XAIResults as FAILED: {e}", exc_info=True
            )
            raise  # Re-raise
