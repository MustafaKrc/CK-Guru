...
const RepositoryStatus: React.FC<{ repo: Repository }> = ({ repo }) => {
  ...
  const ingestionTask = getLatestTaskForEntity(
    taskStatuses,
    "Repository",
    repo.id,
  ...
  if (ingestionTask && (ingestionTask.status === "RUNNING" || ingestionTask.status === "PENDING")) {
    return (
      <div className="flex flex-col gap-1.5 w-full max-w-[160px]">
        <Badge
          variant="outline"
          className="text-blue-600 border-blue-600 dark:text-blue-400 dark:border-blue-500"
        >
          <RefreshCw className="h-3 w-3 mr-1.5 animate-spin" />
          Ingesting... ({ingestionTask.progress ?? 0}%)
        </Badge>
        <Progress
          value={ingestionTask.progress ?? 0}
          className="h-1.5"
          indicatorClassName="bg-blue-500"
        />
        <span
          className="text-xs text-muted-foreground truncate"
          title={ingestionTask.status_message ?? ""}
        >
...
export default function RepositoriesPage() {
  ...
  const fetchRepositories = useCallback(
    async (page: number, limit: number, sort: SortConfig, filter: string) => {
  ...
  const handleAddRepository = async () => {
    ...
    try {
      await apiService.post<Repository, RepositoryCreatePayload>("/repositories", {
        git_url: newRepoUrl,
      });
  ...
  const handleDeleteRepository = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Repository "${selectedRepoForDelete.name}" deleted.`,
      });
  ...
  const handleIngest = async (repo: Repository) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Task ${response.task_id} submitted for ${repo.name}.`,
      });
    } catch (err) {
  ...
  const renderTableContent = () => {
    ...
    return repositories.map((repo) => {
      ...
      const isIngesting =
        ingestLoading[repo.id] ||
        (task && (task.status === "RUNNING" || task.status === "PENDING"));
      return (
        <TableRow key={repo.id} data-state={isIngesting ? "active" : ""}>
          <TableCell className="font-medium">
            <Link href={`/repositories/${repo.id}`} className="hover:underline text-primary">
              {repo.name}
            </Link>
          </TableCell>
          ...
          <TableCell className="text-right">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" disabled={isIngesting}>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => handleIngest(repo)} disabled={isIngesting}>
  ...
  const SortableHeader: React.FC<{ sortKey: SortableKeys; children: React.ReactNode }> = ({
    sortKey,
    children,
  }) => (
      ...
      <div className="container mx-auto py-6 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold tracking-tight">
            Repositories ({pagination.totalItems})
          </h1>
          <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
            <DialogTrigger asChild>
              <Button onClick={() => setNewRepoUrl("")}>
            ...
            <DialogContent>
              ...
              <DialogFooter>
                ...
                <Button onClick={handleAddRepository} disabled={isAddingRepo}>
                  {isAddingRepo && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Add Repository
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>
                  <SortableHeader sortKey="summary">Summary</SortableHeader>
                </TableHead>
                <TableHead>Status</TableHead>
                <TableHead>
                  <SortableHeader sortKey="created_at">Date Added</SortableHeader>
                </TableHead>
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Delete {selectedRepoForDelete?.name}?</DialogTitle>
              <DialogDescription>
                This action is irreversible and will delete all associated data (datasets, models,
                jobs).
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              ...
              >
                {isDeletingRepo && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Delete
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
