...
export default function RepositoryDetailPage() {
  ...
  const [trainingJobsPagination, setTrainingJobsPagination] = useState({
    currentPage: 1,
    totalItems: 0,
    isLoading: true,
  });
  const [hpSearchJobs, setHpSearchJobs] = useState<HPSearchJobRead[]>([]);
  const [hpSearchJobsPagination, setHpSearchJobsPagination] = useState({
    currentPage: 1,
    totalItems: 0,
    isLoading: true,
  });
  const [inferenceJobs, setInferenceJobs] = useState<InferenceJobRead[]>([]);
  const [inferenceJobsPagination, setInferenceJobsPagination] = useState({
    currentPage: 1,
    totalItems: 0,
    isLoading: true,
  });
  const [botPatterns, setBotPatterns] = useState<BotPatternRead[]>([]);
  const [botPatternsPagination, setBotPatternsPagination] = useState({
    currentPage: 1,
    totalItems: 0,
    isLoading: true,
  });
  ...
  const handleIngest = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Task ${response.task_id} submitted for ${repository.name}.`,
      });
    } catch (err) {
  ...
  const getDisplayRepoStatusInfo = (): {
    ...
    if (
      repoIngestionStatus &&
      (repoIngestionStatus.status === "RUNNING" || repoIngestionStatus.status === "PENDING")
    ) {
      return {
  ...
  const fetchPaginatedDatasets = useCallback(
    async (page: number) => {
      if (!repoId || !repository) return;
      setDatasetsPagination((prev) => ({
        ...prev,
        isLoading: true,
        currentPage: page,
        totalItems: page === 1 ? 0 : prev.totalItems,
      }));
      const skip = (page - 1) * ITEMS_PER_PAGE;
      try {
        const response = await apiService.get<PaginatedDatasetRead>(
          `/repositories/${repoId}/datasets?skip=${skip}&limit=${ITEMS_PER_PAGE}`
        );
        if (response && Array.isArray(response.items) && typeof response.total === "number") {
          ...
          handleApiError(
            { message: "Received invalid data structure for datasets." },
  ...
  const fetchPaginatedModels = useCallback(
    async (page: number) => {
      if (!repoId || !repository) return;
      ...
      const skip = (page - 1) * ITEMS_PER_PAGE;
      try {
        const response = await apiService.get<PaginatedMLModelRead>(
          `/repositories/${repoId}/models?skip=${skip}&limit=${ITEMS_PER_PAGE}`
        );
        if (response && Array.isArray(response.items) && typeof response.total === "number") {
          ...
          handleApiError(
            { message: "Received invalid data structure for models." },
  ...
  const fetchPaginatedTrainingJobs = useCallback(
    async (page: number) => {
      if (!repoId || !repository) return;
      setTrainingJobsPagination((prev) => ({
        ...prev,
        isLoading: true,
        currentPage: page,
        totalItems: page === 1 ? 0 : prev.totalItems,
      ...
      const skip = (page - 1) * ITEMS_PER_PAGE;
      try {
        const response = await apiService.get<PaginatedTrainingJobRead>(
          `/repositories/${repoId}/training-jobs?skip=${skip}&limit=${ITEMS_PER_PAGE}`
        );
        if (response && Array.isArray(response.items) && typeof response.total === "number") {
          ...
          handleApiError(
            { message: "Received invalid data structure for training jobs." },
  ...
  const fetchPaginatedHpSearchJobs = useCallback(
    async (page: number) => {
      if (!repoId || !repository) return;
      setHpSearchJobsPagination((prev) => ({
        ...prev,
        isLoading: true,
        currentPage: page,
        totalItems: page === 1 ? 0 : prev.totalItems,
      ...
      const skip = (page - 1) * ITEMS_PER_PAGE;
      try {
        const response = await apiService.get<PaginatedHPSearchJobRead>(
          `/repositories/${repoId}/hp-search-jobs?skip=${skip}&limit=${ITEMS_PER_PAGE}`
        );
        if (response && Array.isArray(response.items) && typeof response.total === "number") {
          ...
          handleApiError(
            { message: "Received invalid data structure for HP search jobs." },
            "Failed to fetch HP search jobs"
          );
          setHpSearchJobs([]);
  ...
  const fetchPaginatedInferenceJobs = useCallback(
    async (page: number) => {
      if (!repoId || !repository) return;
      ...
      const skip = (page - 1) * ITEMS_PER_PAGE;
      try {
        const response = await apiService.get<PaginatedInferenceJobRead>(
          `/repositories/${repoId}/inference-jobs?skip=${skip}&limit=${ITEMS_PER_PAGE}`
        );
        if (response && Array.isArray(response.items) && typeof response.total === "number") {
          ...
          handleApiError(
            { message: "Received invalid data structure for inference jobs." },
            "Failed to fetch inference jobs"
          );
          setInferenceJobs([]);
  ...
  const fetchPaginatedBotPatterns = useCallback(
    async (page: number) => {
      if (!repoId || !repository) return;
      setBotPatternsPagination((prev) => ({
        ...prev,
        isLoading: true,
        currentPage: page,
        totalItems: page === 1 ? 0 : prev.totalItems,
      ...
      const skip = (page - 1) * ITEMS_PER_PAGE;
      try {
        const response = await apiService.get<PaginatedBotPatternRead>(
          `/repositories/${repoId}/bot-patterns?skip=${skip}&limit=${ITEMS_PER_PAGE}&include_global=false`
        );
        if (response && Array.isArray(response.items) && typeof response.total === "number") {
          ...
          handleApiError(
            { message: "Received invalid data structure for bot patterns." },
            "Failed to fetch bot patterns"
          );
          setBotPatterns([]);
  ...
  useEffect(() => {
    if (repository && !isLoadingRepo) {
      // Ensure repo data is loaded and not still loading
  ...
  useEffect(() => {
    if (repository) fetchPaginatedDatasets(datasetsPagination.currentPage);
  }, [datasetsPagination.currentPage, repository, fetchPaginatedDatasets]);
  useEffect(() => {
    if (repository) fetchPaginatedModels(modelsPagination.currentPage);
  }, [modelsPagination.currentPage, repository, fetchPaginatedModels]);
  useEffect(() => {
    if (repository) fetchPaginatedTrainingJobs(trainingJobsPagination.currentPage);
  }, [trainingJobsPagination.currentPage, repository, fetchPaginatedTrainingJobs]);
  useEffect(() => {
    if (repository) fetchPaginatedHpSearchJobs(hpSearchJobsPagination.currentPage);
  }, [hpSearchJobsPagination.currentPage, repository, fetchPaginatedHpSearchJobs]);
  useEffect(() => {
    if (repository) fetchPaginatedInferenceJobs(inferenceJobsPagination.currentPage);
  }, [inferenceJobsPagination.currentPage, repository, fetchPaginatedInferenceJobs]);
  useEffect(() => {
    if (repository) fetchPaginatedBotPatterns(botPatternsPagination.currentPage);
  }, [botPatternsPagination.currentPage, repository, fetchPaginatedBotPatterns]);
  ...
  const renderTaskAwareStatusBadge = (
    taskAwareEntityStatus?: TaskStatusUpdatePayload,
    fallbackStaticStatus?: string
  ) => {
    const currentStatusToDisplay =
      taskAwareEntityStatus ||
      (fallbackStaticStatus
        ? ({ status: fallbackStaticStatus } as TaskStatusUpdatePayload)
        : undefined);
    ...
    switch (
      status?.toUpperCase() // Normalize status to uppercase for comparison
    ) {
      case "SUCCESS":
    ...
    return (
      <MainLayout>
        <PageContainer
          title="Error"
          description={
            repoError || "The requested repository could not be found or an error occurred."
          }
          actions={
        ...
        >
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Please check the repository ID or try again later. If the problem persists, contact
              support.
            </AlertDescription>
          </Alert>
        </PageContainer>
  ...
  return (
    <MainLayout>
      <PageContainer
        ...
        description={
          <a
            ...
            className="text-sm text-primary hover:underline truncate block max-w-full md:max-w-xl lg:max-w-2xl"
          >
            {repository.git_url}
          </a>
        }
      ...
      >
        {repoIngestionStatus &&
          (repoIngestionStatus.status === "RUNNING" ||
            repoIngestionStatus.status === "PENDING") && (
            <div className="mb-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-6">
            ...
            <TabsTrigger value="datasets">
              Datasets (
              {datasetsPagination.isLoading ? (
                <Loader2 className="h-3 w-3 animate-spin" />
              ) : (
                datasetsPagination.totalItems
              )}
              )
            </TabsTrigger>
            <TabsTrigger value="models">
              Models (
              {modelsPagination.isLoading ? (
                <Loader2 className="h-3 w-3 animate-spin" />
              ) : (
            ...
            <TabsTrigger value="bot-patterns">
              Bot Patterns (
              {botPatternsPagination.isLoading ? (
                <Loader2 className="h-3 w-3 animate-spin" />
              ) : (
                botPatternsPagination.totalItems
              )}
              )
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 text-sm">
                <div>
                  <Label className="text-muted-foreground block mb-0.5">Date Added</Label>
                ...
                <div>
                  <Label className="text-muted-foreground block mb-0.5">Datasets Created</Label>
                  <p className="font-semibold">{repository.datasets_count}</p>
                </div>
                <div>
                  <Label className="text-muted-foreground block mb-0.5">
                    Bot Patterns (Specific)
                  </Label>
                  <p className="font-semibold">{repository.bot_patterns_count}</p>
                </div>
                <div>
                  <Label className="text-muted-foreground block mb-0.5">Linked GitHub Issues</Label>
                  <p className="font-semibold">{repository.github_issues_count}</p>
                </div>
                <div>
                  <Label className="text-muted-foreground block mb-0.5">Models Trained</Label>
                ...
                <div>
                  <Label className="text-muted-foreground block mb-0.5">Total Commits</Label>
                  <p>N/A</p>
                </div>
                <div>
                  <Label className="text-muted-foreground block mb-0.5">Branches</Label>
                  <p>N/A</p>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <Card>
                <CardHeader className="flex flex-row items-center justify-between pb-3">
                  ...
                  {datasetsPagination.totalItems > 3 && (
                    <Button
                      variant="link"
                      size="sm"
                ...
                <CardContent>
                  {datasetsPagination.isLoading && datasets.length === 0 ? (
                    ...
                    <div className="text-center py-4 space-y-2">
                      <p className="text-sm text-muted-foreground">No datasets created yet.</p>
                    ...
                    <ul className="space-y-2">
                      {datasets.slice(0, 3).map((ds) => {
                        const datasetTask = getLatestTaskForEntity(
                          taskStatuses,
                          "Dataset",
                          ds.id,
                          "dataset_generation"
                        ...
                        return (
                          <li
                            key={ds.id}
                            className="text-sm flex justify-between items-center py-1.5 border-b border-dashed last:border-b-0"
                          >
                            <Link
                              href={`/datasets/${ds.id}`}
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between pb-3">
                  ...
                  {trainingJobsPagination.totalItems +
                    hpSearchJobsPagination.totalItems +
                    inferenceJobsPagination.totalItems >
                    3 && (
                    <Button
                      variant="link"
                      size="sm"
                ...
                <CardContent>
                  {trainingJobsPagination.isLoading &&
                  hpSearchJobsPagination.isLoading &&
                  inferenceJobsPagination.isLoading &&
                  trainingJobs.length + hpSearchJobs.length + inferenceJobs.length === 0 ? (
                    ...
                    <div className="text-center py-4 space-y-2">
                      <p className="text-sm text-muted-foreground">
                        No jobs have been run for this repository yet.
                      </p>
                      <Button size="sm" asChild>
                    ...
                    <ul className="space-y-3">
                      ...
                      {trainingJobs.slice(0, 1).map((job) => {
                        const jobTask = getLatestTaskForEntity(
                          taskStatuses,
                          "TrainingJob",
                          job.id,
                          "model_training"
                        ...
                        return (
                          ...
                          >
                            <Link
                              href={`/jobs/${job.id}?type=training`}
                              className="hover:underline font-medium truncate pr-2"
                              title={`Train: ${job.config.model_name} on ${datasetUsed?.name || job.dataset_id}`}
                            >
                              Train:{" "}
                              {job.config.model_name.length > 25
                                ? job.config.model_name.substring(0, 25) + "..."
                      ...
                      {hpSearchJobs.slice(0, 1).map((job) => {
                        ...
                        return (
                          ...
                          >
                            <Link
                              href={`/jobs/${job.id}?type=hp_search`}
                              className="hover:underline font-medium truncate pr-2"
                              title={`HP Search: ${job.optuna_study_name} on ${datasetUsed?.name || job.dataset_id}`}
                            >
                              HP Search:{" "}
                              {job.optuna_study_name.length > 20
                                ? job.optuna_study_name.substring(0, 20) + "..."
                      ...
                      {inferenceJobs.slice(0, 1).map((job) => {
                        const jobTask = getLatestTaskForEntity(
                          taskStatuses,
                          "InferenceJob",
                          job.id /*, "model_inference" or generic */
                        );
                        const inputRefCommit =
                          typeof job.input_reference === "object" && job.input_reference.commit_hash
                        ...
                        return (
                          ...
                          >
                            <Link
                              href={`/prediction-insights/${job.id}`}
                              className="hover:underline font-medium truncate pr-2"
                              title={`Inference for commit ${inputRefCommit} on ${formatDate(job.created_at)}`}
                            >
                      ...
                      {trainingJobs.slice(0, 1).length === 0 &&
                        hpSearchJobs.slice(0, 1).length === 0 &&
                        inferenceJobs.slice(0, 1).length === 0 &&
                        !(
                          trainingJobsPagination.isLoading ||
                          hpSearchJobsPagination.isLoading ||
                          inferenceJobsPagination.isLoading
              ...
              {datasetsPagination.isLoading && datasets.length === 0 ? (
                ...
                <Card className="text-center py-10">
                  <CardContent className="flex flex-col items-center justify-center">
                    ...
                    <p className="text-muted-foreground mb-3">
                      No datasets found for this repository.
                    </p>
                    <Button asChild>
                      <Link href={`/datasets/create?repository=${repoId}`}>
                ...
                <>
                        ...
                        {datasets.map((dataset) => {
                          ...
                          return (
                            <TableRow key={dataset.id}>
                              ...
                              <TableCell>
                                {renderTaskAwareStatusBadge(datasetTask, dataset.status)}
                              </TableCell>
                  ...
                  {renderPaginationControls(
                    datasetsPagination.currentPage,
                    datasetsPagination.totalItems,
                    ITEMS_PER_PAGE,
                  ...
                  <div className="text-center text-xs text-muted-foreground mt-1">
                    {datasetsPagination.totalItems > 0
                      ? `Showing ${datasets.length} of ${datasetsPagination.totalItems} datasets`
                      : ""}
                  </div>
                </>
              )}
              ...
              {modelsPagination.isLoading && models.length === 0 ? (
                ...
                <Card className="text-center py-10">
                  <CardContent className="flex flex-col items-center justify-center">
                    ...
                    <p className="text-muted-foreground mb-3">
                      No ML models trained with data from this repository.
                    </p>
                    <Button asChild>
                      <Link href={`/jobs/train?repositoryId=${repoId}`}>
                ...
                <>
                          ...
                          return (
                            <TableRow key={model.id}>
                              ...
                              <TableCell>
                                {sourceDataset ? (
                                  <Link
                                    href={`/datasets/${model.dataset_id}`}
                              ...
                              <TableCell className="text-right space-x-2">
                                ...
                                <Button variant="secondary" size="sm" asChild>
                                  <Link
                                    href={`/jobs/inference?modelId=${model.id}&repositoryId=${repoId}`}
                                  >
                                    Run Inference
                                  </Link>
                                </Button>
                  ...
                  {renderPaginationControls(
                    modelsPagination.currentPage,
                    modelsPagination.totalItems,
                    ITEMS_PER_PAGE,
                    fetchPaginatedModels,
                    modelsPagination.isLoading
                  )}
                  <div className="text-center text-xs text-muted-foreground mt-1">
                    {modelsPagination.totalItems > 0
                      ? `Showing ${models.length} of ${modelsPagination.totalItems} models`
                      : ""}
                  </div>
                </>
              )}
            ...
            <PageContainer
              ...
              actions={
                <div className="space-x-2">
                  ...
                  <Button variant="outline" size="sm" asChild>
                    <Link href={`/jobs/hp-search?repositoryId=${repoId}`}>
                      <Plus className="mr-1 h-3 w-3" />
                      New HP Search
                    </Link>
                  </Button>
            ...
            >
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">
                    Training Jobs (
                    {trainingJobsPagination.isLoading && trainingJobsPagination.totalItems === 0
                      ? "..."
                      : trainingJobsPagination.totalItems}
                    )
                  </CardTitle>
                ...
                <CardContent>
                  {trainingJobsPagination.isLoading && trainingJobs.length === 0 ? (
                    <Skeleton className="h-24 w-full" />
                  ) : !trainingJobsPagination.isLoading && trainingJobs.length === 0 ? (
                    <p className="text-sm text-muted-foreground py-2 text-center">
                      No training jobs for this repository.
                    </p>
                  ) : (
                    <>
                      <Table>
                      ...
                      {renderPaginationControls(
                        trainingJobsPagination.currentPage,
                        trainingJobsPagination.totalItems,
                        ITEMS_PER_PAGE,
                        fetchPaginatedTrainingJobs,
                        trainingJobsPagination.isLoading
                      )}
                    </>
                  )}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">
                    HP Search Jobs (
                    {hpSearchJobsPagination.isLoading && hpSearchJobsPagination.totalItems === 0
                      ? "..."
                      : hpSearchJobsPagination.totalItems}
                    )
                  </CardTitle>
                ...
                <CardContent>
                  {hpSearchJobsPagination.isLoading && hpSearchJobs.length === 0 ? (
                    <Skeleton className="h-24 w-full" />
                  ) : !hpSearchJobsPagination.isLoading && hpSearchJobs.length === 0 ? (
                    <p className="text-sm text-muted-foreground py-2 text-center">
                      No HP search jobs for this repository.
                    </p>
                  ) : (
                    <>
                      <Table>
                          ...
                          {hpSearchJobs.map((job) => {
                            ...
                            return (
                              <TableRow key={`hp-${job.id}`}>
                                ...
                                <TableCell className="text-right">
                                  <Button variant="outline" size="sm" asChild>
                                    <Link href={`/jobs/${job.id}?type=hp_search`}>Details</Link>
                                  </Button>
                                </TableCell>
                              </TableRow>
                            );
                      ...
                      {renderPaginationControls(
                        hpSearchJobsPagination.currentPage,
                        hpSearchJobsPagination.totalItems,
                        ITEMS_PER_PAGE,
                        fetchPaginatedHpSearchJobs,
                        hpSearchJobsPagination.isLoading
                      )}
                    </>
                  )}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">
                    Inference Jobs (
                    {inferenceJobsPagination.isLoading && inferenceJobsPagination.totalItems === 0
                      ? "..."
                      : inferenceJobsPagination.totalItems}
                    )
                  </CardTitle>
                ...
                <CardContent>
                  {inferenceJobsPagination.isLoading && inferenceJobs.length === 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground py-2 text-center">
                      No inference jobs found for models related to this repository.
                    </p>
                  ) : (
                    <>
                      <Table>
                        ...
                        <TableBody>
                          {inferenceJobs.map((job) => {
                            const jobTask = getLatestTaskForEntity(
                              ...
                              "InferenceJob",
                              job.id
                            ); // No specific job_type for generic inference task status
                            return (
                              <TableRow key={`infer-${job.id}`}>
                                <TableCell
                                  className="font-mono text-xs max-w-[150px] sm:max-w-[200px] truncate"
                                  title={
                                    typeof job.input_reference === "object"
                                      ? JSON.stringify(job.input_reference)
                                      : String(job.input_reference)
                                  }
                                >
                                  {typeof job.input_reference === "object"
                                    ? job.input_reference.commit_hash
                                      ? `Commit: ${String(job.input_reference.commit_hash).substring(0, 8)}...`
                                      : JSON.stringify(job.input_reference)
                                    : String(job.input_reference)}
                                </TableCell>
                                <TableCell>
                                ...
                                <TableCell className="text-right">
                                  <Button variant="outline" size="sm" asChild>
                                    <Link href={`/prediction-insights/${job.id}`}>Insights</Link>
                                  </Button>
                                </TableCell>
                              </TableRow>
                            );
                      ...
                      {renderPaginationControls(
                        inferenceJobsPagination.currentPage,
                        inferenceJobsPagination.totalItems,
                        ITEMS_PER_PAGE,
                        fetchPaginatedInferenceJobs,
                        inferenceJobsPagination.isLoading
                      )}
                    </>
                  )}
              ...
              {botPatternsPagination.isLoading && botPatterns.length === 0 ? (
                ...
                <Card className="text-center py-10">
                  <CardContent className="flex flex-col items-center justify-center">
                    ...
                    <Button asChild>
                      <Link href={`/bot-patterns?repository=${repoId}`}>
                        <Plus className="mr-2 h-4 w-4" /> Add Specific Pattern
                      </Link>
                    </Button>
                    <p className="text-xs text-muted-foreground mt-2">
                      <Link href="/bot-patterns" className="hover:underline text-primary">
                        View/Manage Global Patterns
                      </Link>
                    </p>
                  </CardContent>
                </Card>
                ...
                <>
                  ...
                  {renderPaginationControls(
                    botPatternsPagination.currentPage,
                    botPatternsPagination.totalItems,
                    ITEMS_PER_PAGE,
                    fetchPaginatedBotPatterns,
                    botPatternsPagination.isLoading
                  )}
                  <div className="text-center text-xs text-muted-foreground mt-1">
                    {botPatternsPagination.totalItems > 0
                      ? `Showing ${botPatterns.length} of ${botPatternsPagination.totalItems} patterns`
                      : ""}
                  </div>
                </>
              )}
