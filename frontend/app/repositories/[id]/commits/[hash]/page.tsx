...
function CommitDetailPage() {
  ...
  const liveTaskStatus = getLatestTaskForEntity(
    taskStatuses,
    "CommitDetails",
    commitData?.details?.id || 0,
    "feature_extraction"
  );
  ...
  const fetchData = useCallback(
    async (showLoadingToast = false) => {
      ...
      if (showLoadingToast)
        toast({ title: "Refreshing commit data...", description: "Please wait..." });
  ...
  useEffect(() => {
    ...
    if (liveStatus && (liveStatus === "SUCCESS" || liveStatus === "FAILED")) {
      if (
        staticStatus !== CommitIngestionStatusEnum.COMPLETE &&
        staticStatus !== CommitIngestionStatusEnum.FAILED
      ) {
          ...
          <Card className="text-center py-10">
            <CardContent>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Details and metrics for this commit have not been processed yet.
              </p>
          ...
          <Card className="text-center py-10">
            <CardContent className="flex flex-col items-center">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Task ID: {liveTaskStatus?.task_id || commitData?.celery_ingestion_task_id || "N/A"}
              </p>
            </CardContent>
          </Card>
        );
      ...
      case CommitIngestionStatusEnum.FAILED:
        return (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {commitData?.details?.message || "An error occurred during ingestion."}
            </AlertDescription>
          </Alert>
        );
  ...
  const renderCompleteView = () => {
    ...
    if (!details)
      return <Alert variant="destructive">Data marked complete, but details are missing.</Alert>;
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              <CardTitle>Commit Info</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <p>{formatDate(details.author_date)}</p>
              </div>
                  ...
                  {details.parents.map((p) => (
                    <p key={p} className="truncate">
                      <Link
                        ...
                        href={`/repositories/${repoId}/commits/${p}`}
                      >
                        {p}
                      </Link>
                    </p>
              ...
              <div className="pt-2">
                <Label className="text-xs uppercase text-muted-foreground flex items-center">
                  <MessageSquare className="h-3.5 w-3.5 mr-1" />
                  Commit Message
                </Label>
                <ScrollArea className="h-32 rounded-md border bg-muted p-3 mt-1">
                  <pre className="whitespace-pre-wrap font-sans text-xs">{details.message}</pre>
                </ScrollArea>
              </div>
            </CardContent>
          </Card>
              ...
              {hasInferenceJobs ? (
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    This commit has been analyzed. Showing the last{" "}
                    {Math.min(5, inferenceJobs.length)} jobs.
                  </p>
                  <ul className="space-y-2">
                    {inferenceJobs.slice(0, 5).map((job) => (
                      <li
                        key={job.id}
                        className="text-xs flex justify-between items-center border-b pb-1 last:border-0"
                      >
                        <Link
                        ...
                        <Badge
                          variant={
                            job.prediction_result?.commit_prediction === 1
                              ? "destructive"
                              : "secondary"
                          }
                        >
                          {job.prediction_result?.commit_prediction === 1 ? "Defect" : "Clean"}
                        </Badge>
                      </li>
                    ))}
                  </ul>
                ...
                <p className="text-sm text-muted-foreground">
                  No inference jobs have been run for this commit yet.
                </p>
              )}
            </CardContent>
            <CardFooter>
              {hasInferenceJobs ? (
                ...
                <Button className="w-full" asChild>
                  <Link href={`/jobs/inference?repositoryId=${repoId}&commitHash=${commitHash}`}>
