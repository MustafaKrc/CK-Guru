...
const PredictionInsightsPage = () => {
  ...
  const fetchFilterOptions = useCallback(async () => {
    ...
    try {
      ...
      setModels(
        modelsResponse.items.map((m) => ({ value: String(m.id), label: `${m.name} v${m.version}` }))
      );
  ...
  const fetchInferenceJobs = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        if (repositoryFilter !== ALL_FILTER_VALUE)
          params.repository_id = parseInt(repositoryFilter);
  ...
  const getStatusBadge = (taskStatus?: TaskStatusUpdatePayload, staticStatus?: JobStatusEnum) => {
    const currentStatusInfo =
      taskStatus ||
      (staticStatus ? ({ status: staticStatus } as TaskStatusUpdatePayload) : undefined);
    if (!currentStatusInfo || !currentStatusInfo.status)
    ...
    switch (
      ...
      case JobStatusEnum.FAILED.toUpperCase():
        ...
        text = `Failed`;
        break; // Full message shown in tooltip/details
    ...
    return (
      <Badge
        ...
        title={status_message || String(status) || ""}
      >
        {icon}
        {text}
      </Badge>
    ...
    return jobs.map((job) => {
      ...
      return (
        <TableRow key={job.id}>
          ...
          <TableCell>
            <Link
              href={`/repositories/${job.ml_model?.dataset?.repository?.id}`}
              className="font-mono text-xs text-primary hover:underline"
            >
              {job.ml_model?.dataset?.repository?.name || "N/A"}
            </Link>
          </TableCell>
          <TableCell>
            <Link
              href={`/repositories/${job.ml_model?.dataset?.repository?.id}/commits/${job.input_reference.commit_hash}`}
              className="font-mono text-xs text-primary hover:underline"
            >
              {job.input_reference.commit_hash?.substring(0, 7) || "N/A"}
            </Link>
          </TableCell>
          <TableCell>
            <Link
              ...
              className="font-mono text-xs text-primary hover:underline"
            >
              {job.ml_model?.name || "N/A"} (v{job.ml_model?.version || "?"})
            </Link>
          </TableCell>
          ...
          <TableCell>
            ...
            {predictionProb !== undefined && (
              <p className="text-[10px] text-muted-foreground mt-1">
                Prob: {predictionProb.toFixed(3)}
              </p>
            )}
          </TableCell>
        </TableRow>
    ...
    <MainLayout>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          ...
          />
          <SearchableSelect
            options={[{ value: ALL_FILTER_VALUE, label: "All Repositories" }, ...repositories]}
          ...
          />
          <SearchableSelect
            options={[{ value: ALL_FILTER_VALUE, label: "All Models" }, ...models]}
            value={modelFilter}
          ...
          <div className="flex items-center gap-2 lg:col-start-5">
            <Label htmlFor="items-per-page" className="text-sm shrink-0">
              Show:
            </Label>
            <Select
              value={String(pagination.itemsPerPage)}
              onValueChange={(val) =>
                setPagination((p) => ({ ...p, itemsPerPage: parseInt(val), currentPage: 1 }))
              }
            >
        ...
        <div className="rounded-md border">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>
                  <SortableHeader sortKey="job_id" sortConfig={sortConfig} onSort={handleSort}>
                    Job ID
                  </SortableHeader>
                </TableHead>
                <TableHead>
                ...
                <TableHead>
                  <SortableHeader sortKey="commit" sortConfig={sortConfig} onSort={handleSort}>
                    Commit
                  </SortableHeader>
                </TableHead>
                <TableHead>
                  <SortableHeader sortKey="model_name" sortConfig={sortConfig} onSort={handleSort}>
                    Model Used
                  </SortableHeader>
                </TableHead>
                <TableHead>
                ...
                <TableHead>
                  <SortableHeader sortKey="prediction" sortConfig={sortConfig} onSort={handleSort}>
                    Prediction
                  </SortableHeader>
                </TableHead>
              </TableRow>
