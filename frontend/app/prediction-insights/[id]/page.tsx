...
const PredictionInsightDetailPage = () => {
  ...
  const fetchAllData = useCallback(
    async (showLoadingToast = false) => {
      ...
      if (showLoadingToast)
        toast.loading("Refreshing XAI data...", { id: `fetch-data-${inferenceJobId}` });
      ...
      try {
        ...
        const [jobDetailsResponse, xaiResultsResponse] = await Promise.all([
          jobDetailsPromise,
          xaiResultsPromise,
        ]);
        ...
        if (xaiResultsResponse && xaiResultsResponse.length > 0) {
          const currentActiveTabIsValid =
            activeTab &&
            xaiResultsResponse.some(
              (r) => r.xai_type === activeTab && r.status === XAIStatusEnum.SUCCESS
            );
          if (!currentActiveTabIsValid) {
            ...
            if (firstSuccess) {
              ...
              const firstPendingOrRunning = xaiResultsResponse.find(
                (r) => r.status === XAIStatusEnum.PENDING || r.status === XAIStatusEnum.RUNNING
              );
        ...
        if (showLoadingToast)
          toast.success("Data refreshed!", { id: `fetch-data-${inferenceJobId}` });
      } catch (err) {
  ...
  const getXAIStatusBadge = (status: XAIStatusEnum, message?: string | null) => {
    ...
    switch (status.toLowerCase()) {
      case XAIStatusEnum.SUCCESS:
        icon = <CheckCircledIcon className="mr-1 h-3 w-3" />;
        variant = "default";
        text = "Success";
        break;
      ...
      case XAIStatusEnum.PENDING:
        icon = <ReloadIcon className="mr-1 h-3 w-3 animate-spin" />;
        variant = "outline";
        text = "Pending";
        break;
    ...
    return (
      <Badge
        variant={variant}
        className="ml-2 text-xs whitespace-nowrap py-0.5 px-1.5"
  ...
  const xaiTypeIcons: Record<XAITypeEnum, React.ReactNode> = {
    [XAITypeEnum.FEATURE_IMPORTANCE]: (
      <TargetIcon className="mr-1 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4" />
    ),
    ...
    [XAITypeEnum.COUNTERFACTUALS]: (
      <ShuffleIcon className="mr-1 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4" />
    ),
  ...
  useEffect(() => {
    ...
    if (!activeTab && availableXaiTabs.length > 0) {
      const firstSuccess = availableXaiTabs.find(
        (type) => xaiResults.find((r) => r.xai_type === type)?.status === XAIStatusEnum.SUCCESS
      );
      if (firstSuccess) {
        setActiveTab(firstSuccess);
      } else {
        const firstInProgress = availableXaiTabs.find(
          (type) =>
            xaiResults.find((r) => r.xai_type === type)?.status === XAIStatusEnum.RUNNING ||
            xaiResults.find((r) => r.xai_type === type)?.status === XAIStatusEnum.PENDING
        );
        setActiveTab(firstInProgress || availableXaiTabs[0] || XAITypeEnum.FEATURE_IMPORTANCE);
      }
    } else if (
      activeTab &&
      !availableXaiTabs.includes(activeTab as XAITypeEnum) &&
      availableXaiTabs.length > 0
    ) {
  ...
  const renderXAIContent = (xaiType: XAITypeEnum) => {
    ...
    if (!result)
      return (
        <Alert variant="default" className="mt-4">
          ...
          No data available yet for this explanation type. Try generating explanations.
        </Alert>
      );
    ...
    switch (result.status) {
        ...
        if (!result.result_data)
          return (
            <Alert variant="default" className="mt-4">
              ...
              Explanation data is missing though status is success.
            </Alert>
          );
        switch (xaiType) {
          case XAITypeEnum.FEATURE_IMPORTANCE:
            return (
              <FeatureImportanceDisplay data={result.result_data as FeatureImportanceResultData} />
            );
          ...
          default:
            return (
              <Alert variant="default" className="mt-4">
                Unsupported XAI type for display: {xaiType}
              </Alert>
            );
        }
      case XAIStatusEnum.PENDING:
          ...
          <Alert variant="default" className="flex items-center mt-4 py-6 justify-center">
            ...
            <div className="text-center">
              <p className="font-medium">
                Explanation generation is {result.status.toLowerCase()}.
              </p>
              <p className="text-xs text-muted-foreground">
                {result.status_message || "Please check back soon."}
              </p>
            </div>
          </Alert>
        );
        ...
        return (
          <Alert variant="destructive" className="mt-4 py-6">
            ...
            <div>
              ...
              <AlertDescription>
                {result.status_message || "An unknown error occurred."}
              </AlertDescription>
            </div>
          </Alert>
        );
      default:
        return (
          <Alert variant="default" className="mt-4">
            {result.status_message || `Status: ${result.status}`}
          </Alert>
        );
    }
  };
  ...
  const renderMainContent = () => {
    if (!inferenceJobDetails) {
      ...
      return (
        <Alert variant="default">
          <InfoCircledIcon className="h-4 w-4" /> Inference job details not found or still loading.
        </Alert>
      );
    }
    ...
    const outcomeText =
      predictionOutcome === 1 ? "Defect-Prone" : predictionOutcome === 0 ? "Clean" : "N/A";
    const outcomeVariant: "default" | "destructive" | "secondary" =
      predictionOutcome === 1 ? "destructive" : predictionOutcome === 0 ? "default" : "secondary";
      ...
      <>
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              <div className="mb-3 md:mb-0">
                <CardTitle className="text-xl md:text-2xl">Inference Overview</CardTitle>
              ...
              {inferenceJobDetails.prediction_result && (
                <Badge
                  ...
                  className="text-sm md:text-base px-3 py-1.5 self-start md:self-center"
                >
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 text-sm">
            <div className="space-y-1 p-3 border rounded-md bg-muted/30">
              ...
              <p className="font-semibold">
                <Link
                  href={`/models/${inferenceJobDetails.ml_model_id}`}
                  className="text-primary hover:underline"
                >
                  {inferenceJobDetails.ml_model_id}
                </Link>
              </p>
            </div>
            <div className="space-y-1 p-3 border rounded-md bg-muted/30">
              ...
              inferenceJobDetails.input_reference?.repo_id ? (
                <Link
                  href={`/repositories/${inferenceJobDetails.input_reference.repo_id}/commits/${inferenceJobDetails.input_reference.commit_hash}`}
                  className="text-primary hover:underline"
                >
                  <p
                    className="font-mono text-xs"
                    title={String(inferenceJobDetails.input_reference.commit_hash)}
                  >
            ...
            <div className="space-y-1 p-3 border rounded-md bg-muted/30">
              ...
              <p className="font-semibold">
                {inferenceJobDetails.prediction_result?.max_bug_probability?.toFixed(4) ?? "N/A"}
              </p>
            </div>
            <div className="space-y-1 p-3 border rounded-md bg-muted/30">
              <p className="text-xs text-muted-foreground">Files Analyzed</p>
              <p className="font-semibold">
                {inferenceJobDetails.prediction_result?.num_files_analyzed ?? "N/A"}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        {inferenceJobDetails.status !== JobStatusEnum.SUCCESS ? (
          <Alert variant="default" className="mb-6">
            ...
            <AlertDescription>
              Explainability features can only be generated for successfully completed inference
              jobs. Current job status: {getJobStatusBadge(inferenceJobDetails.status)}.
            </AlertDescription>
          </Alert>
          ...
          <Tabs defaultValue={XAITypeEnum.FEATURE_IMPORTANCE} className="w-full">
            ...
            <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1 bg-muted p-1 rounded-lg">
              {Object.values(XAITypeEnum).map((type) => (
                <TabsTrigger
                  ...
                  key={type}
                  disabled
                  className="h-auto py-2 px-1.5 text-xs sm:text-sm"
                >
            ...
            <TabsContent value={XAITypeEnum.FEATURE_IMPORTANCE} className="mt-4">
          ...
          !isLoading && (
            <Card className="mt-6">
              ...
              <CardContent>
                ...
                <p className="mt-2">
                  If the inference job was successful, you can try generating them using the button
    ...
    return (
      <MainLayout>
        <PageContainer title="Error Loading Insights" description={error}>
          ...
          <Button onClick={() => fetchAllData(true)} className="mt-4">
            Try Again
          </Button>
        </PageContainer>
      </MainLayout>
      ...
      <PageContainer
        ...
        actions={
          <div className="flex gap-2">
            ...
            >
              <ReloadIcon
                className={`mr-2 h-4 w-4 ${isLoading || isTriggering ? "animate-spin" : ""}`}
              />
              Refresh
            </Button>
            {inferenceJobDetails?.status === JobStatusEnum.SUCCESS && (
              <Button onClick={handleTriggerXAI} disabled={isTriggering || isLoading} size="sm">
                ...
                {xaiResults &&
                xaiResults.filter((r) => r.status === XAIStatusEnum.SUCCESS).length > 0
                  ? "Regenerate All"
