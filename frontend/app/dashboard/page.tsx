...
type RecentJobItem = (TrainingJobRead | HPSearchJobRead | InferenceJobRead) & {
  jobType: "Training" | "HP Search" | "Inference";
};
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const trainingJobsPromise = apiService.getTrainingJobs({
          limit: RECENT_ITEMS_LIMIT,
          skip: 0,
        });
        const hpSearchJobsPromise = apiService.getHpSearchJobs({
          limit: RECENT_ITEMS_LIMIT,
          skip: 0,
        });
        const inferenceJobsPromise = apiService.getInferenceJobs({
          limit: RECENT_ITEMS_LIMIT,
          skip: 0,
        });

        const [summaryStats, reposResponse, trainingJobsRes, hpSearchJobsRes, inferenceJobsRes] =
          await Promise.all([
            summaryStatsPromise,
            reposPromise,
            trainingJobsPromise,
        ...
        const combinedJobs: RecentJobItem[] = [
          ...
          ...(hpSearchJobsRes.items || []).map((job) => ({
            ...job,
            jobType: "HP Search" as const,
          })),
          ...(inferenceJobsRes.items || []).map((job) => ({
            ...job,
  ...
  const getRepoIngestionStatusText = (repo: Repository) => {
    const task = getLatestTaskForEntity(
      taskStatuses,
      "Repository",
      repo.id,
    ...
    if (
      task &&
      (task.status.toUpperCase() === "RUNNING" || task.status.toUpperCase() === "PENDING")
    ) {
      return "Ingesting";
    }
  ...
  const getJobStatusBadge = (job: RecentJobItem) => {
    ...
    switch (job.jobType) {
      case "Training":
        ...
        taskJobType = "model_training";
        break;
      case "HP Search":
        entityType = "HPSearchJob";
        taskJobType = "hp_search";
        break;
      case "Inference":
        entityType = "InferenceJob";
        taskJobType = "model_inference";
        break;
      default:
    ...
    switch (String(displayStatus).toLowerCase()) {
      ...
      case JobStatusEnum.STARTED:
        ...
        textToShow = `${displayMessage || displayStatus} (${progress ?? 0}%)`;
        break;
    ...
    return (
      <Badge variant={badgeVariant} title={displayMessage || String(displayStatus)}>
        {icon}
        {textToShow}
      </Badge>
    );
  ...
  const getJobName = (job: RecentJobItem): string => {
    if (job.jobType === "Training")
      return (job as TrainingJobRead).config.model_name || "Training Job";
    if (job.jobType === "HP Search")
      return (job as HPSearchJobRead).optuna_study_name || "HP Search Job";
    if (job.jobType === "Inference") {
      const inputRef = (job as InferenceJobRead).input_reference;
      const commitHash = inputRef?.commit_hash
        ? String(inputRef.commit_hash).substring(0, 7)
        : "N/A";
            ...
            <CardContent>
              {isLoadingStats ? (
                ...
                <div className="text-2xl font-bold">
                  {stats?.total_repositories ?? <Skeleton className="h-7 w-10 inline-block" />}
                </div>
              )}
              {isLoadingStats ? (
                ...
                <p className="text-xs text-muted-foreground">
                  {stats?.active_ingestion_tasks ?? 0} active ingestion tasks
                </p>
              )}
            </CardContent>
          </Card>
          <Card className="metric-card-alt border-accent/20">
            ...
            <CardContent>
              {isLoadingStats ? (
                ...
                <div className="text-2xl font-bold">
                  {stats?.total_datasets ?? <Skeleton className="h-7 w-10 inline-block" />}
                </div>
              )}
              {isLoadingStats ? (
                <Skeleton className="h-4 w-3/4" />
                ...
                <p className="text-xs text-muted-foreground">
                  {stats?.datasets_by_status.ready ?? 0} ready,{" "}
                  {stats?.active_dataset_generation_tasks ?? 0} processing
                </p>
              )}
            </CardContent>
          </Card>
          <Card className="metric-card border-primary/20">
            ...
            <CardContent>
              {isLoadingStats ? (
                ...
                <div className="text-2xl font-bold">
                  {stats?.total_ml_models ?? <Skeleton className="h-7 w-10 inline-block" />}
                </div>
              )}
              {isLoadingStats ? (
                <Skeleton className="h-4 w-3/4" />
                ...
                <p className="text-xs text-muted-foreground">
                  Avg. F1: {stats?.average_f1_score_ml_models?.toFixed(2) ?? "N/A"} (
                  {stats?.active_ml_jobs ?? 0} active jobs)
                </p>
              )}
            </CardContent>
          </Card>
              ...
              {isLoadingRecentRepos ? (
                ...
                <div className="space-y-3">
                  {recentRepositories.map((repo) => (
                    <div
                      key={repo.id}
                      className="flex items-center justify-between text-sm p-2 border-b last:border-b-0"
                    >
                      <div>
                        <Link
                      ...
                      <Badge
                        variant={
                          getRepoIngestionStatusText(repo) === "Ingesting"
                            ? "outline"
                            : getRepoIngestionStatusText(repo) === "Ingested"
                              ? "default"
                              : "secondary"
                        }
                        className="text-xs"
