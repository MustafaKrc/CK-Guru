      ...
      <Card className="h-full">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            No {title.toLowerCase()} available for this model.
          </p>
        </CardContent>
      </Card>
    );
        ...
        <ScrollArea className="h-[200px] pr-3">
          ...
          <dl className="space-y-1.5 text-sm">
            {Object.entries(data).map(([key, value]) => (
              <div
                key={key}
                className="flex justify-between items-start border-b border-dashed pb-1 last:border-b-0"
              >
                <dt className="text-muted-foreground break-words mr-2" title={key}>
                  {key.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())}:
                </dt>
                <dd className="font-mono text-right break-all" title={String(value)}>
                  {String(value)}
                </dd>
              </div>
            ))}
...
function ModelDetailPageContent() {
    ...
    async (page: number) => {
      ...
      try {
        ...
        const response = await apiService.get<PaginatedInferenceJobRead>(
          `/ml/infer?model_id=${modelId}&skip=${skip}&limit=${ITEMS_PER_PAGE}`
        );
  ...
  useEffect(() => {
    if (model && activeTab === "inference-jobs") {
      // Fetch jobs only when model is loaded and tab is active
  ...
  const handleDeleteModel = async () => {
    ...
    try {
      ...
      toast({
        title: "Model Deleted",
        description: `Model ${model.name} (v${model.version}) has been deleted.`,
      });
      router.push("/models");
    } catch (err) {
  ...
  const handleCopyArtifactPath = () => {
    if (model?.s3_artifact_path) {
      navigator.clipboard
        .writeText(model.s3_artifact_path)
      ...
      <MainLayout>
        ...
        >
          <Alert variant="destructive">
            ...
            <AlertDescription>Please check the model ID or try again later.</AlertDescription>
          </Alert>
        </PageContainer>
      </MainLayout>
    );
    ...
    <div className="flex items-center gap-2">
      ...
      <AlertDialog>
        <AlertDialogTrigger asChild>
          <Button variant="destructive" size="sm" disabled={isDeletingModel}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the model and its artifact.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="inference-jobs">
              Inference Jobs (
          ...
          <TabsContent value="details" className="space-y-6">
              ...
              <CardContent className="grid md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                ...
                {model.s3_artifact_path && (
                  <div className="col-span-full">
                    ...
                    <div className="flex items-center gap-2">
                      <p
                        className="font-mono text-xs truncate flex-grow"
                        title={model.s3_artifact_path}
                      >
                        {model.s3_artifact_path}
                      </p>
                      <Button
                ...
                {model.dataset_id && (
                  <div>
                    ...
                    {model.dataset?.name && (
                      <p className="text-xs text-muted-foreground ml-5">{model.dataset.name}</p>
                    )}
                  </div>
                )}
                {model.training_job_id && (
                  <div>
                    ...
                    <p>
                      <Link
                        href={`/jobs/${model.training_job_id}?type=training`}
                        className="text-primary hover:underline flex items-center"
                      >
                        <Puzzle className="inline h-3.5 w-3.5 mr-1" />
                        Job ID: {model.training_job_id}
                      </Link>
                    </p>
                    {model.training_job?.config?.model_name && (
                ...
                {model.hp_search_job_id && (
                  <div>
                    ...
                    <p>
                      <Link
                        href={`/jobs/${model.hp_search_job_id}?type=hp_search`}
                        className="text-primary hover:underline flex items-center"
                      >
                        <Layers className="inline h-3.5 w-3.5 mr-1" />
                        Job ID: {model.hp_search_job_id}
                      </Link>
                    </p>
                    {model.hp_search_job?.optuna_study_name && (
                      <p className="text-xs text-muted-foreground ml-5">
                        ({model.hp_search_job.optuna_study_name})
                      </p>
                    )}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <KeyValueDisplay
                ...
                icon={<Cog className="h-5 w-5 text-primary" />} /*isLoading={isLoadingModel}*/
              />
              <KeyValueDisplay
                data={model.performance_metrics}
                title="Performance Metrics"
                icon={<Brain className="h-5 w-5 text-primary" />} /*isLoading={isLoadingModel}*/
              />
            </div>
          </TabsContent>
          ...
          <TabsContent value="inference-jobs" className="space-y-4">
            <PageContainer
              ...
              description={
                inferenceJobsPagination.isLoading && inferenceJobsPagination.totalItems === 0
                  ? "Loading jobs..."
                  : `Showing ${inferenceJobs.length} of ${inferenceJobsPagination.totalItems} jobs.`
              }
            >
              {inferenceJobsPagination.isLoading && inferenceJobs.length === 0 ? (
                ...
                <Card className="text-center py-10">
                  <CardContent className="flex flex-col items-center justify-center">
                    ...
                    <p className="text-muted-foreground mb-3">
                      No inference jobs have used this model yet.
                    </p>
                    <Button onClick={handleRunInference}>
                      <Play className="mr-2 h-4 w-4" /> Run First Inference
                    </Button>
                  </CardContent>
                </Card>
              ) : (
                <>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead>Input (Commit)</TableHead>
                        ...
                        {inferenceJobs.map((job) => {
                          ...
                          const commitHash =
                            ...
                            job.input_reference.commit_hash
                              ...
                              : typeof job.input_reference === "string"
                                ? String(job.input_reference).substring(0, 8) + "..."
                                : "N/A";
                          const fullCommitHash =
                            typeof job.input_reference === "object" &&
                            job.input_reference.commit_hash
                              ? String(job.input_reference.commit_hash)
                              : "N/A";
                          return (
                            <TableRow key={job.id}>
                              <TableCell className="font-mono">{job.id}</TableCell>
                              <TableCell className="font-mono" title={fullCommitHash}>
                                {commitHash}
                              </TableCell>
                              <TableCell>
                  ...
                  {renderPaginationControls(
                    ...
                    ITEMS_PER_PAGE,
                    (page) =>
                      setInferenceJobsPagination((prev) => ({ ...prev, currentPage: page })),
