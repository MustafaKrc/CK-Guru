...
import { MoreHorizontal, Eye, Play, BarChart3, Plus, Loader2, AlertCircle, ArrowUpDown } from "lucide-react";
...
export default function ModelsPage() {
  ...
  const fetchDependencies = useCallback(async () => {
    ...
    try {
      const dsResponse = await apiService.get<PaginatedDatasetRead>(`/datasets?limit=500&status=ready`); // Fetch enough for selection
  ...
  useEffect(() => {
    fetchModels(1, pagination.itemsPerPage, debouncedSearchQuery, typeFilter, datasetFilter, sortConfig);
  ...
  useEffect(() => {
    fetchModels(pagination.currentPage, pagination.itemsPerPage, debouncedSearchQuery, typeFilter, datasetFilter, sortConfig);
  ...
  const renderPaginationControls = () => {
    ...
    return (
      <Pagination className="mt-4">
        <PaginationContent>
          <PaginationItem><PaginationPrevious onClick={() => setPagination(p => ({...p, currentPage: Math.max(1, p.currentPage - 1)}))} aria-disabled={pagination.currentPage <= 1} /></PaginationItem>
          ...
          <PaginationItem><PaginationNext onClick={() => setPagination(p => ({...p, currentPage: Math.min(totalPages, p.currentPage + 1)}))} aria-disabled={pagination.currentPage >= totalPages} /></PaginationItem>
        </PaginationContent>
      </Pagination>
    );
  };
  ...
  const renderContent = () => {
    if (pagination.isLoading) {
      return Array.from({ length: 5 }).map((_, i) => <TableRow key={`skel-${i}`}><TableCell colSpan={7}><Skeleton className="h-8 w-full"/></TableCell></TableRow>);
    }
    if (fetchError) {
      return <TableRow><TableCell colSpan={7} className="text-center text-destructive py-6">{fetchError}</TableCell></TableRow>;
    }
    if (models.length === 0) {
      return <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No models found matching your criteria.</TableCell></TableRow>;
    }

    return models.map((model) => (
      <TableRow key={model.id}>
        <TableCell className="font-medium max-w-[200px] truncate"><Link href={`/models/${model.id}`} className="hover:underline text-primary">{model.name}</Link></TableCell>
        ...
        <TableCell><Badge variant="secondary" className="text-xs">F1: {Number(model.performance_metrics?.f1_weighted ?? 0).toFixed(3)}</Badge></TableCell>
        <TableCell className="text-right">
          <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon"><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent>
            <DropdownMenuItem asChild><Link href={`/models/${model.id}`}><Eye className="mr-2 h-4 w-4"/>View Details</Link></DropdownMenuItem>
            <DropdownMenuItem asChild><Link href={`/jobs/inference?modelId=${model.id}`}><Play className="mr-2 h-4 w-4"/>Run Inference</Link></DropdownMenuItem>
  ...
  return (
    <MainLayout>
      <PageContainer
        ...
        actions={<Button asChild><Link href="/jobs/train"><Plus className="mr-2 h-4 w-4" />Train New Model</Link></Button>}
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <Input placeholder="Search by model name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="md:col-span-2 lg:col-span-1" />
          <Select value={typeFilter} onValueChange={(v) => setTypeFilter(v as any)}><SelectTrigger><SelectValue placeholder="All Model Types" /></SelectTrigger><SelectContent><SelectItem value={ALL_FILTER_VALUE}>All Types</SelectItem>{availableModelTypes.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select>
          <SearchableSelect
          ...
          <div className="flex items-center gap-2 md:col-start-2 lg:col-start-4">
            <Label htmlFor="items-per-page-models" className="text-sm shrink-0">Show:</Label>
            <Select value={String(pagination.itemsPerPage)} onValueChange={handleItemsPerPageChange}>
              <SelectTrigger id="items-per-page-models" className="w-full md:w-[80px]"><SelectValue /></SelectTrigger>
        ...
        <div className="rounded-md border">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead><SortableHeader sortKey="name" sortConfig={sortConfig} onSort={handleSort}>Model Name</SortableHeader></TableHead>
                <TableHead><SortableHeader sortKey="version" sortConfig={sortConfig} onSort={handleSort}>Version</SortableHeader></TableHead>
                <TableHead><SortableHeader sortKey="model_type" sortConfig={sortConfig} onSort={handleSort}>Type</SortableHeader></TableHead>
                <TableHead><SortableHeader sortKey="dataset" sortConfig={sortConfig} onSort={handleSort}>Dataset</SortableHeader></TableHead>
