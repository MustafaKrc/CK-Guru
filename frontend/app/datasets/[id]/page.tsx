...
const KeyValueItem: React.FC<{ label: string; value?: string | number | null | boolean }> = ({
  label,
  value,
}) => (
...
function DatasetDetailPageContent() {
  ...
  const fetchDataPreview = useCallback(
    async (page: number, storagePath?: string | null, explicitStatus?: string | null) => {
    ...
    async (showToast = false) => {
      ...
      try {
        ...
        if (fetchedDataset?.repository_id) {
          try {
            const repoData = await apiService.get<Repository>(
              `/repositories/${fetchedDataset.repository_id}`
            );
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      const response = await apiService.get<PaginatedMLModelRead>(
        `/ml/models?dataset_id=${datasetId}&limit=100`
      );
  ...
  useEffect(() => {
    ...
    if (
      liveDatasetStatus &&
      (liveStatusUpper === JobStatusEnum.SUCCESS.toUpperCase() ||
        liveStatusUpper === JobStatusEnum.FAILED.toUpperCase())
    ) {
      const isStaticFinal =
        staticStatus === DatasetStatusEnum.READY.toUpperCase() ||
        staticStatus === DatasetStatusEnum.FAILED.toUpperCase();
  ...
  const getDisplayDatasetStatusInfo = useCallback(() => {
    ...
    const currentStatusInfo = liveDatasetStatus || {
    ...
    if (
      displayStatus === JobStatusEnum.SUCCESS.toUpperCase() ||
      displayStatus === DatasetStatusEnum.READY.toUpperCase()
    ) {
      displayMessage = "Ready";
      badgeVariant = "default";
      icon = <CheckCircle className="h-4 w-4 text-green-600" />;
    } else if (
      displayStatus === JobStatusEnum.PENDING.toUpperCase() ||
      displayStatus === DatasetStatusEnum.PENDING.toUpperCase()
    ) {
      displayMessage = currentStatusInfo.status_message || "Pending";
      badgeVariant = "outline";
      icon = <Loader2 className="h-4 w-4 animate-spin" />;
    } else if (
      displayStatus === JobStatusEnum.RUNNING.toUpperCase() ||
      displayStatus === JobStatusEnum.STARTED.toUpperCase() ||
      displayStatus === DatasetStatusEnum.GENERATING.toUpperCase()
    ) {
      badgeVariant = "outline";
      ...
      displayStatus === JobStatusEnum.FAILED.toUpperCase() ||
      displayStatus === DatasetStatusEnum.FAILED.toUpperCase()
    ) {
      ...
      badgeVariant =
        liveDatasetStatus &&
        ["RUNNING", "PENDING", "STARTED"].includes(liveDatasetStatus.status.toUpperCase())
          ? "outline"
          : "secondary";
      if (badgeVariant === "outline" && (!icon || icon.type === Info))
        icon = <RefreshCw className="h-4 w-4 animate-spin" />;
    }
  ...
  const isDatasetProcessing =
    displayDatasetStatus.icon?.type === RefreshCw || displayDatasetStatus.icon?.type === Loader2;
  ...
  // --- EARLY RETURNS ---
  if (isLoadingDataset && !dataset && !datasetError) {
    // Initial loading state before dataset is fetched or error occurs
    return <PageLoader message="Loading dataset details..." />;
  }

  if (datasetError || !dataset) {
    // Error state or dataset genuinely not found after fetch attempt
    return (
      <MainLayout>
        <PageContainer
          title="Dataset Not Found"
          description={datasetError || "The requested dataset could not be found or loaded."}
          actions={
        ...
        >
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {datasetError || "Please check the dataset ID or try again."}
            </AlertDescription>
          </Alert>
        </PageContainer>
      </MainLayout>
  ...
  const handleDeleteDataset = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Dataset ${dataset.name} has been marked for deletion.`,
      });
  ...
  const handleRegenerateDataset = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Redirecting to new dataset page... Task ID: ${taskResponse.task_id}`,
      });
  ...
  const pageActions = (
    <div className="flex items-center gap-2">
      <Button
        variant="outline"
        size="sm"
        onClick={() => fetchDatasetDetails(true)}
        disabled={isLoadingDataset || isDatasetProcessing}
      >
        <RefreshCw
          className={`mr-2 h-4 w-4 ${isLoadingDataset || isDatasetProcessing ? "animate-spin" : ""}`}
        />
        Refresh
      </Button>
      <AlertDialog>
        <AlertDialogTrigger asChild>
          <Button variant="destructive" size="sm" disabled={isDatasetProcessing}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the dataset definition and queue data deletion from
      ...
      <PageContainer
        ...
        description={
          <div className="flex items-center gap-2">
            {repository ? (
              <Link
                href={`/repositories/${dataset.repository_id}`}
            ...
            <Badge
              variant={displayDatasetStatus.badgeVariant}
              className="text-xs px-2 py-0.5"
              title={displayDatasetStatus.text}
            >
              {displayDatasetStatus.icon && (
                <span className="mr-1.5">{displayDatasetStatus.icon}</span>
              )}
              {displayDatasetStatus.text.length > 50
                ? displayDatasetStatus.text.substring(0, 47) + "..."
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            {" "}
            {/* Retains its space-y-6 for internal spacing */}
            <Card>
              <CardHeader>
              ...
              <CardContent className="text-sm space-y-3">
                <div>
                  <Label className="text-xs text-muted-foreground uppercase flex items-center tracking-wider">
                    <TargetIcon className="h-3.5 w-3.5 mr-1.5" />
                    Target Column
                  </Label>{" "}
                  <Badge variant="secondary" className="mt-0.5">
                    {dataset.config.target_column}
                  </Badge>
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground uppercase flex items-center tracking-wider">
                    <ListFilter className="h-3.5 w-3.5 mr-1.5" />
                    Feature Columns ({dataset.config.feature_columns.length})
                  </Label>
                  <ScrollArea className="h-20 mt-1 rounded-md border p-2 text-xs bg-muted/10">
                    <ul className="list-disc list-inside pl-2 space-y-0.5">
                      {dataset.config.feature_columns.map((fc) => (
                        <li key={fc} className="truncate" title={fc}>
                          {fc}
                        </li>
                      ))}
                    </ul>
                ...
                <div>
                  <Label className="text-xs text-muted-foreground uppercase flex items-center tracking-wider">
                    ...
                    Cleaning Rules ({
                      dataset.config.cleaning_rules.filter((r) => r.enabled).length
                    }{" "}
                    enabled)
                  </Label>
                  {dataset.config.cleaning_rules.filter((r) => r.enabled).length > 0 ? (
                    <ScrollArea className="h-28 mt-1 rounded-md border p-2 text-xs bg-muted/10">
                      <ul className="space-y-1.5">
                        {dataset.config.cleaning_rules
                          .filter((r) => r.enabled)
                          .map((rule) => (
                            <li
                              key={rule.name}
                              className="border-b border-dashed pb-1 last:border-b-0"
                            >
                                ...
                                <dl className="mt-0.5 pl-3 text-xs">
                                  {Object.entries(rule.params).map(([paramKey, paramValue]) => (
                                    <div key={paramKey} className="flex">
                                      <dt className="text-muted-foreground w-20 truncate">
                                        {paramKey}:
                                      </dt>
                    ...
                    <p className="text-xs text-muted-foreground italic mt-1">
                      No cleaning rules were enabled.
                    </p>
                  )}
                </div>
                    ...
                    <div className="mt-1 space-y-2">
                      <div>
                        <span className="font-semibold text-sm">
                          {dataset.config.feature_selection.name}
                        </span>
                      </div>
                      {Object.keys(dataset.config.feature_selection.params).length > 0 ? (
                        <div className="mt-1 space-y-1 rounded-md border p-2 bg-muted/20">
                          {Object.entries(dataset.config.feature_selection.params).map(
                            ([key, value]) => (
                ...
                {dataset.description && (
                  <div>
                    ...
                    <p className="text-xs italic bg-muted/10 p-2 rounded-md whitespace-pre-wrap">
                      {dataset.description}
                    </p>
                  </div>
                )}
                <div className="flex justify-between pt-2 border-t text-xs text-muted-foreground">
                  ...
                  <p>Updated: {formatDate(dataset.updated_at)}</p>
                </div>
              </CardContent>
              <CardFooter className="flex-col items-stretch gap-2">
                <Button
                  ...
                  disabled={isDatasetProcessing}
                >
                  <RefreshCw
                    className={`mr-2 h-4 w-4 ${isDatasetProcessing && displayDatasetStatus.text.toLowerCase().includes("generating") ? "animate-spin" : ""}`}
                  />{" "}
                  Re-generate Dataset
                </Button>
              </CardFooter>
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    Optimize model performance for this dataset.
                  </CardDescription>
                </CardHeader>
                <CardFooter className="pt-3">
                  {" "}
                  {/* Added pt-3 for a bit more space above button */}
                  <Button size="sm" className="w-full" asChild>
                    <Link href={`/jobs/hp-search?datasetId=${datasetId}`}>
              ...
              <CardHeader>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                  ...
                  <Button
                    ...
                    disabled={
                      !isDatasetEffectivelyReady || isLoadingPreview || totalDatasetRows === 0
                    }
                    size="sm"
                  >
              ...
              <CardContent className="flex-grow">
                ...
                {!isLoadingPreview && !previewError && !isDatasetEffectivelyReady && (
                  <div className="text-center py-10 text-muted-foreground">
                    Dataset not ready for preview. Current status: {displayDatasetStatus.text}
                  </div>
                )}
                {!isLoadingPreview &&
                  !previewError &&
                  ...
                  dataPreview.length === 0 && (
                    <div className="text-center py-10 text-muted-foreground">
                      {totalDatasetRows > 0
                        ? "Could not load preview data, though dataset reports rows."
                        : "No data to preview (dataset is empty or preview could not be loaded)."}
                    </div>
                  )}
                {!isLoadingPreview &&
                  !previewError &&
                  ...
                  dataPreview.length > 0 && (
                    <>
                          ...
                          <TableHeader>
                            <TableRow>
                              {previewColumns.map((col) => (
                                <TableHead
                                  key={col}
                                  className="text-xs px-2 py-1 whitespace-nowrap font-semibold"
                                >
                                  {col}
                                </TableHead>
                              ))}
                            ...
                            {dataPreview.map((row, rowIndex) => (
                              <TableRow key={`row-${rowIndex}`}>
                                {previewColumns.map((col) => (
                                  <TableCell
                                    key={`${col}-${rowIndex}`}
                                    className="text-xs px-2 py-1 whitespace-nowrap truncate max-w-[150px]"
                      ...
                      {totalPagesForPreview > 1 && (
                        <Pagination>
                          <PaginationContent>
                            <PaginationItem>
                              <PaginationPrevious
                                onClick={() => handlePreviewPageChange(currentPage - 1)}
                            ...
                            {Array.from({ length: totalPagesForPreview }, (_, i) => i + 1)
                              .filter(
                                (pn) =>
                                  totalPagesForPreview <= 7 ||
                                  pn === 1 ||
                                  pn === totalPagesForPreview ||
                                  (pn >= currentPage - 2 && pn <= currentPage + 2)
                              )
                              .map((pn, idx, arr) => {
                                ...
                                return (
                                  <React.Fragment key={`page-frag-${pn}`}>
                                    ...
                                    <PaginationItem>
                                      <PaginationLink
                                        onClick={() => handlePreviewPageChange(pn)}
                            ...
                            <PaginationItem>
                              <PaginationNext
                                onClick={() => handlePreviewPageChange(currentPage + 1)}
                                aria-disabled={currentPage >= totalPagesForPreview}
                                className={
                                  currentPage >= totalPagesForPreview
                      ...
                      <div className="text-center text-sm text-muted-foreground mt-2">
                        {totalDatasetRows > 0
                          ? `Page ${currentPage} of ${totalPagesForPreview}`
                          : "No rows to display"}
                      </div>
                    </>
                  )}
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center text-base">
                  <Puzzle className="mr-2 h-4 w-4 text-primary" />
                  Models Trained
                </CardTitle>
                <CardDescription className="text-xs">
                  ML models trained using this dataset.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isLoadingModels ? (
                  ...
                  <p className="text-sm text-muted-foreground py-3 text-center">
                    No models trained with this dataset.
                  </p>
                ) : (
                  <ScrollArea className="h-32">
                    ...
                    <ul className="space-y-1 pr-2">
                      {modelsTrained.map((model) => (
                        <li
                          key={model.id}
                          className="text-sm flex justify-between items-center py-1.5 border-b last:border-b-0"
                        >
                          <Link
              ...
              {isDatasetEffectivelyReady && (
                <CardFooter className="pt-2">
                  <Button size="sm" className="w-full" asChild>
                    <Link href={`/jobs/train?datasetId=${datasetId}`}>
