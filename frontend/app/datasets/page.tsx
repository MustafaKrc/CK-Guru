...
export default function DatasetsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<DatasetStatusEnum | typeof ALL_FILTER_VALUE>(
    ALL_FILTER_VALUE
  );
  ...
  const [selectedDatasetForDelete, setSelectedDatasetForDelete] = useState<DatasetRead | null>(
    null
  );
    ...
    async (
      ...
      try {
        const response = await apiService.getDatasets({
          ...
          limit: limitToFetch,
          status:
            currentStatus === ALL_FILTER_VALUE ? undefined : (currentStatus as DatasetStatusEnum),
  ...
  const handleDeleteDataset = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Dataset ${selectedDatasetForDelete.name} has been marked for deletion.`,
      });
  ...
  const renderStatusBadge = (dataset: DatasetRead) => {
    const liveStatus = getLatestTaskForEntity(
      taskStatuses,
      "Dataset",
      dataset.id,
      "dataset_generation"
    );
    const currentStatusToDisplay = liveStatus || {
    ...
    return datasets.map((dataset) => (
      <TableRow key={dataset.id}>
        <TableCell className="font-medium max-w-[250px] truncate">
          <Link href={`/datasets/${dataset.id}`} className="hover:underline text-primary">
            {dataset.name}
          </Link>
        </TableCell>
        <TableCell className="text-xs">
          {dataset.repository ? (
            <Link
              href={`/repositories/${dataset.repository.id}`}
  ...
  return (
    <MainLayout>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
          ...
          <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as any)}>
            ...
            <SelectContent>
              ...
              {availableDatasetStatuses.map((s) => (
          ...
          <div className="flex items-center gap-2 md:col-start-3 lg:col-start-4">
            <Label htmlFor="items-per-page-datasets" className="text-sm shrink-0">
              Show:
            </Label>
            <Select
            ...
            >
              <SelectTrigger id="items-per-page-datasets" className="w-full md:w-[80px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <AlertDialog
          open={!!selectedDatasetForDelete}
          onOpenChange={(open) => !open && setSelectedDatasetForDelete(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Delete {selectedDatasetForDelete?.name}?</AlertDialogTitle>
              <AlertDialogDescription>
            ...
            <AlertDialogFooter>
              ...
              >
                {isDeletingDataset && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Delete
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
