...
const ParamInput: React.FC<{
  ...
  switch (paramDef.type) {
    ...
    default:
      return (
        <Input type="text" value={String(value ?? "")} onChange={handleChange} className="h-9" />
      );
  }
};
...
export const FeatureSelectionStep: React.FC<FeatureSelectionStepProps> = ({
  ...
  const selectedAlgorithm = isEnabled
    ? availableAlgorithms.find((a) => a.name === selectionConfig.name)
    : null;
  ...
  const handleToggle = (checked: boolean) => {
    if (checked) {
      ...
      if (availableAlgorithms.length > 0) {
        ...
        const defaultParams = firstAlgo.parameters.reduce(
          (acc, param) => {
  ...
  const handleAlgorithmChange = (algoName: string) => {
    ...
    if (newAlgo) {
      const defaultParams = newAlgo.parameters.reduce(
        (acc, param) => {
          if (param.default !== undefined) {
            acc[param.name] = param.default;
          }
      ...
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>
              Optionally apply a feature selection algorithm after cleaning the data.
            </CardDescription>
          </div>
          <div className="flex items-center space-x-2">
            <Label htmlFor="enable-fs" className="text-sm">
              Enable
            </Label>
      ...
      <CardContent className="space-y-6">
        {!isEnabled ? (
          <Alert variant="default">
            <AlertDescription>
              Feature selection is disabled. All configured features will be included in the final
              dataset.
            </AlertDescription>
          </Alert>
          ...
          <Alert variant="default">
            <AlertDescription>
              No feature selection algorithms are available from the backend worker.
            </AlertDescription>
          </Alert>
        ) : (
                  ...
                  {selectedAlgorithm.parameters.map((paramDef) => (
                    <div key={paramDef.name} className="flex items-center justify-between">
                      <div className="flex items-center space-x-1.5">
                        <Label htmlFor={`param-${paramDef.name}`} className="text-sm">
                          {paramDef.name}
                        </Label>
