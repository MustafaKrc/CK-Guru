...
import { ArrowLeft, ArrowRight, HelpCircle, Info, Loader2, AlertCircle, Link, Check, Database, GitBranch, ListFilter, TargetIcon, CheckCircle2, WandSparkles, Settings2 } from "lucide-react";
...
import { DatasetCreatePayload, DatasetTaskResponse, CleaningRuleConfig as BackendCleaningRuleConfig, FeatureSelectionConfig } from "@/types/api/dataset";
...
function CreateDatasetPageContent() {
  ...
  // Step 4 State
  const [featureSelectionConfig, setFeatureSelectionConfig] = useState<FeatureSelectionConfig | null>(null);
  ...
  const handleSubmit = async () => {
    if (!validateStep1() || !validateStep2() || !validateStep3() || !validateStep4()) {
      toast({ title: "Validation Error", description: formError || "Please check previous steps for errors.", variant: "destructive" });
      return;
    }
  ...
  const KeyValueItem: React.FC<{ label: string; value?: string | number | null; badge?: boolean }> = ({ label, value, badge }) => (
      ...
      <PageContainer>
            ...
            <Card>
                ...
                <CardContent className="space-y-6">
                    {isLoadingRules ? ( <Skeleton className="h-40 w-full" /> ) :
                     availableRuleDefinitions.length === 0 ? ( <Alert variant="default"><Info className="h-4 w-4" /><AlertDescription>No cleaning rules available from the backend.</AlertDescription></Alert> ) :
                     (
                        <ScrollArea className="h-[50vh]">
                            ...
                            {configuredCleaningRules.map((rule) => (
                                ...
                                <div className="flex items-start space-x-3">
                                    <Switch id={`rule-switch-${rule.name}`} checked={rule.enabled} onCheckedChange={(checked) => handleRuleEnabledChange(rule.name, Boolean(checked))} className="mt-1" />
                                    <div className="flex-grow grid gap-1.5 leading-none">
                                    <Label htmlFor={`rule-switch-${rule.name}`} className="text-base font-semibold cursor-pointer">{rule.name.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</Label>
                                ...
                                {rule.enabled && rule.parameters && rule.parameters.length > 0 && (
                                    ...
                                    {rule.parameters.map((paramDef) => (
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <Label htmlFor={`${rule.name}-${paramDef.name}`} className="text-sm">{paramDef.name.replace(/_/g, ' ')}</Label>
                                            <TooltipProvider delayDuration={100}><Tooltip><TooltipTrigger asChild><HelpCircle className="h-3.5 w-3.5 text-muted-foreground cursor-help" /></TooltipTrigger><TooltipContent><p>{paramDef.description}</p></TooltipContent></Tooltip></TooltipProvider>
                                        </div>
                                        <Input type={paramDef.type === "integer" ? "number" : "text"} id={`${rule.name}-${paramDef.name}`} value={String(rule.userParams[paramDef.name] ?? paramDef.default ?? '')}
                                                onChange={(e) => handleRuleParamChange(rule.name, paramDef.name, e.target.value)} className="h-9" placeholder={`Default: ${paramDef.default}`}/>
                                        </div>
                                    ))}
                                    </div>
                                )}
        ...
        {currentStep === 5 && (
            <div className="space-y-6">
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center"><CheckCircle2 className="mr-2 h-5 w-5 text-primary"/>Review & Submit</CardTitle>
                        <CardDescription>Review all configurations below before submitting the job to generate your dataset.</CardDescription>
                    </CardHeader>
                </Card>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <SectionCard title="General Information" icon={<GitBranch />}>
                        <KeyValueItem label="Repository" value={availableRepositories.find(r => r.id.toString() === repositoryId)?.name || `ID: ${repositoryId}`} />
                        ...
                        {datasetDescription && (
                            <div>
                                ...
                                <p className="text-xs italic bg-muted/30 p-2 rounded-md whitespace-pre-wrap">{datasetDescription}</p>
                            </div>
                        )}
                    </SectionCard>
                    <SectionCard title="Data Structure" icon={<Database />}>
                        ...
                        <div>
                            <Label className="text-xs text-muted-foreground uppercase block mb-1">Features ({selectedFeatureColumns.length})</Label>
                            <ScrollArea className="h-28 rounded-md border bg-muted/30 p-2">
                                <ul className="list-disc list-inside pl-2 text-xs space-y-0.5">
                                    {selectedFeatureColumns.map(f => <li key={f} className="truncate" title={f}>{f}</li>)}
                                </ul>
                            </ScrollArea>
                        </div>
                    </SectionCard>
                    <SectionCard title="Processing Pipeline" icon={<ListFilter />}>
                        <div>
                            <Label className="text-xs text-muted-foreground uppercase block mb-1">Data Cleaning ({configuredCleaningRules.filter(r => r.enabled).length} enabled)</Label>
                            <ScrollArea className="h-32 rounded-md border bg-muted/30 p-2">
                                {configuredCleaningRules.filter(r => r.enabled).length > 0 ? (
                                    <ul className="space-y-1.5 text-xs">
                                        {configuredCleaningRules.filter(r => r.enabled).map(rule => (
                                            <li key={rule.name} className="border-b border-dashed pb-1.5 last:pb-0 last:border-0">
                                                <p className="font-medium">{rule.name}</p>
                                                {Object.keys(rule.userParams).length > 0 && (
                                                    <dl className="mt-1 pl-3 text-xs">
                                                        {Object.entries(rule.userParams).map(([key, val]) => <KeyValueItem key={key} label={key} value={String(val)} />)}
                                                    </dl>
                                                )}
                                            </li>
                                        ))}
                                    </ul>
                                ) : <p className="text-xs italic text-muted-foreground p-2">No cleaning rules enabled.</p>}
                            </ScrollArea>
                        </div>
                        <Separator className="my-3"/>
                        <div>
                            <Label className="text-xs text-muted-foreground uppercase block mb-1">Feature Selection</Label>
                            {featureSelectionConfig ? (
                                <div className="text-xs p-2 rounded-md bg-muted/30 border">
                                    <p className="font-medium">{availableAlgorithms.find(a => a.name === featureSelectionConfig.name)?.display_name || featureSelectionConfig.name}</p>
                                    {Object.keys(featureSelectionConfig.params).length > 0 && (
                                        <dl className="mt-1 pt-1 border-t border-dashed">
                                            {Object.entries(featureSelectionConfig.params).map(([key, val]) => <KeyValueItem key={key} label={key} value={String(val)} />)}
                                        </dl>
                                    )}
                                </div>
                            ) : (
                                <p className="text-xs italic text-muted-foreground p-2">Feature selection is disabled.</p>
                            )}
                        </div>
                    </SectionCard>
                </div>
                <div className="flex justify-between mt-8">
                    ...
                    <Button onClick={handleSubmit} disabled={isSubmitting}>
                        {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Submitting...</> : "Submit & Generate Dataset"}
                    </Button>
                </div>
            </div>
        )}
