...
function JobsPageContent() {
  ...
  const [trainingPag, setTrainingPag] = useState<JobPaginationState>({
    currentPage: 1,
    totalItems: 0,
    isLoading: true,
    itemsPerPage: 10,
  });
  const [hpPag, setHpPag] = useState<JobPaginationState>({
    currentPage: 1,
    totalItems: 0,
    isLoading: true,
    itemsPerPage: 10,
  });
  const [infPag, setInfPag] = useState<JobPaginationState>({
    currentPage: 1,
    totalItems: 0,
    isLoading: true,
    itemsPerPage: 10,
  ...
  const fetchJobs = useCallback(
    async (jobTypeToFetch: JobTypeKey) => {
      let fetcher: any;
    ...
    [
      trainingPag,
      hpPag,
      infPag, // Depend on all pagination states
      debouncedSearch,
      statusFilter,
      repositoryFilter,
      sortConfig,
  ...
  useEffect(() => {
    ...
    const performInitialLoad = async () => {
      await Promise.all([fetchJobs("training"), fetchJobs("hpSearch"), fetchJobs("inference")]);
  ...
  const handleItemsPerPageChange = (value: string) => {
    ...
    if (activeTab === "training")
      setTrainingPag((p) => ({ ...p, itemsPerPage: newSize, currentPage: 1 }));
    if (activeTab === "hpSearch")
      setHpPag((p) => ({ ...p, itemsPerPage: newSize, currentPage: 1 }));
    if (activeTab === "inference")
      setInfPag((p) => ({ ...p, itemsPerPage: newSize, currentPage: 1 }));
  };
  ...
  const getStatusBadge = (jobId: number, key: JobTypeKey, staticStatus?: any) => {
    ...
    const taskJobType =
      key === "training" ? "model_training" : key === "hpSearch" ? "hp_search" : undefined;
    const live = getLatestTaskForEntity(taskStatuses, entityType, jobId, taskJobType);

    const current =
      live || (staticStatus ? ({ status: staticStatus } as TaskStatusUpdatePayload) : undefined);
    if (!current?.status)
      return (
          ...
          <div className="rounded-md border">
            <Table>
              ...
              <TableBody>
                {data.map(
                  (
                    job: T // Consider using T here instead of any if possible
                  ) => (
                    <TableRow key={job.id}>
                      {columns.map((c, i) => (
                        <TableCell key={i} className={`text-xs ${c.className || ""}`}>
                          {c.accessor(job)}
                        </TableCell>
                      ))}
                    </TableRow>
        ...
        <PaginationContent>
          <PaginationItem>
            <PaginationPrevious
              ...
              className={
                paginationState.currentPage <= 1 || paginationState.isLoading
          ...
          <PaginationItem>
            <PaginationNext
              onClick={() => onPageChange(paginationState.currentPage + 1)}
              aria-disabled={paginationState.currentPage >= totalPages || paginationState.isLoading}
              className={
  ...
  const trainingCols = [
    {
      ...
      accessor: (j: TrainingJobRead) =>
        j.id ? (
          <Link href={`/jobs/${j.id}?type=training`} className="hover:underline text-primary">
            {" "}
            {j.config.model_name}
          </Link>
        ) : (
    ...
    {
      header: (
        <SortableHeader sortKey="repository_name" sortConfig={sortConfig} onSort={handleSort}>
          Repository
        </SortableHeader>
      ),
      accessor: (j: TrainingJobRead) =>
        j.dataset?.repository ? (
          <Link
            href={`/repositories/${j.dataset.repository.id}`}
            className="hover:underline text-primary"
          >
            {j.dataset.repository.name}
          </Link>
        ) : (
          `Repo ID: ${j.dataset?.repository_id || "N/A"}`
        ),
    },
    {
      header: (
        <SortableHeader sortKey="dataset_name" sortConfig={sortConfig} onSort={handleSort}>
          Dataset
        </SortableHeader>
      ),
      accessor: (j: TrainingJobRead) =>
        j.dataset ? (
          <Link href={`/datasets/${j.dataset.id}`} className="hover:underline text-primary">
            {j.dataset.name}
          </Link>
        ) : (
          `Dataset ID: ${j.dataset_id}`
        ),
    },
    {
      header: (
    ...
    },
    {
      ...
      accessor: (j: TrainingJobRead) => formatDate(j.created_at),
    },
  ];
  ...
  const hpCols = [
    {
      ...
      accessor: (j: HPSearchJobRead) =>
        j.id ? (
          <Link href={`/jobs/${j.id}?type=hp_search`} className="hover:underline text-primary">
            {" "}
            {j.optuna_study_name}
          </Link>
        ) : (
    ...
    },
    {
      ...
      accessor: (j: HPSearchJobRead) => getStatusBadge(j.id, "hpSearch", j.status),
    },
    {
      header: (
        <SortableHeader sortKey="created_at" sortConfig={sortConfig} onSort={handleSort}>
      ...
      accessor: (j: HPSearchJobRead) => formatDate(j.created_at),
    },
  ];
  ...
  const infCols = [
    {
      ...
      accessor: (j: InferenceJobRead) =>
        j.input_reference?.commit_hash ? (
          <Link
            href={`/repositories/${j.ml_model?.dataset?.repository?.id}/commits/${j.input_reference.commit_hash}`}
            className="hover:underline text-primary font-mono text-xs"
            title={j.input_reference.commit_hash}
          >
            {j.input_reference.commit_hash.substring(0, 12)}...
          </Link>
        ) : (
    ...
    },
    {
      ...
      accessor: (j: InferenceJobRead) =>
        j.ml_model?.dataset?.repository ? (
          <Link
            href={`/repositories/${j.ml_model.dataset.repository.id}`}
            className="hover:underline text-primary"
          >
            {j.ml_model.dataset.repository.name}
          </Link>
        ) : (
          `Repo ID: ${j.ml_model?.dataset?.repository_id || "N/A"}`
        ),
    },
    {
      header: (
      ...
      accessor: (j: InferenceJobRead) =>
        j.ml_model?.dataset ? (
          <Link
            href={`/datasets/${j.ml_model.dataset.id}`}
            className="hover:underline text-primary"
          >
            {j.ml_model.dataset.name}
          </Link>
        ) : (
          `Dataset ID: ${j.ml_model?.dataset?.id || "N/A"}`
        ),
    },
    {
      header: (
    ...
    },
    {
      ...
      accessor: (j: InferenceJobRead) => j.ml_model?.name || "N/A",
      className: "font-medium",
    },
    {
      header: (
    ...
    },
    {
      ...
      accessor: (j: InferenceJobRead) => formatDate(j.created_at),
    },
  ];
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            <Label htmlFor="items-per-page-jobs" className="text-sm shrink-0">
              Show:
            </Label>
            <Select
              value={String(currentPaginationState.itemsPerPage)}
              onValueChange={handleItemsPerPageChange}
            >
              <SelectTrigger id="items-per-page-jobs" className="w-full md:w-[80px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {[10, 25, 50].map((size) => (
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as JobTypeKey)}>
          ...
          <TabsContent value="training" className="mt-4">
            {renderJobTable("Training Jobs", trainingJobs, trainingCols, trainingPag, trainingErr)}
          </TabsContent>
          <TabsContent value="hpSearch" className="mt-4">
            {renderJobTable("Hyperparameter Search Jobs", hpSearchJobs, hpCols, hpPag, hpErr)}
          </TabsContent>
          <TabsContent value="inference" className="mt-4">
            {renderJobTable("Inference Jobs", inferenceJobs, infCols, infPag, infErr)}
          </TabsContent>
        </Tabs>
      </PageContainer>
    </MainLayout>
