  ...
  devtools(
    ...
    (set, get) => ({
      ...
      connectSSE: () => {
        ...
        if (
          sseEventSourceInstance &&
          (sseEventSourceInstance.readyState === EventSource.OPEN ||
            sseEventSourceInstance.readyState === EventSource.CONNECTING)
        ) {
      ...
      getTasksByEntity: (entityType: string, entityId: number | string) => {
        return Object.values(get().taskStatuses)
          .filter(
      ...
      getLatestTaskForEntity: (entityType: string, entityId: number | string, jobType?: string) => {
        const tasks = Object.values(get().taskStatuses).filter(
          (task) =>
