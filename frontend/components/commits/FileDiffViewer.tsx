...
export const FileDiffViewer: React.FC<FileDiffViewerProps> = ({ diffText }) => {
  ...
  const getLineClass = (line: string) => {
    ...
    if (line.startsWith("@@"))
      return "bg-blue-600/10 text-blue-800 dark:text-blue-300 font-semibold";
    ...
    <ScrollArea className="max-h-[500px] h-96 font-mono text-xs border rounded-md bg-muted/30">
      <div className="p-2">
        {lines.map((line, index) => (
          <div
            key={index}
            className={cn("whitespace-pre-wrap break-words w-full", getLineClass(line))}
          >
            <span
              className={cn("inline-block w-8 select-none text-right pr-2 opacity-50", {
                "border-r": true,
              })}
            >
              {index + 1}
