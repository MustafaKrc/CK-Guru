  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-popover border border-border p-2 shadow-lg rounded-md text-sm text-popover-foreground">
        ...
        {/* label is the feature_condition */}
...
export const LimeDisplay: React.FC<LimeDisplayProps> = ({ data }) => {
  ...
  const selectedInstanceData: InstanceLIMEResult | null = useMemo(() => {
    // Type InstanceLIMEResult
  ...
  const chartData = useMemo(
    () =>
      selectedInstanceData?.explanation
        .map((item) => ({
          ...
          labelFillColor:
            item[1] > 0 ? "hsl(var(--primary-foreground))" : "hsl(var(--destructive-foreground))",
        }))
    ...
    <Card className="bg-card text-card-foreground border-border">
      <CardHeader>
        ...
        <CardDescription className="text-muted-foreground">
          Local Interpretable Model-agnostic Explanations. Shows feature contributions for a
          specific prediction. Positive weights (blue) support the predicted class, negative weights
          (red) oppose it.
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {chartData && chartData.length > 0 ? (
          ...
          <p className="text-sm text-muted-foreground">
            No LIME explanations to display for the selected instance.
          </p>
        ) : (
          <p className="text-sm text-muted-foreground">
            Select an instance to view LIME explanations.
          </p>
        )}
      </CardContent>
    </Card>
