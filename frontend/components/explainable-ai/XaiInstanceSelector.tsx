...
export const XaiInstanceSelector: React.FC<XaiInstanceSelectorProps> = ({
  ...
  const uniqueOptions = useMemo(() => {
    ...
    instances.forEach((inst, index) => {
      ...
      if (identifierKey === "class_name" && inst.class_name) {
        ...
        // Fallback logic if identifierKey is not perfectly matched
        id = inst.class_name;
        displayLabel = inst.class_name;
      } else if (inst.file) {
        id = inst.file;
      ...
      if (!optionMap.has(id)) {
        // Ensure unique IDs for SelectItem values
  ...
  if (uniqueOptions.length === 0) {
    return (
      <p className="text-sm text-muted-foreground mb-4">No instances available for selection.</p>
    );
  }
  ...
  return (
    <div className="mb-6">
      ...
      <Select value={selectedIdentifier || ""} onValueChange={onInstanceChange}>
