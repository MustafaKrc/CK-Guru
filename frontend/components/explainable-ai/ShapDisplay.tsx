  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-popover border border-border p-2 shadow-lg rounded-md text-sm text-popover-foreground">
        <p className="font-bold text-popover-foreground max-w-xs break-words">{label}</p>{" "}
        {/* label is the feature name */}
        {featureOriginalValue !== undefined && (
          <p className="text-muted-foreground text-xs">Feature Value: {featureOriginalValue}</p>
        )}
        ...
        {baseValue !== undefined && (
          <p className="text-xs text-muted-foreground">
            Prediction = Base ({baseValue.toFixed(4)}) + Sum(SHAP)
          </p>
        )}
      </div>
    );
...
export const ShapDisplay: React.FC<ShapDisplayProps> = ({ data }) => {
    ...
    return (
      <Alert variant="default" className="text-foreground bg-card border-border">
        ...
        <AlertDescription className="text-muted-foreground">
          No SHAP data available for this prediction.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  const chartData = useMemo(
    () =>
      selectedInstanceData?.shap_values
        .map((item) => ({
  ...
  ); // Dependency on selectedInstanceData
    ...
    <Card className="bg-card text-card-foreground border-border">
      <CardHeader>
        ...
        <CardDescription className="text-muted-foreground">
          Explains how each feature contributed to pushing the model's prediction away from the
          baseline. Positive values (red) increase the prediction towards being defect-prone;
          ...
          <div className="grid grid-cols-2 gap-4 text-sm mb-4 p-3 border rounded-md bg-muted/30 dark:bg-muted/20">
            <div>
              ...
              <p className="font-semibold text-lg">
                {baseValue !== undefined ? baseValue.toFixed(4) : "N/A"}
              </p>
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">
                Final Prediction for this Instance
              </Label>
              <p className="font-semibold text-lg">
                {predictedValue !== undefined ? predictedValue.toFixed(4) : "N/A"}
              </p>
            </div>
          </div>
        )}
        {chartData && chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height={Math.max(300, chartData.length * 35)}>
            ...
            >
              ...
              />
              <Tooltip
                content={<CustomShapTooltip baseValue={baseValue} />}
                ...
                <LabelList
                  ...
                  content={(props: any) => {
                    ...
                    const fill = isPositive
                      ? "hsl(var(--destructive-foreground))"
                      : "hsl(var(--primary-foreground))"; // Match bar color
          ...
          <p className="text-sm text-muted-foreground">
            No SHAP values to display for the selected instance.
          </p>
        ) : (
