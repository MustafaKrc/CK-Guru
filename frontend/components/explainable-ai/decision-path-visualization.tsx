...
export function DecisionPathVisualization({ data }: DecisionPathProps) {
  ...
  useEffect(() => {
    ...
    const maxX = Math.max(...Object.values(nodePositions).map((pos) => pos.x + nodeWidth)) + leftPadding
    const maxY = Math.max(...Object.values(nodePositions).map((pos) => pos.y + nodeHeight)) + topPadding
