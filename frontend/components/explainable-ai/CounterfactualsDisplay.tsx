...
export const CounterfactualsDisplay: React.FC<CounterfactualsDisplayProps> = ({
  data,
  originalInstanceData,
}) => {
  ...
  const selectedInstanceData: InstanceCounterfactualResult | undefined = useMemo(() => {
    ...
    return instances.find(
      (inst) =>
        (inst.class_name || inst.file || `instance_${instances.indexOf(inst)}`) ===
        selectedInstanceId
    );
  }, [instances, selectedInstanceId]);
    ...
    return (
      <Alert variant="default" className="text-foreground bg-card border-border">
        ...
        <AlertDescription className="text-muted-foreground">
          No counterfactual data available for this prediction.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  const renderFeatureChange = (feature: string, cfValue: any, originalValue?: any) => {
    ...
    return (
      <TableRow
        key={feature}
        className={cn(
          valueChanged ? "bg-accent/50 dark:bg-accent/20" : "",
          "hover:bg-accent/30 dark:hover:bg-accent/15"
        )}
      >
        <TableCell className="font-medium text-xs py-2 text-foreground">{feature}</TableCell>
        {originalInstanceData && (
          <TableCell className="text-right font-mono text-xs py-2 text-muted-foreground">
            {String(originalValue ?? "N/A")}
          </TableCell>
        )}
        <TableCell
          className={`text-right font-mono text-xs py-2 ${valueChanged ? "text-primary font-semibold" : "text-foreground"}`}
        >
          {String(cfValue)}
        </TableCell>
      </TableRow>
    ...
    <Card className="bg-card text-card-foreground border-border">
      <CardHeader>
        <CardTitle className="flex items-center text-lg">
          <ShuffleIcon className="mr-2 h-5 w-5 text-primary" />
        ...
        <CardDescription className="text-muted-foreground">
          ...
          {originalInstanceData && (
            <span className="block mt-1 text-xs">
              ...
              <Badge
                variant={
                  originalInstanceData.predictionProbability > 0.5 ? "destructive" : "default"
                }
      ...
      <CardContent>
        ...
        {selectedInstanceData && selectedInstanceData.counterfactuals.length > 0 ? (
          <ScrollArea className="max-h-[calc(100vh-400px)] pr-3">
            {" "}
            {/* Adjust max-h as needed */}
            <div className="space-y-6">
              {selectedInstanceData.counterfactuals.map((cf, index) => {
                const probDiff = originalInstanceData
                  ? cf.outcome_probability - originalInstanceData.predictionProbability
                  : 0;
                ...
                return (
                  <Card
                    key={index}
                    className="bg-muted/30 dark:bg-muted/20 border-border shadow-sm overflow-hidden"
                  >
                    <CardHeader className="pb-3 pt-4">
                      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                        <CardTitle className="text-md font-semibold">
                          Counterfactual Suggestion #{index + 1}
                        </CardTitle>
                    ...
                    <CardContent>
                      {originalInstanceData && (
                        <div className="mb-3 text-xs flex items-center">
                          ...
                          <span
                            className={`ml-1.5 font-semibold ${isImprovement ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                          >
                            {probDiff !== 0
                              ? `${probDiff > 0 ? "+" : ""}${(probDiff * 100).toFixed(1)}% change`
                              : "No change"}
                          </span>
                          {isImprovement ? (
                            <CheckIcon className="ml-1 h-4 w-4 text-green-600 dark:text-green-400" />
                          ) : (
                            probDiff !== 0 && (
                              <Cross1Icon className="ml-1 h-3.5 w-3.5 text-red-600 dark:text-red-400" />
                            )
                          )}
                          <Progress
                      ...
                      <p className="text-xs text-muted-foreground mb-2">
                        If the following features were changed as shown:
                      </p>
                          ...
                          <TableBody>
                            {Object.entries(cf.features).map(([feature, value]) =>
                              renderFeatureChange(
                                feature,
                                value,
                                originalInstanceData?.features[feature]
                              )
                            )}
                          </TableBody>
                        </Table>
          ...
          <p className="text-sm text-muted-foreground mt-4 text-center py-6">
            No counterfactual examples found for the selected instance.
          </p>
        ) : (
          <p className="text-sm text-muted-foreground mt-4 text-center py-6">
            Select an instance to view counterfactuals.
          </p>
        )}
      </CardContent>
    </Card>
