...
import { CheckCircle2, Database, Brain, WandSparkles, SlidersHorizontal, Settings2 } from 'lucide-react';
...
const SectionCard: React.FC<{ title: string; icon: React.ReactNode; children: React.ReactNode; className?: string }> = ({ title, icon, children, className }) => (
  <Card className={className}>
    <CardHeader className="pb-3 pt-4">
      <CardTitle className="text-base font-semibold flex items-center">{icon}{title}</CardTitle>
    </CardHeader>
    <CardContent className="text-sm">{children}</CardContent>
  </Card>
);

const KeyValueItem: React.FC<{ label: string; value?: string | number | boolean; }> = ({ label, value }) => (
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Review all configurations below before submitting the HP search job.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label htmlFor="studyName">Study Name *</Label>
              <Input id="studyName" value={formData.studyName} onChange={e => updateFormData({ studyName: e.target.value })} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="modelBaseName" className={!formData.saveBestModel ? "text-muted-foreground" : ""}>Base Name for Best Model *</Label>
              <Input id="modelBaseName" value={formData.modelBaseName} onChange={e => updateFormData({ modelBaseName: e.target.value })} disabled={!formData.saveBestModel} />
            </div>
          </div>
          <div className="flex items-center space-x-2 pt-2">
            <Switch id="saveBestModel" checked={formData.saveBestModel} onCheckedChange={checked => updateFormData({ saveBestModel: checked })} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <SectionCard title="Search Space" icon={<WandSparkles className="mr-2 h-4 w-4 text-primary"/>}>
              ...
              {formData.hpSpace.map(s => (
                <div key={s.param_name} className="text-xs">
                  ...
                  <dd className="pl-2 text-muted-foreground">
                    {s.suggest_type}: {s.choices ? s.choices.join(', ') : `${s.low} - ${s.high}`}{s.step ? ` (step ${s.step})`: ''}{s.log ? ' (log)' : ''}
                  </dd>
                </div>
              ))}
            </dl>
        ...
        <SectionCard title="Search Settings" icon={<SlidersHorizontal className="mr-2 h-4 w-4 text-primary"/>}>
          ...
          <KeyValueItem label="CV Folds" value={formData.optunaConfig.hp_search_cv_folds ?? 'Default'} />
        </SectionCard>
      </div>
    </div>
  );
