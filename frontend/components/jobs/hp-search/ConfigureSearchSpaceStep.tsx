...
const CategoricalToggleGroup: React.FC<{
  ...
  return (
    ...
    >
      {options.map(option => (
        <ToggleGroupItem key={String(option.value)} value={String(option.value)} aria-label={`Toggle ${option.label}`}>
          {option.label}
        </ToggleGroupItem>
      ))}
    </ToggleGroup>
...
const ParamConfigRow: React.FC<{
  ...
  const isPredefinedCategorical = config.suggest_type === 'categorical' && definition.options && definition.options.length > 0;
    ...
    <div className="p-4 border rounded-lg space-y-3 bg-muted/50 transition-all duration-300 ease-in-out">
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-2">
          <Checkbox id={`enable-${definition.name}`} checked={config.enabled} onCheckedChange={(checked) => onConfigChange('enabled', !!checked)} />
          <Label htmlFor={`enable-${definition.name}`} className="font-semibold text-base">{definition.name}</Label>
          <TooltipProvider><Tooltip><TooltipTrigger type="button"><HelpCircle className="h-4 w-4 text-muted-foreground"/></TooltipTrigger><TooltipContent><p>{definition.description || "No description."}</p></TooltipContent></Tooltip></TooltipProvider>
        </div>
      </div>
        ...
        <div className="min-h-0">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-2 pt-4 pl-6">
            <div className="space-y-1">
              ...
              <Badge variant="secondary" className="capitalize flex items-center h-9 w-full justify-center text-sm">
                {config.suggest_type}
              </Badge>
            </div>
            ...
            {isBooleanType ? (
              ...
              <div className="md:col-span-2 space-y-2">
                ...
                <CategoricalToggleGroup
                    ...
                    selectedValues={config.choices.split(',').map(s => s.trim()).filter(Boolean)}
                    onChange={(newValues) => onConfigChange('choices', newValues.join(', '))}
                />
              </div>
            ) : config.suggest_type === 'categorical' ? (
              <div className="md:col-span-2 space-y-1">
                <Label htmlFor={`choices-${definition.name}`} className="text-xs">Choices (comma-separated)</Label>
                <Input id={`choices-${definition.name}`} value={config.choices} onChange={e => onConfigChange('choices', e.target.value)} placeholder="e.g., auto, sqrt, log2"/>
              </div>
            ) : (
              // Fallback to numeric inputs for int/float
              <>
                <div className="space-y-1"><Label htmlFor={`low-${definition.name}`} className="text-xs">Low</Label><Input id={`low-${definition.name}`} type="number" value={config.low} onChange={e => onConfigChange('low', e.target.value)} /></div>
                <div className="space-y-1"><Label htmlFor={`high-${definition.name}`} className="text-xs">High</Label><Input id={`high-${definition.name}`} type="number" value={config.high} onChange={e => onConfigChange('high', e.target.value)} /></div>
                <div className="space-y-1"><Label htmlFor={`step-${definition.name}`} className="text-xs">Step (Optional)</Label><Input id={`step-${definition.name}`} type="number" value={config.step} onChange={e => onConfigChange('step', e.target.value)} /></div>
                <div className="flex items-center space-x-2 pt-5"><Switch id={`log-${definition.name}`} checked={config.log} onCheckedChange={checked => onConfigChange('log', checked)} /><Label htmlFor={`log-${definition.name}`} className="text-xs">Log Scale</Label></div>
              </>
            )}
          </div>
        </div>
...
export const ConfigureSearchSpaceStep: React.FC<{
  ...
  useEffect(() => {
    ...
    formData.modelHyperparametersSchema.forEach(param => {
      ...
      newConfigs[param.name] = {
        ...
        choices: existingConfig?.choices?.join(', ') ?? (param.type === 'boolean' ? 'True, False' : (param.options?.map(o=>o.value).join(', '))) ?? '',
      };
    });
    setParamConfigs(newConfigs);
  }, [formData.modelHyperparametersSchema]);

  const deriveHpSpaceFromLocalState = (configs: Record<string, ParamSearchConfig>): HPSuggestion[] => {
      const newHpSpace: HPSuggestion[] = [];
      Object.entries(configs).forEach(([paramName, config]) => {
        if (config.enabled) {
          let suggestion: Partial<HPSuggestion> = { param_name: paramName, suggest_type: config.suggest_type };
          if (config.suggest_type === 'categorical') {
            suggestion.choices = config.choices?.split(',').map(s => {
              const val = s.trim();
  ...
  const handleParamConfigChange = (paramName: string, field: keyof ParamSearchConfig, value: any) => {
  ...
  if (formData.modelHyperparametersSchema.length === 0) {
    return <Alert><AlertDescription>No configurable hyperparameters defined for this model. Default settings will be used.</AlertDescription></Alert>;
  }

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center"><WandSparkles className="mr-2 h-5 w-5 text-primary"/>Configure Search Space</CardTitle>
        <CardDescription>Enable and configure the search range for each hyperparameter of the <strong>{formData.modelDisplayName}</strong> model.</CardDescription>
      </CardHeader>
      <CardContent>
        <ScrollArea>
          <div className="space-y-4 pr-3">
            {formData.modelHyperparametersSchema.map(paramDef => (
              <ParamConfigRow
                ...
                onConfigChange={(field, value) => handleParamConfigChange(paramDef.name, field, value)}
              />
            ))}
          </div>
        </ScrollArea>
