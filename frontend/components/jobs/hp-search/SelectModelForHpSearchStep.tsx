...
export const SelectModelForHpSearchStep: React.FC<SelectModelForHpSearchStepProps> = ({
  ...
  const handleModelTypeSelect = (selectedTypeName: string) => {
    ...
    if (selectedTypeDetails) {
      ...
      toast({ title: "Model Selected", description: `Ready to configure search space for ${selectedTypeDetails.display_name}.` });
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Choose the model architecture for which to optimize hyperparameters.</CardDescription>
      </CardHeader>
      <CardContent>
        <Label htmlFor="model-type-select">Model Type *</Label>
        {isLoading ? <Skeleton className="h-10 w-full mt-2" /> : (
          <Select value={formData.modelType || ""} onValueChange={handleModelTypeSelect} disabled={availableModelTypes.length === 0}>
            <SelectTrigger id="model-type-select"><SelectValue placeholder="Select a model type..." /></SelectTrigger>
            <SelectContent>
              {availableModelTypes.map(type => <SelectItem key={type.type_name} value={type.type_name}>{type.display_name}</SelectItem>)}
            </SelectContent>
          </Select>
        )}
