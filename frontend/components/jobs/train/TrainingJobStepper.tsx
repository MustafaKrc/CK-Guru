...
export const TrainingJobStepper: React.FC<TrainingJobStepperProps> = ({
  ...
  const progressPercentage = TOTAL_STEPS > 1 
  ...
  return (
    <nav aria-label="Progress" className="mb-8">
      ...
      <div className="relative w-full">
        ...
        <div 
          className="absolute left-[16px] right-[16px] top-[15px] h-0.5 bg-gray-200 dark:bg-gray-700 z-0"
        ></div>
        ...
        <div 
          ...
          style={{ width: `calc(${progressPercentage}% * (100% - ${2 * circleHalfWidth}px) / 100)` }} 
        ...
        <ol role="list" className="flex justify-between items-center w-full relative z-10"> 
          {steps.map((step, stepIdx) => (
            <li key={step.name} className="flex flex-col items-center flex-1"> {/* No `relative` here, as the parent `div` handles it */}
              ...
              <div className="relative z-10"> 
                {stepIdx < currentStep - 1 || stepIdx <= maxCompletedStep ? ( // Completed step
                  ...
                  <button
                    ...
                    aria-current="step"
                  >
                    <span className="text-sm font-medium">{stepIdx + 1}</span> {/* Display step number */}
      ...
      <div className="text-sm text-muted-foreground text-right mt-4">Step {currentStep} of {TOTAL_STEPS}</div>
    </nav>
  );
};