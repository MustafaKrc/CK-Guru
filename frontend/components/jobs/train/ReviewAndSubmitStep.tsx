...
const KeyValueItem: React.FC<{
...
}> = ({ label, value, badge, truncateValue }) => (
  <div className="flex justify-between items-start py-1.5 border-b border-dashed last:border-b-0">
    ...
    <dd
      className={`font-medium text-right ${truncateValue ? "truncate" : "break-all"}`}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center">
            ...
            Review & Submit Training Job
          </CardTitle>
          <CardDescription>
            Review all settings. Provide a name for this job and a base name for the new model that
            will be created.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label htmlFor="trainingJobName" className="text-sm">
                Training Job Name *
              </Label>
              <Input
                id="trainingJobName"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="modelBaseName" className="text-sm">
                New Model Base Name *
              </Label>
              <Input
                id="modelBaseName"
              ...
              <p className="text-xs text-muted-foreground">
                A version number will be appended by the system (e.g., _v1).
              </p>
            </div>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="trainingJobDescription" className="text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <SectionCard title="Source Data" icon={<Database />}>
          <dl className="space-y-1">
            <KeyValueItem
              label="Repository"
              value={formData.repositoryName || `ID: ${formData.repositoryId}`}
            />
            <KeyValueItem
              label="Dataset"
              value={formData.datasetName || `ID: ${formData.datasetId}`}
            />
          </dl>
        </SectionCard>
        ...
        <SectionCard title="Model Configuration" icon={<Brain />}>
          <dl className="space-y-1">
            ...
            <KeyValueItem
              label="New Model Name (Base)"
              value={formData.modelBaseName || "Not set"}
            />
            <KeyValueItem
              label="Model Type"
              value={formData.modelDisplayName || formData.modelType}
              badge
            />
          </dl>
        </SectionCard>
        ...
        <SectionCard title="Hyperparameters" icon={<Settings2 />} className="md:col-span-1">
          {Object.keys(formData.configuredHyperparameters).length > 0 ? (
            ...
            <p className="text-sm italic text-muted-foreground">
              Using model defaults (no overrides configured).
            </p>
          )}
        </SectionCard>
      </div>
