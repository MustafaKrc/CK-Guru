...
import { Repository, PaginatedRepositoryRead, DatasetRead, PaginatedDatasetRead, DatasetTaskResponse, DatasetCreatePayload } from '@/types/api';
...
export const SelectRepositoryAndDatasetStep: React.FC<SelectRepositoryAndDatasetStepProps> = ({
  ...
  const handleCreateNewDatasetSubmit = async () => {
    if (!formData.repositoryId || !newDatasetName.trim()) {
      toast({ title: "Error", description: "Repository and new dataset name are required.", variant: "destructive" });
      return;
    }
    ...
    const payload: DatasetCreatePayload = {
      ...
      config: {
        ...
        cleaning_rules: [ // Example default cleaning rules
    ...
    try {
      ...
      toast({ title: "Dataset Creation Started", description: `Task ID: ${taskResponse.task_id}. Polling for readiness...` });
      ...
      const pollInterval = setInterval(async () => {
        attempts++;
        try {
          const createdDataset = await apiService.get<DatasetRead>(`/datasets/${taskResponse.dataset_id}`);
          if (createdDataset.status.toUpperCase() === DatasetStatusEnum.READY.toUpperCase()) {
            ...
            toast({ title: "Dataset Created & Selected", description: `${createdDataset.name} is ready.` });
          } else if (createdDataset.status.toUpperCase() === DatasetStatusEnum.FAILED.toUpperCase()) {
            clearInterval(pollInterval);
            setIsCreatingDataset(false);
            toast({ title: "Dataset Creation Failed", description: createdDataset.status_message || "Unknown error.", variant: "destructive" });
          } else if (attempts >= maxAttempts) {
            clearInterval(pollInterval);
            setIsCreatingDataset(false);
            toast({ title: "Dataset Timeout", description: "Dataset creation is taking longer than expected. Please check its status on the Datasets page.", variant: "default" });
          }
        } catch (pollError) {
          console.error("Polling error:", pollError);
           if (attempts >= maxAttempts) {
            clearInterval(pollInterval);
            setIsCreatingDataset(false);
            toast({ title: "Dataset Polling Error", description: "Could not confirm dataset status.", variant: "destructive" });
          }
        }
      }, 6000);
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center"><DatabaseZap className="mr-2 h-5 w-5 text-primary"/>Source Data Selection</CardTitle>
          <CardDescription>Choose the repository and the dataset you want to use for training.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
            ...
            {isLoadingRepositories ? <Skeleton className="h-10 w-full" /> : (
              ...
              >
                <SelectTrigger id="repository-select">
                  <SelectValue placeholder={availableRepositories.length === 0 ? "No repositories available" : "Select a repository..."} />
                </SelectTrigger>
                <SelectContent>
             ...
             {availableRepositories.length === 0 && !isLoadingRepositories && (
                <Alert variant="default" className="mt-2">
                    <AlertDescription>
                        No repositories found. Please <Link href="/repositories" className="font-medium text-primary hover:underline">add a repository</Link> first.
                    </AlertDescription>
                </Alert>
            )}
          </div>
          ...
          {formData.repositoryId && (
            <div className="space-y-4 pt-4 border-t">
                  ...
                  <Dialog open={showCreateDatasetModal} onOpenChange={setShowCreateDatasetModal}>
                    ...
                    <DialogContent className="sm:max-w-[500px]">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                For repository: <strong>{formData.repositoryName || `ID ${formData.repositoryId}`}</strong>.
                                <br/>
                                This quick add uses default feature and cleaning configurations. For full control, use the dedicated "Create Dataset" page.
                            </DialogDescription>
                        </DialogHeader>
                        <div className="grid gap-4 py-4">
                            <div className="space-y-1.5">
                                <Label htmlFor="new-dataset-name">New Dataset Name *</Label>
                                <Input id="new-dataset-name" value={newDatasetName} onChange={(e) => setNewDatasetName(e.target.value)} placeholder="e.g., Main Bug Prediction Dataset"/>
                            </div>
                             <div className="space-y-1.5">
                                <Label htmlFor="new-dataset-description">Description (Optional)</Label>
                                <Textarea id="new-dataset-description" value={newDatasetDescription} onChange={(e) => setNewDatasetDescription(e.target.value)} placeholder="A brief description..." rows={2}/>
                            </div>
                        </div>
                        <DialogFooter>
                             <DialogClose asChild>
                                <Button variant="outline" disabled={isCreatingDataset}>Cancel</Button>
                            </DialogClose>
                            <Button onClick={handleCreateNewDatasetSubmit} disabled={isCreatingDataset || !newDatasetName.trim()}>
                                {isCreatingDataset ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Creating & Polling...</> : "Create & Select"}
                            </Button>
                        </DialogFooter>
                    </DialogContent>
                  </Dialog>
              ...
              {selectionMode === 'select' && (
                <div className="space-y-2">
                  {isLoadingExistingDatasets ? <Skeleton className="h-10 w-full" /> : (
                    <Select
                    ...
                    >
                      <SelectTrigger disabled={isCreatingDataset || isLoadingExistingDatasets || existingDatasets.length === 0}>
                        <SelectValue placeholder={
                            isCreatingDataset ? "Waiting for new dataset..." : 
                            isLoadingExistingDatasets ? "Loading datasets..." :
                            existingDatasets.length === 0 ? "No 'Ready' datasets found" : 
                  ...
                  {existingDatasets.length === 0 && !isLoadingExistingDatasets && !isCreatingDataset && (
                    <Alert variant="default" className="mt-2 text-xs">
                        <AlertDescription>
                            No 'Ready' datasets found for this repository. You can use the button above to create one.
                        </AlertDescription>
                    </Alert>
                  )}
                </div>
          ...
          {formData.datasetId && formData.datasetName && (
            <Alert variant="default" className="mt-4 bg-primary/5 border-primary/20">
              ...
              <div className="ml-2">
                <p className="font-semibold text-primary">Dataset Selected: {formData.datasetName}</p>
                <p className="text-xs text-muted-foreground">
                  Features: {formData.datasetFeatureSpace.length > 0 ? formData.datasetFeatureSpace.slice(0,3).join(', ') + (formData.datasetFeatureSpace.length > 3 ? '...' : '') : 'N/A'}
                  {formData.datasetFeatureSpace.length > 0 && ` (${formData.datasetFeatureSpace.length} total)`}
                </p>
                <p className="text-xs text-muted-foreground">Target: {formData.datasetTargetColumn || 'N/A'}</p>
              </div>
            </Alert>
          )}
        </CardContent>
