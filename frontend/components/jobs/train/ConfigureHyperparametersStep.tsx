    ...
    case "boolean":
      return (
        <div className="flex items-center pt-1.5">
          <Switch id={name} checked={Boolean(valueToUse)} onCheckedChange={handleSwitchChange} />
        </div>
      );
    case "enum":
    case "text_choice":
      if (!options || options.length === 0) {
        return (
          <p className="text-xs text-red-500">
            Error: No options defined for enum/text_choice '{name}'.
          </p>
        );
      }
      return (
        <Select value={String(valueToUse ?? "")} onValueChange={handleSelectChange}>
...
export const ConfigureHyperparametersStep: React.FC<ConfigureHyperparametersStepProps> = ({
  ...
  if (formData.modelHyperparametersSchema.length === 0) {
    return (
      <Alert variant="default">
        <AlertDescription>
          No configurable hyperparameters defined for the selected model ({formData.modelType}). The
          model will be trained with its default settings.
        </AlertDescription>
      </Alert>
    );
  }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Adjust hyperparameters for the selected model: <strong>{formData.modelType}</strong>{" "}
            (Type: {formData.modelType}). Values set here will override model defaults.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ScrollArea className="max-h-[calc(100vh-450px)] pr-3">
            {" "}
            {/* Adjust max-h as needed */}
            <div className="space-y-6">
              {formData.modelHyperparametersSchema.map((hpDef, index) => (
                <div
                  key={hpDef.name || `hp-${index}`}
                  className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-2 items-start"
                >
                  <div className="md:col-span-1 flex items-center space-x-1 pt-1.5">
                    <Label
                      htmlFor={hpDef.name || `hp-${index}`}
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-xs text-xs" side="top" align="start">
                            ...
                            {hpDef.default_value !== undefined && (
                              <p className="mt-1">
                                Default:{" "}
                                <code className="bg-muted/50 px-1 py-0.5 rounded-sm">
                                  {String(hpDef.default_value)}
                                </code>
                              </p>
                            )}
                            {hpDef.example_value !== undefined &&
                              hpDef.default_value === undefined && (
                                <p className="mt-1">
                                  Example:{" "}
                                  <code className="bg-muted/50 px-1 py-0.5 rounded-sm">
                                    {String(hpDef.example_value)}
                                  </code>
                                </p>
                              )}
                          </TooltipContent>
                  ...
                  <div className="md:col-span-2">
                    <HyperparameterInputFactory
                      ...
                      onChange={(value) =>
                        handleHyperparameterChange(hpDef.name || `hp-${index}`, value)
                      }
                    />
                  </div>
                </div>
