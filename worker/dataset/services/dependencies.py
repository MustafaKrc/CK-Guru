...
from .interfaces import (
    IDatasetGeneratorStep, IDataLoader, IOutputWriter, ICleaningService, IRepositoryFactory
)
...
class StepRegistry:
    ...
    def __init__(self):
        ...
        logger.debug(f"StepRegistry initialized with {len(self._step_classes)} known step types.")

    def get_step(self, step_class: Type[IDatasetGeneratorStep]) -> IDatasetGeneratorStep:
...
class DependencyProvider:
    ...
    def __init__(self, session_factory: Callable[[], Session]):
        ...
        if not self.rule_registry:
             logger.warning("DependencyProvider initialized, but WORKER_RULE_REGISTRY is empty. Cleaning service might fail.")
    ...
    def _get_data_loader(self, context: DatasetContext) -> IDataLoader:
        ...
        if not context.repository_db: raise ValueError("Repository DB object missing for DataLoader.")
    ...
    def _get_output_writer(self) -> IOutputWriter:
        ...
        if IOutputWriter not in self._cached_services:
            self._cached_services[IOutputWriter] = OutputWriter(settings.s3_storage_options)
    ...
    def _get_cleaning_service(self, context: DatasetContext) -> ICleaningService:
        ...
        if not context.dataset_config: raise ValueError("DatasetConfig missing for CleaningService.")
        if not self.rule_registry: raise RuntimeError("Rule Registry is empty, cannot create Cleaning Service.")
    ...
    def get_dependencies_for_step(
        ...
        step_logger = StepLogger(logger, f"Task {context.task_instance.request.id} - DependencyProvider")
        step_logger.debug(f"Providing dependencies for step {step.__class__.__name__}: {list(deps.keys())}")
        return deps