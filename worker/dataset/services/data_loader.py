...
from sqlalchemy.orm import Session # Keep Session type hint for factory

# Import interfaces and specific repositories
from .interfaces import IDataLoader
from shared.repositories import CommitGuruMetricRepository, CKMetricRepository, BotPatternRepository # Import Repositories if needed, but query uses models
from shared.db.models import CommitGuruMetric, CKMetric, BotPattern # Import models directly for query construction
...
CK_METRIC_COLUMNS = [ # Example, use the actual list
    'file', 'class_name', 'type', 'cbo', 'cboModified', 'fanin', 'fanout',
...
class DataLoader(IDataLoader): # Implement interface
    ...
    def __init__(self, session_factory: callable, repository_id: int, bot_patterns: List[BotPattern]):
        ...
        if self.bot_patterns:
            ...
            if bot_condition is not None:
                ...
                logger.debug("Applied bot filters to base query.")
                 ...
                 logger.warning("Bot patterns provided, but failed to build filter condition.")
        else:
    ...
    def estimate_total_rows(self) -> int:
        ...
        with BaseRepository._session_scope(self) as session: # Access context manager via Base
            try:
            ...
            try:
                ...
                for result_batch in stream.partitions(batch_size):
                    ...
                    for row in result_batch:
                        ...
                        ckm_data['class'] = getattr(row.ckm, 'class_name', None) # Use model attribute name
                        ckm_data['type'] = getattr(row.ckm, 'type_', None)      # Use model attribute name
                        ...
                        combined_data['repository_id'] = self.repository_id # Add repo_id
