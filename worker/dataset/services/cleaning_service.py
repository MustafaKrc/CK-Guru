...
import pandas as pd
from pydantic import BaseModel, Field, ValidationError, create_model # Import create_model

from shared.core.config import settings
from .cleaning_service_base import BaseCleaningService
# Adjust import path for base rule definitions
from .cleaning_rules.base import CleaningRuleBase, RuleParamDefinition

logger = logging.getLogger(__name__)
logger.setLevel(settings.LOG_LEVEL.upper())

class RuleBasedCleaningService(BaseCleaningService):
    ...
    def __init__(
        ...
        super().__init__(cleaning_rules_config, dataset_config)
        self.rule_registry = rule_registry # Store injected registry
        self.batch_rules_info: List[Dict] = [] # Store {'instance': rule, 'params_model': model}
    ...
    def _validate_and_prepare_params(self, rule_name: str, rule_params_def: List[RuleParamDefinition], config_params: Dict) -> Optional[BaseModel]:
        ...
        for param_def in rule_params_def:
            ...
            if param_def.required and field_default is not None:
                logger.warning(f"Rule '{rule_name}', Param '{param_def.name}': 'required' is True but a default value is provided. Default will be used if not in config.")
                # Pydantic v2 uses default value if field is optional (default is not ...)
                # If required=True, Pydantic expects it unless a default is set.
                # Let Pydantic handle this logic. Keep required=True if needed.
                # field_info = Field(default=field_default) # If required=True pydantic v2 raises error on creation if not provided

            if param_def.required and field_default is None:
                 # For required fields without defaults, use Ellipsis (...)
                 field_info = Field(..., description=param_def.description)
            else:
                 # For optional fields or required fields with defaults
        ...
        try:
            # Validate the parameters from the config against the dynamic model
            ...
            for error in e.errors():
                logger.error(f"  - Param '{error['loc'][0]}': {error['msg']}")
            # Log the problematic config params for debugging
            logger.error(f"  - Received Params: {config_params}")
            # Log the expected model schema for debugging
            # logger.error(f"  - Expected Schema: {ParamsModel.schema_json(indent=2)}") # Pydantic v1
            logger.error(f"  - Expected Schema: {ParamsModel.model_json_schema(indent=2)}") # Pydantic v2
    ...
    def _load_and_prepare_rules(self):
        ...
        for rule_cfg in self.cleaning_rules_config:
            ...
            if not rule_name or not is_enabled:
                continue
            ...
            if rule_cls:
                try:
                    instance = rule_cls()
                    # Validate config parameters NOW using the dynamic model approach
                    # This prepares the *default* parameters for the rule if config is missing some
                    # Actual runtime parameters will be merged later if needed
                    ...
                    if validated_params_model is None:
                         ...
                         continue # Skip rule if params don't validate

                    rule_info = {
                        'instance': instance,
                        'params_model': validated_params_model # Store the validated model instance
                    }

                    if instance.is_batch_safe:
                        self.batch_rules_info.append(rule_info)
                    else:
                        self.global_rules_info.append(rule_info)
        ...
        logger.info(f"RuleBased: Loaded and prepared {len(self.batch_rules_info)} batch-safe and {len(self.global_rules_info)} global rules.")
    ...
    def apply_batch_rules(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for rule_info in self.batch_rules_info:
            ...
            # Use the validated parameters (including defaults) stored during load
            validated_params: BaseModel = rule_info['params_model']
            rule_name = instance.rule_name

            try:
                start_shape = current_df.shape
                # Pass the validated parameters as a dictionary
                ...
                if start_shape != end_shape:
                    logger.debug(f"Batch rule '{rule_name}' applied. Shape change: {start_shape} -> {end_shape}")
                if current_df.empty:
