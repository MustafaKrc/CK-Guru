...
class RuleBasedCleaningService(ICleaningService):  # Inherit directly from interface
    ...
    def _validate_and_prepare_params(
        self,
        rule_name: str,
        rule_params_def: List[RuleParamDefinition],
        config_params: Dict,
    ) -> Optional[BaseModel]:
        """
        ...
        for param_def in rule_params_def:
            ...
            if param_def.required and field_default is None:
                ...
                field_info = Field(
                    default=field_default, description=param_def.description
                )
        ...
        if not param_fields:
            ParamsModel = create_model(
                f"{rule_name.capitalize()}Params", **{}
            )  # Empty model
        else:
            ParamsModel = create_model(
                f"{rule_name.capitalize()}Params", **param_fields
            )

        try:
            validated_params_model = ParamsModel(**config_params)
            logger.debug(
                f"Rule '{rule_name}': Parameters validated successfully: {validated_params_model.model_dump()}"
            )
            ...
            logger.error(
                f"  - Expected Schema: {ParamsModel.model_json_schema(indent=2)}"
            )
            return None
    ...
    def _load_and_prepare_rules(self):
        ...
        for rule_cfg in self.cleaning_rules_config:
            ...
            if rule_cls:
                try:
                    instance = rule_cls()
                    validated_params_model = self._validate_and_prepare_params(
                        rule_name, instance.parameters, config_params
                    )
                    if validated_params_model is None:
                        logger.error(
                            f"Skipping rule '{rule_name}' due to parameter validation errors."
                        )
                        continue
                    rule_info = {
                        "instance": instance,
                        "params_model": validated_params_model,
                    }
                    ...
                    logger.debug(
                        f"Loaded and prepared rule '{rule_name}' (Batch safe: {instance.is_batch_safe})"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to instantiate or prepare rule '{rule_name}': {e}",
                        exc_info=True,
                    )
            else:
                logger.warning(
                    f"Rule '{rule_name}' enabled in config but not found in registry."
                )
        logger.info(
    ...
    def apply_batch_rules(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.batch_rules_info:
            return df
        logger.debug(
            f"Applying {len(self.batch_rules_info)} batch-safe cleaning rules..."
        )
        current_df = df
        for rule_info in self.batch_rules_info:
            ...
            try:
                ...
                current_df = instance.apply(
                    current_df, validated_params.model_dump(), self.dataset_config
                )
                end_shape = current_df.shape
                if start_shape != end_shape:
                    logger.debug(
                        f"Batch rule '{rule_name}' applied. Shape change: {start_shape} -> {end_shape}"
                    )
                if current_df.empty:
                    logger.warning(
                        f"DataFrame became empty after applying batch rule '{rule_name}'."
                    )
                    break
            except Exception as e:
                logger.error(
                    f"Error applying batch rule '{rule_name}': {e}", exc_info=True
                )
        return current_df
    ...
    def apply_global_rules(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for rule_info in self.global_rules_info:
            ...
            try:
                ...
                end_shape = current_df.shape
                logger.info(
                    f"Global rule '{rule_name}' applied. Shape change: {start_shape} -> {end_shape}"
                )
                if current_df.empty:
                    logger.warning(
                        f"DataFrame became empty after applying global rule '{rule_name}'."
                    )
                    break
            except Exception as e:
                logger.error(
                    f"Error applying global rule '{rule_name}': {e}", exc_info=True
                )
