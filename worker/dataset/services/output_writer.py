...
class OutputWriter(IOutputWriter): # Implement interface
    ...
    def __init__(self, storage_options: Dict):
        ...
        self._fs: Optional[s3fs.S3FileSystem] = None # Type hint for lazy loaded attribute
    ...
    def write_parquet(self, df: pd.DataFrame, s3_uri: str, target_column_name: Optional[str] = None):
        ...
        try:
            ...
            logger.error(f"Error writing final dataset to s3://{s3_path}: {write_err}", exc_info=True)
            # Attempt cleanup of potentially partial file
            try:
                 if self.fs.exists(s3_path):
                     logger.warning(f"Attempting cleanup of failed write at s3://{s3_path}")
                     self.fs.rm(s3_path)
            except Exception as cleanup_err:
                 logger.error(f"Failed cleanup after write error to s3://{s3_path}: {cleanup_err}")
