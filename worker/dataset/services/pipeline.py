...
from .dependencies import StepRegistry, DependencyProvider # Import DI classes
    ...
    def run(self, initial_context: DatasetContext) -> DatasetContext:
        ...
        try:
            for i, step_class in enumerate(steps_to_run):
                ...
                step_log_prefix = f"Task {context.task_instance.request.id} - Step [{step_key}]"
                pipeline_logger.info(f"Executing step {i+1}/{total_steps} [{step_key}]...")
                ...
                pipeline_logger.info(f"Completed step {i+1}/{total_steps} [{step_key}].")
            ...
            pipeline_logger.error(f"Pipeline execution failed at step [{step_name_failed}]: {e}", exc_info=True)
