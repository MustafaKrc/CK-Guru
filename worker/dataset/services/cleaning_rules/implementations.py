...
class Rule0DropDuplicates(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if not valid_subset:
            logger.warning("Rule 0 skipped: No valid subset columns found (%s).", subset_cols)
            return df
        try:
            df_clean = df.drop_duplicates(subset=valid_subset, keep='first') # Keep first occurence
            ...
            return df_clean
             ...
             logger.error(f"Rule 0 failed due to unhashable type in subset {valid_subset}: {e}. Returning original DataFrame.", exc_info=True)
             return df
        except Exception as e:
             logger.error(f"Rule 0 failed unexpectedly: {e}. Subset: {valid_subset}. Dtypes: {df[valid_subset].dtypes if valid_subset else 'N/A'}", exc_info=True)
...
class Rule2RemoveRecentCleanLastChange(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if not all(col in df.columns for col in required_cols):
            logger.warning(f"Rule 2 skipped: Missing one or more required columns: {required_cols}.")
            return df
        if df.empty:
            return df
        ...
        df['last_class_time'] = df.groupby('class_name')['author_date_unix_timestamp'].transform('max')
        ...
        mask_is_last_for_class = df['author_date_unix_timestamp'] == df['last_class_time']
        ...
        df_clean = df[~mask_to_drop].drop(columns=['last_class_time']) # Drop the temporary column
        dropped = initial_len - df_clean.shape[0]
        if dropped > 0:
            logger.debug(f"Rule 2: Dropped {dropped} recent, clean last changes for classes.")
        return df_clean
...
class Rule3RemoveEmptyClass(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in required_cols):
            ...
            mask = (df['totalMethodsQty'].fillna(0) > 0) | (df['totalFieldsQty'].fillna(0) > 0)
            df_clean = df[mask]
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                 logger.debug(f"Rule 3: Dropped {dropped} rows corresponding to empty classes.")
            return df_clean
...
class Rule4RemoveTrivialGetSet(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in required_cols):
            ...
            mask = ~((df['totalMethodsQty'].fillna(0) > 0) & (df['wmc'].fillna(0) <= 1) & (df['rfc'].fillna(0) <= 1))
            df_clean = df[mask]
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                logger.debug(f"Rule 4: Dropped {dropped} rows likely corresponding to trivial getter/setter classes.")
            return df_clean
...
class Rule5RemoveNoAddedLines(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if 'la' in df.columns:
            ...
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                 logger.debug(f"Rule 5: Dropped {dropped} rows with no lines added (la <= 0).")
            return df_clean
...
class Rule6RemoveCommentOnlyChange(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if d_cols:
            ...
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                 logger.debug(f"Rule 6: Dropped {dropped} rows with only comment changes (all d_* were 0).")
            return df_clean
...
class Rule7RemoveTrivialMethodChange(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in required_cols):
            ...
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                logger.debug(f"Rule 7: Dropped {dropped} rows with trivial line changes (<{min_change}) but method count changes.")
            return df_clean
...
class Rule8RemoveTypeExceptionFiles(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if 'file' in df.columns:
            ...
            mask = df['file'].astype(str).str.contains(r'(Type|Exception)\.java$', regex=True, na=False)
            df_clean = df[~mask] # Keep rows where the mask is False
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                logger.debug(f"Rule 8: Dropped {dropped} rows matching '*Type.java' or '*Exception.java'.")
            return df_clean
...
class Rule9RemoveDeadCode(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in required_cols):
            ...
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                logger.debug(f"Rule 9: Dropped {dropped} rows likely corresponding to dead code (cbo=0, fanin=0).")
            return df_clean
...
class Rule10RemoveDataClass(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in required_cols):
            ...
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                logger.debug(f"Rule 10: Dropped {dropped} rows likely corresponding to data classes.")
            return df_clean
...
class Rule11RemoveNoCodeChange(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in required_cols):
            ...
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                logger.debug(f"Rule 11: Dropped {dropped} rows with no lines added or deleted.")
            return df_clean
...
class Rule13RemoveMinimalChange(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if d_cols and 'is_buggy' in df.columns:
            ...
            dropped = initial_len - df_clean.shape[0]
            if dropped > 0:
                 logger.debug(f"Rule 13: Dropped {dropped} non-buggy rows with minimal delta sum < {threshold}.")
            return df_clean
...
class RuleClusterLargeCommits(CleaningRuleBase):
    ...
    def apply(self, df: pd.DataFrame, params: Dict[str, Any], config: Dict[str, Any]) -> pd.DataFrame:
        ...
        if not all(col in df.columns for col in required_cols):
            logger.warning(f"Rule Cluster skipped: Missing required columns: {required_cols}.")
            return df
        if df.empty:
            return df
        ...
        avg_count = int(round(df_below['changed_file_count'].mean())) if not df_below.empty else 1
        ...
        if not cluster_features:
            logger.warning("Rule Cluster skipped: No numeric features found for clustering.")
        ...
        for commit, group in df_above.groupby(commit_hash_col):
            ...
            for feature in cluster_features:
                if group_for_cluster[feature].isnull().any():
                    ...
                    group_for_cluster[feature].fillna(0, inplace=True) # Fallback if all were NaN

            try:
                n_clusters = min(avg_count, group.shape[0]) # Cannot have more clusters than samples
                if n_clusters < 1: # Should not happen if group is not empty, but safeguard
                     logger.warning(f"Rule Cluster: Calculated n_clusters={n_clusters} for commit {commit[:7]}. Skipping clustering for this commit.")
                     reduced_rows_list.append(group)
                     continue

                kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init='auto', init='k-means++')
                clusters = kmeans.fit_predict(group_for_cluster[cluster_features])
                group_for_cluster['cluster'] = clusters
            except Exception as e:
                logger.error(f"Rule Cluster: Clustering failed for commit {commit[:7]}: {e}. Keeping original rows.", exc_info=True)
                reduced_rows_list.append(group)
                continue

            agg_funcs = {}
            def aggregate_target(series):
                return (series.astype(float).mean() >= 0.5) if not series.empty else False
            ...
            for col in group_for_cluster.columns:
                ...
                if col == commit_hash_col or col in info_columns:
                    agg_funcs[col] = 'first'
                elif col in cluster_features: # Aggregate numeric features used for clustering
                    agg_funcs[col] = 'mean'
                elif col in y_column:
                    agg_funcs[col] = aggregate_target
                elif pd.api.types.is_numeric_dtype(col_type): # Aggregate other numeric cols
                     agg_funcs[col] = 'mean'
            ...
            try:
                ...
                logger.error(f"Rule Cluster: Aggregation failed for commit {commit[:7]}: {agg_e}. Keeping original rows.", exc_info=True)
                reduced_rows_list.append(group)
