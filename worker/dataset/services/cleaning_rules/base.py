...
def register_rule(cls: Type[CleaningRuleBase]):
    ...
    if cls.rule_name in WORKER_RULE_REGISTRY:
        logger.warning(f"Rule name '{cls.rule_name}' from class {cls.__name__} already registered. Overwriting.")
...
def discover_rules(module_path: str = "services.cleaning_rules.implementations"):
    ...
    try:
        ...
        logger.info(f"Rule discovery complete. {len(WORKER_RULE_REGISTRY)} rules registered globally from '{module_path}'.")
    except ImportError as e:
        logger.error(f"Could not discover rules from module '{module_path}': {e}")
    except Exception as e:
        logger.error(f"Error during rule discovery from '{module_path}': {e}", exc_info=True)