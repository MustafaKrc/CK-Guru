...
class ApplyGlobalCleaningRulesStep(IDatasetGeneratorStep):
    ...
    def execute(
        ...
        if context.processed_dataframe is None or context.processed_dataframe.empty:
            step_logger.warning(
                "Input DataFrame is None or empty, skipping global cleaning."
            )
            return context
        ...
        step_logger.info(
            f"Applying global cleaning rules to DataFrame with shape {initial_shape}..."
        )
        ...
        try:
            ...
            step_logger.info(
                f"Global cleaning rules applied. Shape change: {initial_shape} -> {final_shape}"
            )
            if context.processed_dataframe.empty:
                step_logger.warning(
                    "DataFrame is empty after applying global cleaning rules."
                )
                context.warnings.append(f"{self.name}: DataFrame became empty.")
        except Exception as e:
            step_logger.error(
                f"Error applying global cleaning rules: {e}", exc_info=True
            )
