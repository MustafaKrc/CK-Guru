...
from shared.repositories import DatasetRepository # For final status update
...
class WriteOutputStep(IDatasetGeneratorStep):
    ...
    def execute(
        ...
        **kwargs
        ...
        context.output_storage_uri = f"s3://{settings.S3_BUCKET_NAME}/datasets/{filename}"
        ...
        target_column = context.dataset_config.target_column if context.dataset_config else None
        ...
        try:
            ...
            output_writer.write_parquet(df_final, context.output_storage_uri, target_column_name=target_column)
            ...
            if context.rows_written >= min_rows_for_sampling:
                step_logger.info(f"Creating background data sample (size={sample_size})...")
                background_sample_df = df_final.sample(
                    n=min(sample_size, context.rows_written), random_state=42
                )
                step_logger.info(f"Writing background sample ({background_sample_df.shape}) to {context.background_sample_uri}...")
                output_writer.clear_existing(context.background_sample_uri)
                output_writer.write_parquet(background_sample_df, context.background_sample_uri, target_column_name=target_column)
                background_sample_written = True
                step_logger.info("Background sample written successfully.")
            else:
                step_logger.warning(f"Dataset too small ({context.rows_written} rows) for background sampling (min: {min_rows_for_sampling}). Skipping.")
                context.background_sample_uri = None # Ensure URI is None if not written
            ...
            if not updated:
                 ...
                 step_logger.critical("Failed to update dataset status to READY in DB after successful write!")
                 context.warnings.append("CRITICAL: Failed to update DB status to READY.")
            ...
            step_logger.error(f"Error during output writing or final DB update: {e}", exc_info=True)
            ...
            if context.output_storage_uri: output_writer.clear_existing(context.output_storage_uri)
            if context.background_sample_uri: output_writer.clear_existing(context.background_sample_uri)
