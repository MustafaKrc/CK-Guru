...
class CalculateCommitStatsStep(IDatasetGeneratorStep):
    ...
    def execute(self, context: DatasetContext, **kwargs) -> DatasetContext:
        ...
        step_logger.info(
            f"Calculating commit stats for DataFrame with shape {df.shape}..."
        )
        ...
        try:
            if "files_changed" in df.columns:
                ...
                step_logger.warning(
                    "Missing 'files_changed' column, setting 'changed_file_count' to 0."
                )
                df["changed_file_count"] = 0

            required_line_cols = ["la", "ld", "changed_file_count"]
            if all(col in df.columns for col in required_line_cols):
                denominator = df["changed_file_count"].replace(
                    0, 1
                )  # Avoid division by zero
                df["lines_per_file"] = (
                    df["la"].fillna(0) + df["ld"].fillna(0)
                ) / denominator
            else:
                missing_cols = [
                    col for col in required_line_cols if col not in df.columns
                ]
                step_logger.warning(
                    f"Missing columns ({missing_cols}) for 'lines_per_file' calculation, setting to 0."
                )
                df["lines_per_file"] = 0
