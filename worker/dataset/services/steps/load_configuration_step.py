...
from shared.repositories import DatasetRepository, RepositoryRepository, BotPatternRepository
...
class LoadConfigurationStep(IDatasetGeneratorStep):
    ...
    def execute(
        ...
        **kwargs
        ...
        step_logger.info(f"Loading configuration for Dataset ID: {context.dataset_id}...")
        ...
        try:
            ...
            if not context.dataset_db:
                raise NotFoundError(f"Dataset definition {context.dataset_id} not found.")

            # Check if already being generated by another task
            if context.dataset_db.status == DatasetStatusEnum.GENERATING and context.dataset_db.celery_task_id != context.task_instance.request.id:
            ...
            # --- Load Repository ---
            context.repository_db = repository_repo.get_by_id(context.dataset_db.repository_id)
            if not context.repository_db:
                raise NotFoundError(f"Repository {context.dataset_db.repository_id} not found for dataset {context.dataset_id}.")

            # --- Load Bot Patterns ---
            # Use BotPatternRepository method
            context.bot_patterns_db = list(bot_pattern_repo.get_patterns(repository_id=context.repository_db.id, include_global=True))
            step_logger.info(f"Fetched {len(context.bot_patterns_db)} applicable bot patterns.")

            # --- Load and Validate Config ---
            config_dict = context.dataset_db.config if isinstance(context.dataset_db.config, dict) else {}
            try:
                context.dataset_config = DatasetConfig(**config_dict) # Validate against schema
                ...
                step_logger.error(f"Invalid dataset configuration: {config_err}", exc_info=True)
                raise ValueError(f"Invalid dataset configuration: {config_err}") from config_err
            ...
            updated = job_status_updater.update_dataset_start(
                ...
                task_id=context.task_instance.request.id
            )
            if not updated:
                # If status update fails, we cannot proceed reliably
                raise RuntimeError("Failed to update dataset status to GENERATING in DB.")

            step_logger.info("Configuration loaded successfully. Dataset status set to GENERATING.")
            ...
            step_logger.error(f"Unexpected error loading configuration: {e}", exc_info=True)
            # Try to update status if dataset was loaded
            if context.dataset_db:
                job_status_updater.update_dataset_completion(
                    context.dataset_id, DatasetStatusEnum.FAILED, f"Unexpected error during config load: {e}"
                )
