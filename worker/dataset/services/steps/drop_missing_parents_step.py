...
class DropMissingParentsStep(IDatasetGeneratorStep):
    ...
    def execute(self, context: DatasetContext, **kwargs) -> DatasetContext:
        ...
        if '_parent_metric_found' not in df.columns:
            step_logger.warning("'_parent_metric_found' column not present. Assuming no parents were missing or step was skipped.")
            return context # Return context unmodified if flag isn't there

        initial_rows = len(df)
        step_logger.info(f"Dropping rows with missing parents from DataFrame with shape {df.shape}...")
        ...
        try:
            ...
            context.processed_dataframe.drop(columns=['_parent_metric_found'], inplace=True, errors='ignore')

            dropped = initial_rows - len(context.processed_dataframe)
            step_logger.info(f"Dropped {dropped} rows due to missing parent metrics. New shape: {context.processed_dataframe.shape}")
        except Exception as e:
             step_logger.error(f"Error dropping rows with missing parents: {e}", exc_info=True)
