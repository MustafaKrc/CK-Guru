...
class CalculateDeltaMetricsStep(IDatasetGeneratorStep):
    ...
    def execute(self, context: DatasetContext, **kwargs) -> DatasetContext:
        ...
        if context.processed_dataframe is None or context.processed_dataframe.empty:
            step_logger.warning("Input DataFrame is None or empty, skipping delta calculation.")
            return context

        delta_df = context.processed_dataframe # Work directly on the context DataFrame
        step_logger.info(f"Calculating delta metrics for DataFrame with shape {delta_df.shape}...")
        ...
        if '_parent_metric_found' not in delta_df.columns:
            step_logger.error("'_parent_metric_found' column missing. Cannot calculate delta metrics.")
            context.warnings.append(f"{self.name}: Missing '_parent_metric_found' column.")
            # Add empty delta columns?
            for col in CK_METRIC_COLUMNS: delta_df[f"d_{col}"] = np.nan
            ...
            for col in CK_METRIC_COLUMNS:
                ...
                if current_col in delta_df.columns and parent_col in delta_df.columns:
                    ...
                    delta_df.loc[mask_invalid, delta_col] = np.nan
                     ...
                     step_logger.warning(f"Current column '{current_col}' not found, cannot calculate delta '{delta_col}'.")
                     delta_df[delta_col] = np.nan # Add NaN delta column
                elif parent_col not in delta_df.columns:
                     step_logger.warning(f"Parent column '{parent_col}' not found, cannot calculate delta '{delta_col}'.")
                     delta_df[delta_col] = np.nan # Add NaN delta column
