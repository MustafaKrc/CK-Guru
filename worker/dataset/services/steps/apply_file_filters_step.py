...
class ApplyFileFiltersStep(IDatasetGeneratorStep):
    ...
    def execute(self, context: DatasetContext, **kwargs) -> DatasetContext:
        ...
        try:
            ...
            is_not_package_info = (
                ~df["file"].astype(str).str.endswith("package-info.java", na=False)
            )
            ...
            valid_file_mask = (
                is_java & is_not_package_info & is_not_test & is_not_example
            )
            ...
            step_logger.info(
                f"File filters dropped {dropped} rows. New shape: {context.processed_dataframe.shape}"
            )
