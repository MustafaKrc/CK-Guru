...
class ProcessGloballyStep(IDatasetGeneratorStep):
    ...
    def __init__(self):
        ...
        logger.debug(
            f"Initialized with global steps: {[s.name for s in self.global_steps]}"
        )
        ...
        **kwargs,
        ...
        try:
            ...
            for sub_step in self.global_steps:
                ...
                if (
                    current_context.processed_dataframe is None
                    or current_context.processed_dataframe.empty
                ):
                    step_logger.warning(
                        f"DataFrame became empty after global sub-step [{sub_step.name}]. Skipping remaining global steps."
                    )
                    break
