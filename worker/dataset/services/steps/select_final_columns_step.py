...
class SelectFinalColumnsStep(IDatasetGeneratorStep):
    ...
    def execute(self, context: DatasetContext, **kwargs) -> DatasetContext:
        ...
        if context.processed_dataframe is None or context.processed_dataframe.empty:
            step_logger.warning("Input DataFrame is None or empty, cannot select final columns.")
            context.final_dataframe = pd.DataFrame() # Ensure final_dataframe is empty df
            return context

        df = context.processed_dataframe
        step_logger.info(f"Selecting final columns from DataFrame with shape {df.shape}...")

        if not context.dataset_config:
             step_logger.error("Dataset configuration missing in context. Cannot determine final columns.")
             raise ValueError("Dataset configuration required for final column selection.")
        ...
        if not feature_columns:
            ...
            raise ValueError("Feature columns must be specified in dataset configuration.")
        if not target_column:
             step_logger.error("No target column defined in dataset configuration.")
             raise ValueError("Target column must be specified in dataset configuration.")
        ...
        try:
            ...
            step_logger.info(f"Selected {len(final_columns)} final columns. Output shape: {context.final_dataframe.shape}")
        except KeyError as e:
             step_logger.error(f"KeyError during final column selection: {e}. Columns: {final_columns}. Available: {available_columns}", exc_info=True)
             raise ValueError(f"Failed to select final columns: {e}") from e
        except Exception as e:
             step_logger.error(f"Unexpected error during final column selection: {e}", exc_info=True)
