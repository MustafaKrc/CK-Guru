...
class FeatureSelectionStep(IDatasetGeneratorStep):
    ...
    def execute(self, context: DatasetContext, **kwargs) -> DatasetContext:
        ...
        if context.final_dataframe is None or context.final_dataframe.empty:
            step_logger.warning("Input DataFrame is None or empty, skipping feature selection.")
            return context
        ...
        try:
            ...
            current_features = [col for col in context.final_dataframe.columns if col != target_column_name]
            features_df = context.final_dataframe[current_features]
            target_series = context.final_dataframe[target_column_name]

            step_logger.info(f"Selecting features from {len(current_features)} available features")

            # Apply feature selection
            selected_features = strategy.select_features(features_df, target_series, params)
            
            if not selected_features:
                step_logger.warning(f"Algorithm '{algorithm_name}' returned no features. Dataset will only contain target column.")
                selected_features = []  # Ensure it's an empty list
            else:
                step_logger.info(f"Selected {len(selected_features)} features: {selected_features}")
            ...
            step_logger.info(f"Feature selection complete. Final DataFrame shape: {context.final_dataframe.shape}")
