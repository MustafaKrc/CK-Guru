...
def sync_definitions_to_db():
    ...
    try:
        with get_sync_db_session() as session:
            ...
            }  # Use attribute access instead of dict subscription
            session.query(FeatureSelectionDefinitionDB).filter(
                FeatureSelectionDefinitionDB.last_updated_by == worker_id,
            ...
            if definitions_to_upsert:
                upsert_stmt = pg_insert(FeatureSelectionDefinitionDB).values(
                    definitions_to_upsert
                )
            ...
            session.commit()
            logger.info(
                f"Successfully synced {len(definitions)} feature selection definitions to DB."
            )
    except Exception as e:
        logger.error(
            f"Failed to sync feature selection definitions to DB: {e}", exc_info=True
        )
