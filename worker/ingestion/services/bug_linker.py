...
class GitCommitLinker:
    ...
    def __init__(self, git_service: IGitService):
        ...
        logger.info(
            f"GitCommitLinker initialized for repository: {git_service.repo_path}"
        )

    def link_corrective_commits(
        self,
        corrective_commits_info: Mapping[
            str, Optional[int]
        ],  # Hash -> Earliest Issue Timestamp
    ) -> Dict[str, List[str]]:
        """
        ...
        bug_link_map: Dict[str, List[str]] = (
            {}
        )  # buggy_hash -> [fixing_hash1, fixing_hash2,...]
        total_corrective = len(corrective_commits_info)
        processed_count = 0

        logger.info(
            f"BugLinker: Starting bug linking for {total_corrective} corrective commits..."
        )
        ...
        for corrective_hash in corrective_commit_hashes:
            processed_count += 1
            earliest_issue_ts = corrective_commits_info.get(
                corrective_hash
            )  # Get timestamp

            if processed_count % 10 == 0 or processed_count == total_corrective:
                ts_info = (
                    f"(IssueTS: {earliest_issue_ts})"
                    if earliest_issue_ts
                    else "(No IssueTS)"
                )
                logger.info(
                    f"BugLinker: Linking {processed_count}/{total_corrective}: {corrective_hash[:7]} {ts_info}..."
                )
            ...
            try:
                ...
                if not modified_regions:
                    logger.debug(
                        f"BugLinker: No code regions found for {corrective_hash[:7]}."
                    )
                    continue
                ...
                logger.error(
                    f"BugLinker: Error linking {corrective_hash[:7]}: {e}",
                    exc_info=True,
                )

        linked_count = len(bug_link_map)
        logger.info(
            f"BugLinker: Finished. Identified {linked_count} potential bug-introducing commits from {total_corrective} corrective commits."
        )
        if logger.isEnabledFor(logging.DEBUG) and bug_link_map:
            debug_map_str = json.dumps(
                {
                    k[:7]: [v[:7] for v in vals]
                    for k, vals in list(bug_link_map.items())[:5]
                },
                indent=2,
            )
    ...
    def _get_modified_regions(self, commit_hash: str) -> Dict[str, List[int]]:
        ...
        try:
            ...
            files_modified_output = self.git_service.run_git_command(
                files_cmd_args, check=True
            )
            files_modified = set(
                fn.strip() for fn in files_modified_output.splitlines() if fn.strip()
            )

            modified_regions = self._parse_diff_for_modified_lines(
                diff_output, files_modified
            )
            ...
            if (
                "unknown revision" in err_lower
                or "bad revision" in err_lower
                or "no merge base" in err_lower
            ):
                logger.warning(
                    f"BugLinker: Could not get diff for {commit_hash[:7]} (likely initial commit or merge issue): {err_lower}"
                )
            else:
                logger.error(
                    f"BugLinker: Error getting git diff for {commit_hash[:7]}: {err_lower}"
                )
            return {}  # Return empty on error
        except Exception as e:
            logger.error(
                f"BugLinker: Unexpected error getting diff/files for {commit_hash[:7]}: {e}",
                exc_info=True,
            )
            return {}
    ...
    def _parse_diff_for_modified_lines(
        self, diff_output: str, files_modified: Set[str]
    ) -> Dict[str, List[int]]:
            ...
            if line.startswith("diff --git"):
                ...
                if match:
                    ...
                    if potential_file in files_modified:
                        file_ext_parts = potential_file.rsplit(
                            ".", 1
                        )  # Split only once from right
                        if (
                            len(file_ext_parts) > 1
                            and file_ext_parts[-1].upper() in CODE_FILE_EXTENSIONS
                        ):
                ...
                if hunk_match:
                    ...
                    while i < len(lines) and not lines[i].startswith(
                        ...
                        if hunk_line.startswith("-") and not hunk_line.startswith(
                            ...
                            modified_lines_map[current_file].append(
                                current_line_num_old
                            )
                            current_line_num_old += 1
                        elif hunk_line.startswith("+") and not hunk_line.startswith(
                            "+++"
                        ):
                            pass  # Ignore added lines
                        else:  # Context line (ignore with -U0) or other lines
                            if not hunk_line.startswith("+"):
                                current_line_num_old += 1  # Increment only if not added
                else:
                    logger.warning(
                        f"BugLinker: Could not parse hunk header: {line} in file {current_file}"
                    )
    ...
    def _git_annotate_regions(
        ...
        initial_blame_start_commit = self.git_service.find_commit_hash_before_timestamp(
            earliest_issue_timestamp
        )

        if not initial_blame_start_commit:
            logger.error(
                f"BugLinker: Could not determine blame start for {corrective_commit_hash[:7]}. Skipping blame."
            )
        ...
        for file_path, line_numbers in regions.items():
            ...
            for attempt in range(2):  # Max 2 attempts (initial, then fallback)
                if attempt == 1 and not used_issue_timestamp:
                    break  # No fallback if initial was already parent
                if attempt == 1:  # If it's the fallback attempt
                    current_blame_start = default_blame_start
                    logger.warning(
                        f"BugLinker: Falling back to blame from parent {current_blame_start[:7]} for '{file_path}'"
                    )

                logger.debug(
                    f"BugLinker: Blaming {file_path} L{line_numbers[0]}-{line_numbers[-1]} from {current_blame_start[:7]}"
                )
                ...
                try:
                    ...
                    if (
                        "fatal: no such path" in stderr_lower
                        and attempt == 0
                        and used_issue_timestamp
                    ):
                        logger.warning(
                            f"BugLinker: Blame from {current_blame_start[:7]} failed for '{file_path}' (no path). Will try fallback."
                        )
                        continue  # Go to next attempt (fallback)
                    elif "bad revision" in stderr_lower:
                        logger.warning(
                            f"BugLinker: Blame failed for '{file_path}' at {current_blame_start[:7]} (bad revision): {e.stderr.strip()}"
                        )
                    else:
                        logger.warning(
                            f"BugLinker: Blame failed for '{file_path}' at {current_blame_start[:7]}: {e.stderr.strip()}"
                        )
                    break  # Error other than 'no path' on first attempt, or any error on fallback: stop trying for this file
                except Exception as e:
                    logger.error(
                        f"BugLinker: Unexpected error running blame for {file_path} at {current_blame_start[:7]}: {e}",
                        exc_info=True,
                    )
                ...
                try:
                    ...
                    for line in blame_output.strip().splitlines():
                        ...
                        if (
                            ...
                            if (
                                potential_hash != corrective_commit_hash
                                and potential_hash != current_blame_start
                            ):
                                blame_hashes_in_output.add(potential_hash)
                    introducing_commits.update(blame_hashes_in_output)
                except Exception as parse_err:
                    logger.error(
                        f"BugLinker: Error parsing blame output for {file_path} (start: {current_blame_start[:7]}): {parse_err}",
                        exc_info=True,
                    )
