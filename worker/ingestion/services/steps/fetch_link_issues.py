...
from shared.repositories import GitHubIssueRepository, CommitGuruMetricRepository
...
class FetchAndLinkIssuesStep(IngestionStep):
    ...
    def execute(self, context: IngestionContext, *,
                ...
                repository_api_client: IRepositoryApiClient) -> IngestionContext:
        ...
        for payload in context.raw_commit_guru_data:
            if payload.commit_message: # Check if message exists
                issue_numbers = repository_api_client.extract_issue_ids(payload.commit_message)
                if issue_numbers:
                    commit_hash_to_issue_numbers[payload.commit_hash] = issue_numbers

        if not commit_hash_to_issue_numbers:
            self._log_info(context, "No issue numbers found in commit messages to link.")
            return context
        ...
        for commit_hash, issue_numbers in commit_hash_to_issue_numbers.items():
            ...
            if not commit_db_id or commit_db_id == -1: # Check if ID is valid
                self._log_warning(context, f"Skipping issue linking for {commit_hash[:7]}, DB ID not found or invalid.")
                continue
            ...
            for number_str in issue_numbers:
                ...
                api_response = repository_api_client.get_issue(owner, repo_name, number_str)
                ...
                elif api_response.status_code not in [404, 410]: # Log if not found isn't the reason
                    self._log_warning(context, f"Failed to get/create DB entry for issue #{issue_number} for commit {commit_hash[:7]}. API Status: {api_response.status_code}")
            ...
            if linked_issue_db_ids_for_commit:
                try:
                    ...
                    guru_repo.link_issues_to_commit(commit_db_id, linked_issue_db_ids_for_commit)
                    self._log_debug(context, f"Attempted linking {len(linked_issue_db_ids_for_commit)} issues to commit ID {commit_db_id}.")
                except Exception as link_err:
                    # Log error but continue processing other commits
                    self._log_error(context, f"Failed linking issues for commit ID {commit_db_id}: {link_err}", exc_info=False)

            # Update progress periodically
            if total_links_to_process > 0 and processed_link_count % 50 == 0:
                step_progress = int(100 * (processed_link_count / total_links_to_process))
