...
import pandas as pd
from git import Repo, GitCommandError # Keep GitPython import for Repo object usage
...
class CalculateCKMetricsStep(IngestionStep):
    ...
    def execute(self, context: IngestionContext, *,
                ck_runner: ICKRunnerService,
                ck_repo: CKMetricRepository,
                git_service: IGitService) -> IngestionContext:
        ...
        try:
            ...
            if context.is_single_commit_mode:
                ...
                self._log_info(context, f"Calculating CK metrics for specific commits: {[h[:7] for h in commits_to_process_hashes]}")
            else:
                # Full History Mode
                self._log_info(context, "Starting CK metric calculation for repository history...")
                ...
                commits_iterator = context.repo_object.iter_commits(rev=default_branch_ref) # Remove first_parent=True to get all
                commits_to_process_hashes = [c.hexsha for c in commits_iterator]
                self._log_info(context, f"Found {len(commits_to_process_hashes)} commits for CK analysis.")
            ...
            for i, commit_hash in enumerate(commits_to_process_hashes):
                step_progress = int(80 * ((i + 1) / total_commits_for_ck)) if total_commits_for_ck else 0
                self._update_progress(context, f'Calculating CK ({i+1}/{total_commits_for_ck} - {commit_hash[:7]})...', step_progress)
                ...
                metrics_exist = False
                if context.is_single_commit_mode: # Only check DB in inference mode for now
                    try:
                        metrics_exist = ck_repo.check_metrics_exist_for_commit(context.repository_id, commit_hash)
                    except Exception as db_check_err:
                        self._log_warning(context, f"Failed to check DB for existing CK metrics for {commit_hash[:7]}: {db_check_err}. Will attempt calculation.")

                if metrics_exist:
                    self._log_info(context, f"CK metrics already exist in DB for {commit_hash[:7]}. Skipping calculation.")
                ...
                self._log_info(context, f"Running CK calculation for commit {commit_hash[:7]}...")
                ...
                if not metrics_df.empty:
                    ...
                    for record_dict in metrics_df.to_dict(orient='records'):
                        try:
                            ...
                            self._log_warning(context, f"Validation error creating CKMetricPayload for {commit_hash[:7]}, file {record_dict.get('file')}: {e}. Skipping record.")
                        except Exception as model_e:
                            self._log_error(context, f"Unexpected error creating CK Pydantic model for {commit_hash[:7]}: {model_e}", exc_info=True)


                    if ck_payload_list:
                         context.raw_ck_metrics[commit_hash] = ck_payload_list
                         self._log_debug(context, f"Stored {len(ck_payload_list)} CK payloads for {commit_hash[:7]}.")
                    else:
                         self._log_info(context, f"No valid CK payloads generated from DataFrame for commit {commit_hash[:7]}.")
                else:
        ...
        self._log_info(context, f"Finished CK processing step. Generated/Found metrics for {num_commits_with_metrics} commits.")
        self._update_progress(context, "CK processing finished.", 90) # Update progress description
        return context