...
class PersistCommitGuruMetricsStep(IngestionStep):
    ...
    def execute(self, context: IngestionContext, *, guru_repo: CommitGuruMetricRepository) -> IngestionContext:
        ...
        if not context.raw_commit_guru_data:
            self._log_info(context, "No raw Commit Guru data (Pydantic Payloads) to persist.")
            return context
        ...
        self._log_info(context, f"Preparing {total_commits} Commit Guru metric Payloads for persistence...")
        ...
        for i, payload in enumerate(commit_payloads):
            ...
            try:
                ...
                metric_data = payload.model_dump(exclude_unset=True, exclude_none=True) # exclude_none might be useful
                ...
                commits_to_upsert.append(metric_data)
                 ...
                 self._log_error(context, f"Error preparing payload for commit {payload.commit_hash[:7]}: {e}", exc_info=False)
            ...
            if total_commits > 0 and processed_count % 100 == 0:
                ...
                self._update_progress(context, f'Preparing Guru ({processed_count}/{total_commits})...', step_progress)
        ...
        if commits_to_upsert:
            ...
            try:
                ...
                self._log_info(context, f"CommitGuruMetric persistence processed {len(db_ids_map)} records.")
            except Exception as e:
                self._log_error(context, f"CommitGuruMetric persistence failed: {e}", exc_info=True)
                raise
