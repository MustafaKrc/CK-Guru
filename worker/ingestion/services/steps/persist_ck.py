...
import logging
import pandas as pd # Keep for DataFrame check if needed, but main input is list
from typing import Dict, List, Any # Add List, Any
...
class PersistCKMetricsStep(IngestionStep):
    ...
    def execute(self, context: IngestionContext, *, ck_repo: CKMetricRepository) -> IngestionContext:
        # Check the context attribute which now contains Pydantic models
        if not context.raw_ck_metrics:
            self._log_info(context, "No raw CK metrics data (Pydantic Payloads) to persist.")
            return context
        ...
        log_msg_prefix = f"Persisting CK metrics Payloads for {total_commits_to_process} commits..."
        ...
        for commit_hash, payload_list in context.raw_ck_metrics.items():
            ...
            for payload in payload_list:
                try:
                    metric_data = payload.model_dump(exclude_unset=True, exclude_none=True)
                    ...
                    if 'file' not in metric_data or not metric_data['file']:
                         logger.warning(f"CK Payload missing 'file' for commit {commit_hash[:7]}. Skipping record.")
                         continue
                    ...
                    metric_data_db = payload.model_dump(exclude_unset=True, exclude_none=True)
                    # Add IDs again
                    metric_data_db['repository_id'] = context.repository_id
                    metric_data_db['commit_hash'] = commit_hash
                    if 'file' not in metric_data_db or not metric_data_db['file']: continue # Sanity check file again


                    all_instances_to_upsert.append(metric_data_db)

                except Exception as e:
                    logger.error(f"Error preparing CK payload for commit {commit_hash[:7]}: {e}", exc_info=False)

            # Update progress periodically
            if total_commits_to_process > 0 and processed_commit_count % 50 == 0:
                step_progress = int(95 * (processed_commit_count / total_commits_to_process))
        ...
        if all_instances_to_upsert:
            ...
            try:
                ...
                self._log_error(context, f"CKMetric persistence failed: {e}", exc_info=True)
                raise
