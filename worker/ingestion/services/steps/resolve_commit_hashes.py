...
class ResolveCommitHashesStep(IngestionStep):
    ...
    async def execute(
        ...
        self._log_info(
            context,
            f"Resolving target ref '{original_target_ref}' and finding parent...",
        )
        ...
        try:
            ...
            if parent_hash:
                ...
                self._log_info(
                    context, f"Resolved parent commit: {context.parent_commit_hash}"
                )
            else:
                ...
                self._log_warning(
                    context,
                    f"Target commit {context.target_commit_hash[:7]} has no parent (initial commit).",
                )
            ...
            raise ValueError(
                str(e)
            ) from e  # Re-raise as ValueError for pipeline handling?
