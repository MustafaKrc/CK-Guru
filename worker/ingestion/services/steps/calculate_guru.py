...
from services.git_log_parser import GitLogParser, COMMIT_GURU_LOG_FORMAT, ParsedNumstatLine 
from services.commit_state_tracker import DevExperienceMetrics, FileStateTracker, DeveloperExperienceTracker, FileUpdateResult
    ...
    def execute(self, context: IngestionContext, *, git_service: IGitService) -> IngestionContext:
        ...
        for i, commit_dict_data in enumerate(parsed_commits):
            ...
            try:
                ...
                logger.warning(f"Invalid timestamp '{timestamp_str}' for commit {commit_hash[:7]}. Using 0.")
                timestamp = 0
            commit_dict_data['author_date_unix_timestamp'] = timestamp # Ensure it's int
            ...
            final_dict_data['files_changed'] = [pnl.file_name for pnl in numstat_lines_parsed] or None

            # Determine 'fix' flag
            commit_message = final_dict_data.get('commit_message', '').lower()
            CORRECTIVE_KEYWORDS = {'fix', 'bug', 'defect', 'error', 'patch'}
            final_dict_data['fix'] = any(word in commit_message for word in CORRECTIVE_KEYWORDS)
            ...
            try:
                ...
                self._log_warning(context, f"Validation error creating CommitGuruMetricPayload for {commit_hash[:7]}: {e}. Skipping commit.")
            except Exception as model_e:
                self._log_error(context, f"Unexpected error creating Pydantic model for {commit_hash[:7]}: {model_e}", exc_info=True)
        ...
        self._log_info(context, f"Calculated Commit Guru metrics for {len(context.raw_commit_guru_data)} commits (as Pydantic Payloads).")
