...
class GitService(IGitService):
    ...
    def clone_or_fetch(self, git_url: str) -> None:
        ...
        if self.repo_path.exists():
            logger.info(
                f"Found existing clone at {self.repo_path}. Fetching updates..."
            )
            try:
                ...
                logger.warning(
                    f"Error updating existing repo at {self.repo_path}. Re-cloning. Error: {e}"
                )
                try:
                    shutil.rmtree(self.repo_path)  # Clean up problematic clone
                except OSError as rm_err:
                    logger.error(
                        f"Failed to remove existing clone directory {self.repo_path}: {rm_err}"
                    )
                    raise FileNotFoundError(
                        f"Failed to clean up existing broken clone at {self.repo_path}"
                    ) from rm_err
        ...
        try:
            ...
            logger.error(
                f"Unexpected error during clone of {git_url}: {e}", exc_info=True
            )
    ...
    def run_git_command(
        ...
        try:
            ...
            raise GitCommandError(
                error_message, stderr=stderr_output, returncode=e.returncode
            ) from e
            ...
            logger.error(
                f"Unexpected error running git command '{full_cmd[:100]}...' in {self.repo_path}: {e}",
                exc_info=True,
            )
    ...
    def resolve_ref_to_hash(self, ref: str) -> str:
        ...
        try:
            full_hash = self.run_git_command(
                cmd_args, check=True, suppress_stderr=True
            ).strip()
            if not full_hash or len(full_hash) != 40:
                raise GitRefNotFoundError(
                    f"Resolved ref '{ref}' resulted in invalid hash '{full_hash}'."
                )
            ...
            if e.stderr and (
                ...
                or "bad revision" in e.stderr.lower()
            ):
                raise GitRefNotFoundError(
                    f"Reference '{ref}' not found or invalid in repository {self.repo_path}."
                ) from e
    ...
    def get_first_parent_hash(self, commit_hash: str) -> Optional[str]:
        ...
        try:
            ...
            parent_hash = self.run_git_command(
                cmd_args, check=False, suppress_stderr=True
            ).strip()
            if parent_hash and len(parent_hash) == 40:
                ...
                logger.info(
                    f"Commit {commit_hash} has no first parent (likely initial commit)."
                )
                return None
        except GitCommandError as e:
            # Should be suppressed by --quiet, but log if error occurs anyway
            logger.error(
                f"Error getting parent for commit {commit_hash}: {e}", exc_info=True
            )
            return None
    ...
    def find_commit_hash_before_timestamp(self, timestamp: int) -> Optional[str]:
        ...
        try:
            ...
            if commit_hash and len(commit_hash) == 40:  # Basic validation
                logger.debug(
                    f"Found commit {commit_hash[:7]} before timestamp {timestamp} in {self.repo_path}"
                )
                return commit_hash
            else:
                # This is expected if the timestamp is too early
                logger.info(
                    f"No commit found before timestamp {timestamp} in {self.repo_path}"
                )
                return None
        except Exception as e:  # Catch errors from run_git_command or others
            logger.error(
                f"Error finding commit before timestamp {timestamp} in {self.repo_path}: {e}",
                exc_info=True,
            )
            return None
    ...
    def checkout_commit(self, commit_hash: str, force: bool = True) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error checking out commit {commit_hash} in {self.repo_path}: {e}",
                exc_info=False,
            )  # Error logged by run_git_command
            return False
    ...
    def determine_default_branch(self) -> str:
        ...
        try:
            ...
            if not repo.heads and not origin.refs:
                logger.warning(
                    f"No local heads or remote refs found in {self.repo_path}. Fetching explicitly."
                )
                origin.fetch()
            ...
            if available_refs:
                ...
                logger.warning(
                    f"Could not determine default branch (main/master/HEAD). Using fallback ref: {fallback_ref}"
                )
                return fallback_ref
            else:
            ...
            logger.error(
                f"Error determining default branch in {self.repo_path}: {e}",
                exc_info=True,
            )
