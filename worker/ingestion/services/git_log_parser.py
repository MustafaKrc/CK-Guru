...
class GitLogParser:
    ...
    def parse_custom_log(self, log_output: str) -> List[Dict[str, Any]]:
        ...
        if not commit_blobs or (len(commit_blobs) == 1 and not commit_blobs[0]):
            logger.warning(
                "Log output does not contain any commit markers or is empty."
            )
            return []
        ...
        for blob in commit_blobs:
            ...
            if not match:
                logger.warning(
                    f"Could not parse commit blob structure: {blob[:100]}..."
                )
                continue
        ...
        logger.info(
            f"GitLogParser: Parsed {len(commits_data)} commits from log output."
        )
        return commits_data

    def parse_numstat_line(
        self, line: str, commit_hash_debug: str
    ) -> Optional[ParsedNumstatLine]:
        ...
        if len(parts) != 3:
            ...
            if line.strip():
                logger.warning(
                    f"Skipping malformed numstat line (parts!=3): '{line}' in commit {commit_hash_debug}"
                )
            return None
        ...
        try:
            ...
            logger.warning(
                f"Could not parse LA/LD '{la_str}/{ld_str}' for file path '{file_path_raw}' in commit {commit_hash_debug}. Skipping file."
            )
            return None
        ...
        if " => " in file_name:
            ...
            if match:
                ...
                logger.debug(
                    f"Processed rename syntax '{file_path_raw}' to '{file_name}'"
                )
            else:
                logger.warning(
                    f"Could not reliably parse rename syntax: '{file_path_raw}'. Using raw path."
                )
        ...
        subsystem = (
            file_parts[0] if len(file_parts) > 1 else "root"
        )  # Use root if no '/'
