...
class PipelineRunner:
    ...
    def __init__(
        ...
        logger.debug(f"PipelineRunner initialized with strategy: {type(strategy).__name__}")
    ...
    def run(self, initial_context: IngestionContext) -> IngestionContext:
        ...
        try:
            for i, step_class in enumerate(steps_to_run):
                ...
                logger.info(f"PipelineRunner: Executing step {i+1}/{total_steps} [{self.current_step_instance.name}]...")
                ...
                logger.info(f"PipelineRunner: Completed step {i+1}/{total_steps} [{self.current_step_instance.name}].")

                #Update overall progress based on step completion
                runner_progress = int(100 * ((i + 1) / total_steps))
                context.task_instance.update_state(state='PROGRESS', meta={'progress': runner_progress, 'step': f"Completed {step_key}"})


            # Optional: Commit Unit of Work if implemented in DependencyProvider
            self.dependency_provider.commit_unit_of_work()

            logger.info(f"PipelineRunner: Pipeline execution finished successfully.")
            return context
            ...
            step_name_failed = self.current_step_instance.name if self.current_step_instance else step_key
            logger.error(f"Pipeline execution failed at step [{step_name_failed}]: {e}", exc_info=True)
            # Add failure info to context warnings? Or rely on exception propagation?
            context.warnings.append(f"Pipeline failed at step [{step_name_failed}]: {type(e).__name__}")
