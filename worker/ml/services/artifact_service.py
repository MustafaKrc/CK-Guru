...
class ArtifactService(IArtifactService):
    ...
    def fs(self) -> s3fs.S3FileSystem:
        ...
        if self._fs is None:
            logger.info("Initializing S3FileSystem client on demand for ArtifactService...")
            try:
                self._fs = s3fs.S3FileSystem(**settings.s3_storage_options)
                logger.info("S3 FileSystem client initialized successfully for ArtifactService.")
    ...
    def _get_s3_path(self, artifact_uri: str) -> str:
        ...
        if not artifact_uri or not isinstance(artifact_uri, str) or not artifact_uri.startswith("s3://"):
            ...
            if fs_client:
                try:
                     ...
                     if fs_client.exists(s3_path):
                         ...
                         logger.warning(f"Removed potentially incomplete artifact at {uri} after save error.")
                except Exception as cleanup_err:
                     logger.error(f"Failed to clean up artifact at {uri} after save error: {cleanup_err}")
            return False
    ...
    def load_dataframe_artifact(self, uri: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.error(f"Error loading DataFrame artifact (Parquet) from {uri}: {e}", exc_info=True)
            return None
    ...
    def write_dataframe_artifact(self, df: pd.DataFrame, uri: str) -> bool:
        ...
        if df is None or not isinstance(df, pd.DataFrame):
            logger.error("Cannot write DataFrame artifact: Input is not a valid DataFrame.")
            return False
        if not uri:
        ...
        try:
            ...
            logger.error(f"Error writing DataFrame artifact (Parquet) to {uri}: {e}", exc_info=True)
            # Attempt cleanup
            fs_client = getattr(self, '_fs', None)
            if fs_client:
                try:
                    s3_path = self._get_s3_path(uri)
                    if fs_client.exists(s3_path): fs_client.rm(s3_path)
