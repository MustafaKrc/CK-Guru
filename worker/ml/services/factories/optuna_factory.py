...
from shared.schemas.hp_search_job import PrunerTypeEnum, SamplerTypeEnum

logger = logging.getLogger(__name__)


def create_sampler(
    sampler_type_str: Optional[str], params: Dict[str, Any], seed: int
    ...
    sampler_type_enum = (
        SamplerTypeEnum(sampler_type_str) if sampler_type_str else SamplerTypeEnum.TPE
    )
    sampler_type = sampler_type_enum.value.lower()

    logger.info(
        f"Creating Optuna sampler of type: '{sampler_type}' with params: {params}"
    )
    try:
        if sampler_type == SamplerTypeEnum.TPE.value:
            ...
            if not params:
                logger.warning(
                    "CMA-ES sampler selected without specific parameters. Using Optuna defaults."
                )
        ...
        # Add other samplers here based on SamplerTypeEnum
            ...
            logger.warning(
                f"Unsupported sampler type '{sampler_type}'. Falling back to TPESampler."
            )
            return optuna.samplers.TPESampler(seed=seed)
    except Exception as e:
        logger.error(
            f"Failed to create sampler type '{sampler_type}' with params {params}: {e}",
            exc_info=True,
        )
...
def create_pruner(
    pruner_type_str: Optional[str], params: Dict[str, Any], seed: Optional[int] = None
) -> Optional[optuna.pruners.BasePruner]:
    """Factory function to create Optuna pruners."""
    # Use default from Enum if string is None or empty
    pruner_type_enum = (
        PrunerTypeEnum(pruner_type_str) if pruner_type_str else PrunerTypeEnum.MEDIAN
    )
    pruner_type = pruner_type_enum.value.lower()

    logger.info(
        f"Creating Optuna pruner of type: '{pruner_type}' with params: {params}"
    )
    ...
    try:
        ...
        # Add other pruners here based on PrunerTypeEnum
            ...
            logger.warning(
                f"Unsupported pruner type '{pruner_type}'. Falling back to MedianPruner."
            )
            return optuna.pruners.MedianPruner()
    except Exception as e:
        logger.error(
            f"Failed to create pruner type '{pruner_type}' with params {params}: {e}",
            exc_info=True,
        )
