...
class DependencyProvider:
    ...
    def _get_or_create(self, lookup_key_type: Type, concrete_class: Type, *args, **kwargs) -> Any:
        """
        ...
        if lookup_key_type not in self._cache:
            logger.debug(f"Creating instance of {concrete_class.__name__} (for key {lookup_key_type.__name__})")
            try:
                # Pass session_factory to repositories/updaters that need it
                if issubclass(concrete_class, (ModelRepository, XaiResultRepository, MLFeatureRepository,
                                                JobStatusUpdater, DatasetRepository, InferenceJobRepository,
                                                TrainingJobRepository, HPSearchJobRepository)):
                     instance = concrete_class(self.session_factory, *args, **kwargs)
                else:
                     instance = concrete_class(*args, **kwargs) # For services like ArtifactService
                self._cache[lookup_key_type] = instance
            except Exception as e:
                logger.error(f"Failed to instantiate {concrete_class.__name__}: {e}", exc_info=True)
                raise RuntimeError(f"Failed to create dependency: {concrete_class.__name__}") from e
