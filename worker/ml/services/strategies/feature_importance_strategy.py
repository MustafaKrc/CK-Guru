...
class FeatureImportanceStrategy(BaseXAIStrategy):
    ...
    def explain(
        ...
        if shap is None:
            logger.error(
                "SHAP library is not installed. Cannot calculate SHAP-based feature importance."
            )
            return None
        ...
        logger.info(
            f"Calculating Feature Importance using SHAP for {len(X_inference)} instances..."
        )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"TreeExplainer failed ({tree_err}). Trying KernelExplainer."
                )
                background_sample = (
                    self.background_data
                    if self.background_data is not None
                    and not self.background_data.empty
                    else X_inference
                )
                if len(background_sample) > 100:
                    background_sample = shap.sample(background_sample, 100)
                if not hasattr(self.model, "predict_proba"):
                    raise TypeError("KernelExplainer requires predict_proba.")

                explainer = shap.KernelExplainer(
                    self.model.predict_proba, background_sample
                )
                shap_values_pos_class = explainer.shap_values(
                    X_inference, nsamples="auto"
                )[1]

            if shap_values_pos_class is None:
                raise RuntimeError(
                    "Failed to obtain SHAP values for feature importance."
                )
            ...
            importance_list: List[FeatureImportanceValue] = [
                ...
                for fn, imp in zip(
                    feature_names, avg_abs_shap
                )  # avg_abs_shap should be 1D array of shape (n_features,)
            ]
            ...
            logger.error(
                f"Error calculating SHAP-based Feature Importance: {e}", exc_info=True
            )
            return None
