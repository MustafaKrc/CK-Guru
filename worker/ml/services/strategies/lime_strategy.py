...
from shared.schemas.xai import LIMEResultData, InstanceLIMEResult
...
class LIMEStrategy(BaseXAIStrategy):
    ...
    def explain(self, X_inference: pd.DataFrame, identifiers_df: pd.DataFrame) -> Optional[LIMEResultData]:
        if lime is None:
            logger.error("LIME library is not installed. Cannot generate LIME explanations.")
            return None
        if X_inference.empty:
            logger.warning("LIMEStrategy: Input DataFrame is empty. No explanations generated.")
            return None
        if self.background_data is None or self.background_data.empty:
            logger.error("LIMEStrategy: Background data is required but missing or empty.")
            # Or potentially fallback to using X_inference as background, but log heavily
            # return None
            logger.warning("Using inference data as background data for LIME (less ideal).")
        ...
        try:
            ...
            num_features_lime = min(10, len(feature_names)) # Limit features shown by LIME
            ...
            for i in range(len(X_inference)):
                ...
                try:
                    ...
                    lime_explanation: List[Tuple[str, float]] = [(f, round(float(w), 4)) for f, w in explanation.as_list(label=1)]

                    instance_results.append(InstanceLIMEResult(
                        file=instance_id_row.get('file'),
                        class_name=instance_id_row.get('class_name'),
                        explanation=lime_explanation
                    ))
                except Exception as instance_err:
                     logger.error(f"LIME explanation failed for instance index {i}: {instance_err}", exc_info=True)
