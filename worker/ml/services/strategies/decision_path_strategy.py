...
class DecisionPathStrategy(BaseXAIStrategy):
    ...
    def explain(
        ...
        if not isinstance(self.model, (RandomForestClassifier, DecisionTreeClassifier)):
            logger.error(
                f"Decision Path explanation is not supported for model type: {type(self.model).__name__}. Supported: RandomForestClassifier, DecisionTreeClassifier."
            )
            return None

        logger.info(
            f"Generating Decision Path explanations for {len(X_inference)} instances..."
        )
        ...
        try:
            ...
            for i in range(len(X_inference)):
                ...
                for tree_idx, estimator in enumerate(estimators):
                    ...
                    if not isinstance(tree_, _tree.Tree):
                        logger.warning(
                            f"Estimator {tree_idx}'s 'tree_' attribute is not a valid Tree object ({type(tree_)}). Skipping."
                        )
                        continue
                    ...
                    for node_id_idx, node_id in enumerate(node_index):
                        ...
                        node_value_raw = (
                            tree_.value[node_id][0]
                            if tree_.value is not None and node_id < len(tree_.value)
                            else []
                        )
                        ...
                        if node_id_idx > 0:
                            ...
                            label = (
                                "True"
                                if node_id == tree_.children_left[prev_node_id]
                                else "False"
                            )
            ...
            if not instance_paths:
                logger.warning(
                    "Decision Path explanation generated no valid instance paths."
                )
                return None
            ...
            logger.error(
                f"Error generating Decision Path explanation: {e}", exc_info=True
            )
            return None
