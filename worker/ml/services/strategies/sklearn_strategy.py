...
class SklearnStrategy(BaseModelStrategy):
    ...
    def _get_model_instance(self) -> Any:
        ...
        filtered_cfg = {k: v for k, v in self.model_config.items() if k in valid_params}
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> TrainResult:
        ...
        if test_size > 0.0 and test_size < 1.0:
            ...
            logger.warning(
                "Test size is 0 or >= 1. Training on full dataset without evaluation split."
            )
    ...
    def predict(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        logger.info(
            f"SklearnStrategy: Generating predictions for {len(data)} samples..."
        )
        try:
