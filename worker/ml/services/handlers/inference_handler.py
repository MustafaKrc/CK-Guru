...
class InferenceJobHandler(BaseMLJobHandler):
    ...
    def _load_and_validate_job_details(self) -> bool:
        ...
        try:
            ...
            if job_record.status not in [JobStatusEnum.PENDING, JobStatusEnum.RUNNING]:
                logger.warning(
                    f"Job {self.job_id} terminal state {job_record.status.value}. Skipping."
                )
                return False
            ...
            if not self.ml_model_id:
                raise ValueError("ml_model_id missing.")
            ...
            if not model_record:
                raise ValueError(f"MLModel {self.ml_model_id} not found.")
            if not model_record.s3_artifact_path:
                raise ValueError(f"MLModel {self.ml_model_id} artifact path missing.")
    ...
    def _load_model_strategy(self) -> BaseModelStrategy:
        ...
        if self.ml_model_id is None:
            raise ValueError("ml_model_id not set.")
        ...
        try:
            model_type_enum = schemas.ModelTypeEnum(model_record.model_type)
        except ValueError:
            raise ValueError(f"Unsupported model_type '{model_record.model_type}'")
        ...
        strategy = create_model_strategy(
            model_type_enum,
            model_record.hyperparameters or {},
            {},
            self.artifact_service,
        )
        strategy.load_model(
            artifact_path
        )  # Strategy uses self.artifact_service internally
    ...
    def _get_features(self) -> pd.DataFrame:
        ...
        if not repo_id or not commit_hash:
            raise ValueError("repo_id/commit_hash missing in input_ref.")
        ...
        features_df = self.feature_repo.get_features_for_commit(
            repo_id, commit_hash
        )  # Needs session factory access
        if features_df is None or features_df.empty:
            raise ValueError(
                f"Failed to retrieve features for Repo={repo_id}, Commit={commit_hash[:7]}."
            )
        ...
        try:
            ...
            if not expected_features:
                ...
                expected_features = (
                    ...
                    .columns.difference(identifier_cols)
                    .tolist()
                )
                if not expected_features:
                    raise ValueError("No numeric features found.")
        except Exception as e:
            raise ValueError(f"Failed get expected features: {e}") from e

        missing_features = set(expected_features) - set(features_df.columns)
        if missing_features:
            raise ValueError(
                f"Missing columns expected by model: {sorted(list(missing_features))}"
            )

        X = features_df[expected_features].copy().fillna(0)  # Simple fillna
        logger.info(
            f"Data prepared. Features: {X.shape}, Identifiers: {identifiers_df.shape}"
        )
    ...
    def _execute_prediction(self, X_inference: pd.DataFrame) -> Dict[str, Any]:
        ...
        if self.model_strategy is None:
            raise RuntimeError("Model strategy not loaded.")
    ...
    def _package_results(
        self, ml_result: Dict[str, Any], identifiers_df: pd.DataFrame
            ...
            try:
                for i in range(num_files_analyzed):
                    ...
                    if row_probabilities and i < len(row_probabilities):
                        ...
                        elif isinstance(probs, (float, np.float_)):
                            prob_class_1 = (
                                float(probs)
                                if prediction == 1
                                else (1.0 - float(probs))
                            )
                    if prediction == 1:
                        commit_prediction = 1
                ...
                error_msg = f"Failed process results: {e}"
                commit_prediction = -1
                max_bug_probability = -1.0
                detailed_results = []
        ...
        prediction_package = InferenceResultPackage(
            ...
            max_bug_probability=(
                round(max_bug_probability, 4) if max_bug_probability >= 0 else -1.0
            ),
        ...
        return (
            prediction_package.model_dump(exclude_none=True),
            error_msg,
        )  # Return dict and error
        ...
        try:
            ...
            if not self._load_and_validate_job_details():
                results_payload["message"] = (
                    f"Job {self.job_id} skipped or failed loading."
                )
            ...
            packaged_result_dict, error_msg = self._package_results(
                ml_result, identifiers_df
            )
            results_payload["prediction_result"] = (
                packaged_result_dict  # Store packaged result dict
            )
            ...
            logger.critical(
                f"Inference Job {self.job_id} failed: {status_message}", exc_info=True
            )
            # Update result payload with error
            results_payload["status"] = JobStatusEnum.FAILED
            results_payload["error"] = status_message
            # Ensure prediction_result reflects failure if error happened before packaging
            if "prediction_result" not in results_payload or not results_payload[
                "prediction_result"
            ].get("error"):
            ...
            logger.info(
                f"Attempting final DB status update Job {self.job_id} to {final_status.value}"
            )
