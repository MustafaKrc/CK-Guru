...
class TrainingJobHandler(BaseMLJobHandler):
    ...
    def _load_and_validate_job_details(self) -> bool:
        ...
        try:
            ...
            if job_record.status not in [JobStatusEnum.PENDING, JobStatusEnum.RUNNING]:
                logger.warning(
                    f"Job {self.job_id} in terminal state {job_record.status.value}. Skipping."
                )
                return False
    ...
    def _load_data(self) -> pd.DataFrame:
        ...
        df = self.artifact_service.load_dataframe_artifact(
            dataset_path
        )  # Use injected service
        if df is None or df.empty:
            raise ValueError(f"Failed load/empty dataset from {dataset_path}")
    ...
    def _prepare_data(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if not self.job_config:
            raise RuntimeError("Job config not loaded.")
        features = self.job_config.get("feature_columns", [])
        target = self.job_config.get("target_column")
        if not features or not target:
            raise ValueError("Missing feature_columns or target_column.")
    ...
    def _create_and_train_strategy(
        ...
        if not self.job_config:
            raise RuntimeError("Job config needed.")
        ...
        if not isinstance(model_type_enum, schemas.ModelTypeEnum):
            try:
                ...
                raise ValueError(f"Invalid model_type '{model_type_enum}'.")

        # Pass artifact_service during creation
        strategy = create_model_strategy(
            model_type_enum, hyperparams, self.job_config, self.artifact_service
        )
    ...
    def _save_results(
        ...
        model_type_enum: Optional[schemas.ModelTypeEnum] = schemas.ModelTypeEnum(
            self.job_config.get("model_type")
        )
        hyperparams = self.job_config.get("hyperparameters", {})
        ...
        try:
            ...
            if not save_success:
                ...
                logger.critical(
                    f"Failed to save artifact {s3_uri} AFTER creating DB record {new_model_id}. Manual cleanup may be needed."
                )
            ...
            logger.error(
                f"Error during saving results for job {self.job_id}: {e}", exc_info=True
            )
        ...
        try:
            if not self._load_and_validate_job_details():
                return {
                    ...
                    "message": f"Job {self.job_id} skipped/failed loading.",
                }
            ...
            logger.critical(
                f"Training Job {self.job_id} failed: {status_message}", exc_info=True
            )
