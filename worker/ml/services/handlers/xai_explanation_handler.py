...
from services.artifact_service import (  # Note: This is the CONCRETE class
    ArtifactService,
)
...
class XAIExplanationHandler:
    ...
    def _prepare_data_for_xai(
        ...
        if not available_identifiers:
            ...
            raise ValueError(
                "Input features missing required identifier columns (at least 'file' or 'class_name')."
            )
        ...
        if hasattr(self.model, "feature_names_in_"):
            ...
            if len(potential_features) == num_features_expected:
                ...
                logger.warning(
                    f"Inferred {num_features_expected} expected features from data columns based on feature_importances_ length."
                )
            else:
                logger.warning(
                    "Could not reliably determine expected features from model. Trying all numeric non-identifiers."
                )
        ...
        if not expected_features:
            ...
            expected_features = (
                ...
                .columns.difference(available_identifiers)
                .tolist()
            )
            logger.warning("Using all numeric, non-identifier columns as features.")
            if not expected_features:
                raise ValueError(
                    "No numeric feature columns found after attempting inference."
                )

        missing_features = set(expected_features) - set(features_df.columns)
        if missing_features:
            raise ValueError(
                f"Features DataFrame missing columns expected by model: {sorted(list(missing_features))}"
            )
    ...
    def _load_background_data_for_xai(
        self, dataset_id: Optional[int], X_inference: pd.DataFrame
        ...
        if not dataset_id:
            logger.warning(
                "No training dataset ID linked. Using inference data sample for background."
            )
            # Ensure sampling doesn't fail on very small inference sets
            sample_n = min(100, len(X_inference)) if len(X_inference) > 0 else 0
            return (
                X_inference.sample(n=sample_n, random_state=42)
                if sample_n > 0
                else None
            )
        ...
        if not dataset_record:
            logger.warning(
                f"Dataset record {dataset_id} not found. Using inference data sample."
            )
            sample_n = min(100, len(X_inference)) if len(X_inference) > 0 else 0
            return (
                X_inference.sample(n=sample_n, random_state=42)
        ...
        if not background_path:
            logger.warning(
                f"No background data path for dataset {dataset_id}. Using inference data sample."
            )
            sample_n = min(100, len(X_inference)) if len(X_inference) > 0 else 0
            return (
                X_inference.sample(n=sample_n, random_state=42)
        ...
        try:
            background_df = self.artifact_service.load_dataframe_artifact(
                background_path
            ...
            target_column = (
                dataset_record.config.get("target_column")
                if isinstance(dataset_record.config, dict)
                else None
            )
            ...
            logger.error(
                f"Failed load background data {background_path}: {e}", exc_info=True
            )
    ...
    def process_explanation(self) -> Optional[Dict]:
        ...
        logger.info(
            f"Handler: Starting explanation generation for XAIResult ID {self.xai_result_id} (Task: {task_id})"
        )
        ...
        try:
            ...
            if not xai_record:
                raise Ignore(f"XAIResult {self.xai_result_id} not found.")
            if (
                xai_record.status == XAIStatusEnum.RUNNING
                and xai_record.celery_task_id != task_id
            ):
                raise Ignore("Already running.")
            if xai_record.status in [
                XAIStatusEnum.SUCCESS,
                XAIStatusEnum.FAILED,
                XAIStatusEnum.REVOKED,
            ]:
                raise Ignore("Already terminal.")
            ...
            if not inference_job:
                raise ValueError("InferenceJob not found.")
            ...
            if not ml_model_record or not ml_model_record.s3_artifact_path:
                raise ValueError("MLModel/path not found.")

            input_ref = (
                inference_job.input_reference
                if isinstance(inference_job.input_reference, dict)
                else {}
            )
            repo_id = input_ref.get("repo_id")
            commit_hash = input_ref.get("commit_hash")
            if not repo_id or not commit_hash:
                raise ValueError("Missing repo/commit info.")
            ...
            # --- Load Model & Features ---
            self.model = self.artifact_service.load_artifact(
                ml_model_record.s3_artifact_path
            )
            if not self.model:
                raise RuntimeError(
                    f"Failed load model: {ml_model_record.s3_artifact_path}"
                )
            features_df = self.feature_repo.get_features_for_commit(
                repo_id, commit_hash
            )  # repo handles session
            if features_df is None or features_df.empty:
                raise ValueError("Failed get features.")
            ...
            background_data_df = self._load_background_data_for_xai(
                ml_model_record.dataset_id, X_inference
            )
            ...
            logger.info(
                f"Handler: XAIResult {self.xai_result_id} status updated to RUNNING (generating)."
            )

            xai_type = xai_record.xai_type
            strategy: BaseXAIStrategy = XAIStrategyFactory.create(
                xai_type, self.model, background_data_df
            )
            ...
            if result_data_obj:
                result_data_json = result_data_obj.model_dump(
            ...
            logger.info(
                f"Handler: Ignoring/Rejecting task for XAIResult {self.xai_result_id}: {e}"
            )
            raise e
        except Exception as e:
            status_message = f"Generation failed: {type(e).__name__}: {e}"
            logger.critical(
                f"Handler: XAI generation failed for {self.xai_result_id}: {status_message}",
                exc_info=True,
            )
            final_status = XAIStatusEnum.FAILED
        finally:
            logger.info(
                f"Handler: Attempting final DB update XAIResult {self.xai_result_id} to {final_status.value}"
            )
            try:
                self.xai_repo.update_xai_result_sync(
                    self.xai_result_id,
                ...
                logger.critical(
                    f"CRITICAL: Failed final DB update for XAIResult {self.xai_result_id}: {db_err}",
                    exc_info=True,
                )
