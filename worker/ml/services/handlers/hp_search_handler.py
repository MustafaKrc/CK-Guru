...
from shared.repositories import ModelRepository, DatasetRepository, XaiResultRepository, MLFeatureRepository, HPSearchJobRepository
...
class HPSearchJobHandler(BaseMLJobHandler):
    ...
    def _load_and_validate_job_details(self) -> bool:
        ...
        try:
            ...
            if job_record.status not in [JobStatusEnum.PENDING, JobStatusEnum.RUNNING, JobStatusEnum.SUCCESS, JobStatusEnum.FAILED]:
                logger.warning(f"Job {self.job_id} in unexpected state {job_record.status.value} for continuation. Skipping.")
                return False

            self.job_db_record = job_record
            self.job_config = dict(job_record.config or {})
            self.dataset_id = job_record.dataset_id
            if not self.dataset_id: raise ValueError("dataset_id missing.")
            if not self.job_config: raise ValueError("Job config missing.")
            if not job_record.optuna_study_name: raise ValueError("Optuna study name missing.")

            # --- Validate Dataset using DatasetRepository ---
            dataset_record = self.dataset_repo.get_record(self.dataset_id)
            if not dataset_record or dataset_record.status != DatasetStatusEnum.READY or not dataset_record.storage_path:
                status_val = dataset_record.status.value if dataset_record else 'Not Found'
                raise ValueError(f"Dataset {self.dataset_id} not READY (Status: {status_val}) or path missing.")
    ...
    def _load_data(self) -> pd.DataFrame:
        ...
        if not self._dataset_storage_path: # Path should be cached by _load_and_validate_job_details
            ...
            if not self._dataset_storage_path:
                raise ValueError(f"Storage path still not found for Dataset {self.dataset_id}.")
        ...
        if df is None or df.empty: raise ValueError(f"Failed load/empty dataset {dataset_path}")
    ...
    def _prepare_data(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if not self.job_config: raise RuntimeError("Job config missing.")
        features = self.job_config.get('feature_columns', [])
        target = self.job_config.get('target_column')
        if not features or not target: raise ValueError("Missing features/target in config.")
    ...
    def _determine_optimization_direction(self) -> str:
        ...
        maximize_metrics = { m.value for m in schemas.ObjectiveMetricEnum } # Assuming maximize
    ...
    def _execute_hp_search(self, X, y) -> optuna.Study:
        ...
        if not study_name: raise ValueError("Optuna study name missing.")
        sampler = self._create_optuna_sampler()
        pruner = self._create_optuna_pruner()
        logger.info(f"Creating/loading study: '{study_name}' Dir:{direction} Sampler:{sampler.__class__.__name__} Pruner:{pruner.__class__.__name__}")
        study = optuna.create_study(study_name=study_name, storage=storage, load_if_exists=True, direction=direction, sampler=sampler, pruner=pruner)

        hp_space_config = self.job_config.get('hp_space', [])
        if not hp_space_config: raise ValueError("hp_space not defined.")
        objective = Objective(X, y, hp_space_config, self.job_config, self.artifact_service) 
        ...
        def progress_callback(study: optuna.Study, trial: optuna.trial.FrozenTrial):
             ...
             progress = 35 + int(60 * (completed_or_stopped / n_trials)) if n_trials > 0 else 35
             self._update_progress(f"Optuna trial {trial.number+1}/{n_trials} ({trial.state.name})...", min(progress, 95))

        logger.info(f"Starting Optuna optimize study '{study_name}', {n_trials} trials...")
    ...
    def _save_best_model(self, study: optuna.Study, X, y) -> Optional[int]:
        ...
        if not self.job_config.get('save_best_model', True): return None
        logger.info("Training and saving best model...")
        try: best_trial = study.best_trial
        except ValueError: logger.warning("No best trial found."); return None
        ...
        except Exception as e: raise ValueError("Could not parse job config") from e

        best_hyperparams = best_trial.params
        final_strategy = create_model_strategy(model_type_enum, best_hyperparams, self.job_config, self.artifact_service)
        ...
        try:
            ...
            model_data = {
                ...
                'hyperparameters': best_hyperparams, 'performance_metrics': train_result.metrics,
                'dataset_id': self.dataset_id, 'hp_search_job_id': self.job_id,
            ...
            save_success = final_strategy.save_model(s3_uri) # Strategy uses injected artifact_service
            if not save_success:
        ...
        except Exception as e: logger.error(f"Error _save_best_model job {self.job_id}: {e}", exc_info=True); raise
    ...
    def process_job(self) -> Dict:
        ...
        try:
            ...
            if not self._load_and_validate_job_details():
                 return {'status': JobStatusEnum.SKIPPED if self.job_db_record and self.job_db_record.status != JobStatusEnum.FAILED else 'FAILED',
            ...
            try:
                ...
                status_message = f"HP search completed. Best trial: {best_trial.number}."
                if best_model_id: status_message += f" Model saved: {best_model_id}."
            except ValueError: # No best trial
                final_status = JobStatusEnum.SUCCESS # Still success, just no best model
                status_message = "HP search finished, but no best trial found."
                study_results = {'status_message': status_message} # Store message in results for DB update
