...
def train_model_task(self: Task, training_job_id: int):
    ...
    logger.info(f"Task {task_id}: Received training request for Job ID {training_job_id}")
    # Default result in case of early failure before handler processing
    final_task_result = {'job_id': training_job_id, 'status': JobStatusEnum.FAILED, 'message': 'Task failed during initialization.'}
    ...
    try:
        ...
        if handler_status == JobStatusEnum.SUCCESS:
            ...
            logger.info(f"Task {task_id}: Training Job {training_job_id} reported SUCCESS by handler.")
        elif handler_status == JobStatusEnum.SKIPPED:
             logger.info(f"Task {task_id}: Training Job {training_job_id} was skipped by handler: {handler_message}")
             self.update_state(state=JobStatusEnum.SUCCESS, meta=final_task_result) # Treat skip as celery success
        else: # Handler indicated failure
             logger.error(f"Task {task_id}: Training Job {training_job_id} reported FAILURE by handler: {final_task_result.get('error', handler_message)}")
             self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(InternalError(handler_message), final_task_result))
    ...
    # --- Exception Handling for Celery Control/Unexpected Errors ---
        ...
        self.update_state(state='REVOKED', meta=build_failure_meta(e, final_task_result))
        raise # Re-raise for Celery
    except Ignore as e:
        logger.info(f"Task {task_id}: Ignored. Reason: {e}")
        self.update_state(state=JobStatusEnum.SUCCESS, meta={'status': 'IGNORED', 'message': str(e)})
        ...
        self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(e, final_task_result))
        ...
        try:
             ...
             status_updater.update_job_completion(training_job_id, TrainingJob, JobStatusEnum.FAILED, error_msg[:1000])
        except Exception as final_db_err: logger.error(f"Task {task_id}: Failed last resort DB update: {final_db_err}")
        # Ensure Celery knows
        self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(e, final_task_result))
...
def hyperparameter_search_task(self: Task, hp_search_job_id: int):
    ...
    task_id = self.request.id
    logger.info(f"Task {task_id}: Received HP search request for Job ID {hp_search_job_id}")
    final_task_result = {'job_id': hp_search_job_id, 'status': JobStatusEnum.FAILED, 'message': 'Task failed during initialization.'}
    ...
    try:
        ...
        if handler_status == JobStatusEnum.SUCCESS:
            self.update_state(state=JobStatusEnum.SUCCESS, meta=final_task_result)
            logger.info(f"Task {task_id}: HP Search Job {hp_search_job_id} reported SUCCESS by handler.")
        elif handler_status == JobStatusEnum.SKIPPED:
             logger.info(f"Task {task_id}: HP Search Job {hp_search_job_id} was skipped by handler.")
             self.update_state(state=JobStatusEnum.SUCCESS, meta=final_task_result)
        else: # Handler indicated failure
             logger.error(f"Task {task_id}: HP Search Job {hp_search_job_id} reported FAILURE by handler: {final_task_result.get('error', 'Unknown handler error')}")
             self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(InternalError(final_task_result.get('error', 'handler failure')), final_task_result))
    ...
    # --- Exception Handling (Similar to train_model_task) ---
        ...
        logger.warning(f"Task {task_id}: Terminated.")
        self.update_state(state='REVOKED', meta={'exc_type': 'Terminated', 'exc_message': 'Task terminated.'})
        raise e
    except Ignore as e:
        logger.info(f"Task {task_id}: Ignored. Reason: {e}")
        self.update_state(state=JobStatusEnum.SUCCESS, meta={'status': 'IGNORED', 'message': str(e)})
        ...
        try: # Last resort DB update
             ...
             status_updater.update_job_completion(hp_search_job_id, HyperparameterSearchJob, JobStatusEnum.FAILED, error_msg[:1000])
        except Exception as final_db_err: logger.error(f"Task {task_id}: Failed last resort DB update: {final_db_err}")
        self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(e, final_task_result))
...
def inference_predict_task(self: Task, inference_job_id: int):
    ...
    final_task_result = {'job_id': inference_job_id, 'status': JobStatusEnum.FAILED, 'message': 'Task failed during initialization.'}
    ...
    try:
        ...
        final_task_result = handler.process_job() # This runs prediction AND updates DB status

        handler_status = final_task_result.get('status')
        if handler_status == JobStatusEnum.SUCCESS:
            prediction_successful = True # Mark prediction as successful
            logger.info(f"Task {task_id}: Inference Job {inference_job_id} prediction reported SUCCESS by handler.")
        elif handler_status == JobStatusEnum.SKIPPED:
             logger.info(f"Task {task_id}: Inference Job {inference_job_id} was skipped by handler.")
        else: # Handler indicated failure
             logger.error(f"Task {task_id}: Inference Job {inference_job_id} reported FAILURE by handler: {final_task_result.get('error', 'Unknown handler error')}")
        ...
        if prediction_successful:
            ...
            try:
                orchestration_task = celery_app.send_task(orchestration_task_name, args=args, queue="ml_queue")
                if orchestration_task and orchestration_task.id:
                    logger.info(f"Task {task_id}: Dispatched XAI orchestration task {orchestration_task.id}")
                    final_task_result['xai_orchestration_task_id'] = orchestration_task.id
                else:
                    logger.error(f"Task {task_id}: Failed to dispatch XAI task (invalid task returned).")
                    final_task_result['xai_dispatch_error'] = 'Invalid task object returned by celery'
            except Exception as dispatch_err:
                logger.error(f"Task {task_id}: Failed to dispatch XAI task: {dispatch_err}", exc_info=True)
        ...
        if prediction_successful:
            ...
            self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(
    ...
    # --- Exception Handling ---
        ...
        logger.error(f"Task {task_id}: Rejected. Reason: {e}")
        self.update_state(state=JobStatusEnum.FAILED, meta={'exc_type': 'Reject', 'exc_message': str(e)})
        raise e
    except Exception as e:
        ...
        try: # Last resort DB update
             ...
             status_updater.update_job_completion(inference_job_id, InferenceJob, JobStatusEnum.FAILED, error_msg[:1000])
        except Exception as final_db_err: logger.error(f"Task {task_id}: Failed last resort DB update: {final_db_err}")
        self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(e, final_task_result))
        raise Reject(error_msg, requeue=False) from e
...
def orchestrate_xai_task(self: Task, inference_job_id: int):
    ...
    task_id = self.request.id
    logger.info(f"Task {task_id}: Received XAI orchestration request for InferenceJob ID {inference_job_id}")
    final_task_result = {'inference_job_id': inference_job_id, 'status': JobStatusEnum.FAILED, 'message': 'Orchestration failed'}
    ...
    try:
        ...
        if final_task_result.get('status') == JobStatusEnum.SUCCESS:
            self.update_state(state=JobStatusEnum.SUCCESS, meta=final_task_result)
            logger.info(f"Task {task_id}: XAI Orchestration reported SUCCESS by handler.")
        else: # Should not happen unless Reject is caught differently now
            logger.error(f"Task {task_id}: XAI Orchestration reported unexpected status by handler: {final_task_result}")
            self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(e, final_task_result))
...
def generate_explanation_task(self: Task, xai_result_id: int):
    ...
    task_id = self.request.id
    logger.info(f"Task {task_id}: Received explanation generation request for XAIResult ID {xai_result_id}")
    ...
    try:
        ...
        if final_task_result is not None: # Assume success if handler returned data
            self.update_state(state=JobStatusEnum.SUCCESS, meta={'result_preview': str(final_task_result)[:200]}) # Return preview
            logger.info(f"Task {task_id}: XAI Explanation generation reported SUCCESS by handler.")
        else: # Handler failed internally and updated DB, likely returned None
            logger.error(f"Task {task_id}: XAI Explanation generation failed (handler returned None or error occurred before return).")
            self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(e, final_task_result))
    ...
    # --- Exception Handling ---
        ...
        try:
             provider = DependencyProvider(); xai_repo = provider.get_xai_result_repository()
             from shared.schemas.enums import XAIStatusEnum
             xai_repo.update_xai_result_sync(xai_result_id, XAIStatusEnum.FAILED, error_msg[:1000], commit=True)
        except Exception as final_db_err: logger.error(f"Task {task_id}: Failed last resort DB update: {final_db_err}")
        self.update_state(state=JobStatusEnum.FAILED, meta=build_failure_meta(e, final_task_result))
        raise Reject(error_msg, requeue=False) from e