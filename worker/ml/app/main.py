...
from shared.repositories.ml_model_type_definition_repository import (
    ...
    for strategy_cls in strategy_classes:
        if hasattr(strategy_cls, "get_supported_model_types_with_schemas"):
            try:
                ...
                supported_types_map: Dict[
                    ModelTypeEnum, List[HyperparameterDefinitionSchema]
                ...
                for model_type_enum, schema_list in supported_types_map.items():
                    ...
                    if type_name_str in discovered_type_names:
                        logger.warning(
                            f"Model type '{type_name_str}' schema already defined by another strategy. Skipping re-definition from {strategy_cls.__name__}."
                        )
                        continue
                    ...
                    display_name = (
                        type_name_str.replace("sklearn_", "Scikit-Learn ")
                        .replace("_", " ")
                        .title()
                    )
                    if "Xgboost" in display_name:
                        display_name = display_name.replace("Xgboost", "XGBoost")
                    if "Lightgbm" in display_name:
                        display_name = display_name.replace("Lightgbm", "LightGBM")
                    ...
                    logger.debug(
                        f"Discovered model type: {type_name_str} with {len(schema_list)} HP definitions from {strategy_cls.__name__}"
                    )
            except Exception as e:
                logger.error(
                    f"Error discovering model types from strategy {strategy_cls.__name__}: {e}",
                    exc_info=True,
                )
        else:
            logger.warning(
                f"Strategy {strategy_cls.__name__} does not have 'get_supported_model_types_with_schemas' method."
            )
...
def update_model_type_registry_in_db():
    ...
    if not definitions:
        logger.warning(
            "No ML model type definitions discovered. DB registry not updated."
        )
    ...
    try:
        ...
        with get_sync_db_session() as session:
            repo = MLModelTypeDefinitionRepository(
                lambda: session
            )  # Pass lambda for session factory
            repo.upsert_model_type_definitions(definitions, WORKER_IDENTIFIER)
        logger.info("ML model type registry DB update complete.")
    except Exception as e:
        logger.error(
            f"Failed to update ML model type registry in DB: {e}", exc_info=True
        )
