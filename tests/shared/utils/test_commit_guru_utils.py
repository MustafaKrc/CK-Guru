...
def test_parse_numstat_line_added_deleted():
    ...
    assert result == ParsedNumstatLine(
        la=10, ld=5, file_name="src/main.py", subsystem="src", directory="src"
    )
...
def test_parse_numstat_line_added_only():
    ...
    result = _parse_numstat_line(line, "dummy_hash")
    assert result == ParsedNumstatLine(
        la=15, ld=0, file_name="new_file.txt", subsystem="root", directory="root"
    )  # Directory is root if no /
...
def test_parse_numstat_line_deleted_only():
    ...
    assert result == ParsedNumstatLine(
        la=0,
        ld=7,
        file_name="path/to/old_file.java",
        subsystem="path",
        directory="path/to",
    )
...
def test_parse_numstat_line_binary():
    ...
    result = _parse_numstat_line(line, "dummy_hash")
    assert result == ParsedNumstatLine(
        la=0, ld=0, file_name="assets/image.jpg", subsystem="assets", directory="assets"
    )
...
def test_update_file_state_new_file():
    ...
    parsed_line = ParsedNumstatLine(
        la=10, ld=2, file_name="a.py", subsystem="a.py", directory="root"
    )
    author = "dev1"
    timestamp = 1000
    ...
    assert result == FileUpdateResult(
        previous_loc=0, previous_nuc=0, time_diff_days=0.0, authors_involved={"dev1"}
    )
...
def test_update_file_state_existing_file():
    ...
    commit_files_state: Dict[str, CommitFile] = {
        "a.py": CommitFile(
            name="a.py", loc=5, authors=["dev1"], lastchanged=500
        )  # nuc defaults to 1
    }
    commit_files_state["a.py"].nuc = 3  # Set a previous nuc value

    parsed_line = ParsedNumstatLine(
        la=7, ld=1, file_name="a.py", subsystem="a.py", directory="root"
    )
    author = "dev2"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [20, 5, 5],
            30,
            -((20 / 30 * math.log2(20 / 30)) + 2 * (5 / 30 * math.log2(5 / 30))),
        ),  # Skewed
        ...
        (
            [10, 0, 5],
            15,
            -((10 / 15 * math.log2(10 / 15)) + (5 / 15 * math.log2(5 / 15))),
        ),  # Zero modifications ignored
...
def test_calculate_commit_guru_metrics_full_run(mock_run_cmd, tmp_path):
    ...
    assert set(commit2["files_changed"]) == {
        "src/feature_x.py",
...
def test_calculate_commit_guru_metrics_with_since(mock_run_cmd, tmp_path):
    ...
    mock_run_cmd.return_value = (
        SAMPLE_GIT_LOG_OUTPUT_MULTI  # Assume log returns commits after p0h
    )
    repo_path = tmp_path
    ...
    expected_cmd = (
        f"git log {since_hash}..HEAD {commit_guru_utils.COMMIT_GURU_LOG_FORMAT}"
    )
    mock_run_cmd.assert_called_once_with(expected_cmd, cwd=repo_path)
...
class TestGitCommitLinker:
    ...
    def patch_linker_deps(self):
        ...
        with (
            patch(
                "shared.utils.commit_guru_utils.run_git_command", autospec=True
            ) as mock_run,
            patch(
                "shared.utils.commit_guru_utils.find_commit_hash_before_timestamp",
                autospec=True,
            ) as mock_find,
        ):
    ...
    def test_get_modified_regions(self, linker, patch_linker_deps):
        ...
        assert (
            "README.md" not in regions
        )  # Should be skipped as not in CODE_FILE_EXTENSIONS

        assert mock_run_cmd.call_count == 2
        assert (
            "git diff -U0 dummy_hash^ dummy_hash"
            in mock_run_cmd.call_args_list[0].args[0]
        )
        assert (
            "git diff --name-only dummy_hash^ dummy_hash"
            in mock_run_cmd.call_args_list[1].args[0]
        )
    ...
    def test_get_modified_regions_initial_commit_error(self, linker, patch_linker_deps):
        ...
        mock_run_cmd, _ = patch_linker_deps
        mock_run_cmd.side_effect = subprocess.CalledProcessError(
            1, "git diff", stderr="unknown revision"
        )
    ...
    def test_get_modified_regions_other_error(self, linker, patch_linker_deps):
        ...
        mock_run_cmd, _ = patch_linker_deps
        mock_run_cmd.side_effect = subprocess.CalledProcessError(
            1, "git diff", stderr="some other error"
        )
    ...
    def test_git_annotate_regions_with_timestamp_and_fallback(
        self, linker, patch_linker_deps
    ):
        ...
        mock_run_cmd.side_effect = [
            subprocess.CalledProcessError(
                1,
                "git blame",
                stderr="fatal: no such path 'src/main.py' in ts_commit_hash",
            ),
    ...
    def test_git_annotate_regions_excludes_self_and_start(
        self, linker, patch_linker_deps
    ):
...
"""
    ...
    def test_link_corrective_commits(self, linker, patch_linker_deps):
        ...
        def mock_annotate_side_effect(
            regions: Dict, commit_hash: str, timestamp: Optional[int]
        ):
            print(
        ...
        with (
            patch.object(
                linker,
                "_get_modified_regions",
                side_effect=mock_get_regions_side_effect,
                autospec=True,
            ) as mock_regions_method,
            patch.object(
                linker,
                "_git_annotate_regions",
                side_effect=mock_annotate_side_effect,
                autospec=True,
            ) as mock_annotate_method,
        ):
