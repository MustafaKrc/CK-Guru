...
from unittest.mock import MagicMock, call, patch  # Use patch from unittest.mock
...
def test_run_git_command_success(mock_subprocess_run, tmp_path):
    ...
    mock_subprocess_run.return_value = MockCompletedProcess(
        stdout=expected_stdout, stderr=""
    )
...
def test_run_git_command_success_with_stderr(mock_subprocess_run, tmp_path, caplog):
    ...
    mock_subprocess_run.return_value = MockCompletedProcess(
        stdout=expected_stdout, stderr=warning_stderr
    )
...
def test_find_commit_timestamp_called_process_error(mock_run_cmd, tmp_path):
    ...
    mock_run_cmd.side_effect = subprocess.CalledProcessError(
        1, "git rev-list", stderr="fatal: bad revision 'HEAD'"
    )
...
def test_checkout_commit_git_command_error(mock_repo):
    ...
    mock_repo.git.checkout.side_effect = git.GitCommandError(
        "checkout",
        1,
        stderr=f"pathspec '{commit_hash}' did not match any file(s) known to git",
    )
...
def test_determine_default_branch_finds_via_head(mock_repo_for_branch):
    ...
    mock_repo_for_branch.remotes.origin.refs = [
        ...
        create_mock_ref(
            "origin/HEAD", is_symbolic=True, ref_name="origin/develop"
        ),  # HEAD points to develop
    ]
    result = git_utils.determine_default_branch(mock_repo_for_branch)
...
def test_determine_default_branch_handles_git_error(mock_repo_for_branch):
    ...
    mock_repo_for_branch.remotes.origin = MagicMock(
        side_effect=git.GitCommandError("fetch", 1, stderr="connection failed")
    )
