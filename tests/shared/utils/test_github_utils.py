...
from unittest.mock import patch, MagicMock, ANY # ANY is useful for comparing headers loosely
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("https://github.com/valid-owner/repo-with-hyphens.git", ("valid-owner", "repo-with-hyphens")),
...
def create_mock_response(
    ...
    elif status_code >= 400 and status_code != 403: # Simulate non-403 errors for raise_for_status
...
@pytest.mark.parametrize(
    ...
    [
        ({"X-RateLimit-Remaining": "50", "X-RateLimit-Reset": "1678886400"}, 50, 1678886400),
        ...
        ({"x-ratelimit-remaining": "10", "x-ratelimit-reset": "1678886401"}, 10, 1678886401), # Case-insensitive
    ]
)
def test_parse_rate_limit_headers(headers, expected_remaining, expected_reset):
    ...
    remaining, reset = fetcher._parse_rate_limit_headers(requests.structures.CaseInsensitiveDict(headers))
...
def test_make_request_success_200_ok(mock_requests_session_get):
    ...
    mock_headers = {"ETag": "etag123", "X-RateLimit-Remaining": "99", "X-RateLimit-Reset": "1700000000"}
    mock_requests_session_get.return_value = create_mock_response(200, json_data=mock_json, headers=mock_headers)
...
def test_make_request_success_with_request_etag(mock_requests_session_get):
    ...
    mock_headers = {"ETag": "etag_response", "X-RateLimit-Remaining": "98", "X-RateLimit-Reset": "1700000001"}
    mock_requests_session_get.return_value = create_mock_response(200, json_data=mock_json, headers=mock_headers)
...
def test_make_request_not_modified_304(mock_requests_session_get):
    ...
    mock_headers = {"ETag": "etag123", "X-RateLimit-Remaining": "97", "X-RateLimit-Reset": "1700000002"} # ETag can be same or different
    mock_requests_session_get.return_value = create_mock_response(304, headers=mock_headers)
...
def test_make_request_rate_limit_hit_and_retry(mock_sleep, mock_requests_session_get):
    ...
    mock_response_200 = create_mock_response(
        200,
        json_data={"id": 2},
        headers={"ETag": "etag456", "X-RateLimit-Remaining": "5000", "X-RateLimit-Reset": str(reset_time + 3600)}
    )
    ...
    assert mock_sleep.call_args[0][0] <= 10 + github_utils.RATE_LIMIT_BUFFER_SECONDS + 2 # Allow buffer + slight timing diff
...
def test_make_request_rate_limit_max_retries(mock_sleep, mock_requests_session_get):
    ...
    mock_requests_session_get.side_effect = [mock_response_403] * (github_utils.MAX_RATE_LIMIT_RETRIES + 1)
    ...
    assert mock_requests_session_get.call_count == github_utils.MAX_RATE_LIMIT_RETRIES + 1
...
def test_make_request_unexpected_exception(mock_requests_session_get):
    ...
    mock_requests_session_get.side_effect = ValueError("Something went wrong unexpectedly")
    ...
    assert "Something went wrong unexpectedly" in response.error_message # Check for the specific message

# -- GitHubIssueFetcher.get_issue_data Tests --
@patch.object(GitHubIssueFetcher, '_make_request', autospec=True) # Mock the internal method
def test_get_issue_data_calls_make_request(mock_internal_make_request):
    ...
    expected_api_response = GitHubAPIResponse(status_code=200, json_data={"issue": "data"}, etag="new_etag")
    ...
    expected_url = f"https://api.github.com/repos/{owner}/{repo_name}/issues/{issue_number}"
