...
def check_sql(condition, expected_substrings, disallowed_substrings=None):
    ...
    if disallowed_substrings:
        for sub in disallowed_substrings:
            assert sub.lower() not in sql_string # Check things that *shouldn't* be there
...
def test_get_bot_filter_only_inclusion_exact():
    ...
    patterns = [BotPattern(pattern='dependabot[bot]', pattern_type=PatternTypeEnum.EXACT, is_exclusion=False)]
    cgm_alias = aliased(MockCommitGuruMetric)
...
def test_get_bot_filter_only_inclusion_wildcard():
    ...
    patterns = [BotPattern(pattern='*bot*', pattern_type=PatternTypeEnum.WILDCARD, is_exclusion=False)]
    cgm_alias = aliased(MockCommitGuruMetric)
    condition = get_bot_filter_condition(patterns, cgm_alias)
    # Expects: alias.author_name LIKE '%bot%'
    check_sql(condition, ["commit_guru_metrics_mock_1.author_name like %(author_name_1)s"])

def test_get_bot_filter_only_inclusion_regex():
    """Test only regex inclusion patterns."""
    patterns = [BotPattern(pattern='^[a-z]+-bot$', pattern_type=PatternTypeEnum.REGEX, is_exclusion=False)]
    cgm_alias = aliased(MockCommitGuruMetric)
    condition = get_bot_filter_condition(patterns, cgm_alias)
...
def test_get_bot_filter_only_exclusion_exact():
    ...
    patterns = [BotPattern(pattern='admin', pattern_type=PatternTypeEnum.EXACT, is_exclusion=True)]
    cgm_alias = aliased(MockCommitGuruMetric)
    condition = get_bot_filter_condition(patterns, cgm_alias)
    # Expects: NOT (alias.author_name = 'admin') -> compiles to != or similar
    check_sql(condition, ["commit_guru_metrics_mock_1.author_name = %(author_name_1)s"]) # Should be simple negation
...
def test_get_bot_filter_mixed_patterns():
    ...
    patterns = [
        BotPattern(pattern='dependabot[bot]', pattern_type=PatternTypeEnum.EXACT, is_exclusion=False),
        BotPattern(pattern='*actions*', pattern_type=PatternTypeEnum.WILDCARD, is_exclusion=False),
        BotPattern(pattern='admin', pattern_type=PatternTypeEnum.EXACT, is_exclusion=True),
        BotPattern(pattern='test-user', pattern_type=PatternTypeEnum.EXACT, is_exclusion=True),
    ]
    cgm_alias = aliased(MockCommitGuruMetric)
    condition = get_bot_filter_condition(patterns, cgm_alias)
    ...
    ], disallowed_substrings=["and"]) # Top level should now be OR
...
def test_get_parent_metrics_all_found(mock_db_session):
    ...
    parent_metric_c1 = CKMetric(repository_id=1, commit_hash='c1', file='a.py', class_name='ClassA', loc=10, wmc=5)
    parent_metric_c2 = CKMetric(repository_id=1, commit_hash='c2', file='b.py', class_name='ClassB', loc=20, wmc=8)
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = [parent_metric_c1, parent_metric_c2]
...
def test_get_parent_metrics_some_not_found_db(mock_db_session):
    ...
    parent_metric_c1 = CKMetric(repository_id=1, commit_hash='c1', file='a.py', class_name='ClassA', loc=10)
    # Only return the metric for c1
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = [parent_metric_c1]
    ...
    assert result_df.loc[input_df.index[0], 'parent_loc'] == 10
    assert pd.isna(result_df.loc[input_df.index[1], 'parent_loc']) # Should be NaN if not found
...
def test_get_parent_metrics_empty_input(mock_db_session):
    ...
    input_df = pd.DataFrame(columns=[
        'repository_id', 'commit_hash', 'parent_hashes', 'file', 'class_name', 'id'
    ])
    result_df = get_parent_ck_metrics(mock_db_session, input_df)
...
def test_calculate_delta_metrics_basic():
    ...
    pd.testing.assert_series_equal(result_df['d_loc'], pd.Series([10.0, 5.0]), check_names=False)
    pd.testing.assert_series_equal(result_df['d_wmc'], pd.Series([2.0, -2.0]), check_names=False)
...
def test_calculate_delta_metrics_with_nans():
    ...
    input_df = pd.DataFrame({
        ...
        '_parent_metric_found': [True, True, True], # Assume parent row existed but value was NaN
    })
    result_df = calculate_delta_metrics(input_df)
...
def test_calculate_delta_metrics_non_numeric():
    ...
    assert result_df.loc[0, 'd_loc'] == 10.0
    assert pd.isna(result_df.loc[1, 'd_loc']) # Non-numeric coerces to NaN, delta is NaN
