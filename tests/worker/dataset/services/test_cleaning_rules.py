    ...
    df = create_df(
        {
            ...
            "is_buggy": [
                ...
                False,
                True,
            ],  # C1 last is clean/recent, C2 last is buggy
            "id": [1, 2, 3, 4],
        }
    )
...
def test_rule2_empty_df():
    rule = Rule2RemoveRecentCleanLastChange()
    df = create_df(
        {"class_name": [], "author_date_unix_timestamp": [], "is_buggy": [], "id": []}
    )
    result = rule.apply(df, {}, {})
    assert result.empty
...
def test_rule3_is_empty():
    ...
    df = create_df(
        {
            "totalMethodsQty": [1, 0, 0],
            "totalFieldsQty": [1, 0, np.nan],
            "id": [1, 2, 3],
        }
    )
    result = rule.apply(df, {}, {})
...
def test_rule4_not_trivial():
    rule = Rule4RemoveTrivialGetSet()
    df = create_df(
        {"totalMethodsQty": [1, 1], "wmc": [2, 1], "rfc": [1, 2], "id": [1, 2]}
    )
    result = rule.apply(df, {}, {})
    assert len(result) == 2
...
def test_rule6_comment_only():
    ...
    df = create_df(
        {"d_loc": [0, 0, 1], "d_wmc": [0, 0, 0], "la": [1, 0, 1], "id": [1, 2, 3]}
    )
    result = rule.apply(df, {}, {})
    assert (
        len(result) == 1
    )  # Only row where la > 0 kept (others dropped by Rule 6 logic)
...
def test_rule7_custom_threshold():
    ...
    df = create_df(
        {"la": [2, 5], "ld": [2, 0], "d_totalMethodsQty": [1, 1], "id": [1, 2]}
    )
...
def test_rule8_remove_type():
    ...
    df = create_df(
        {"file": ["MyType.java", "SomeClass.java", "mytype.java"], "id": [1, 2, 3]}
    )
    result = rule.apply(df, {}, {})
    assert len(result) == 2
    assert sorted(result["id"].tolist()) == [
        2,
        3,
    ]  # MyType.java removed, mytype.java kept
...
def test_rule8_remove_exception():
    ...
    df = create_df(
        {
            "file": ["MyException.java", "MyEx.java", "ExceptionBase.java"],
            "id": [1, 2, 3],
        }
    )
    result = rule.apply(df, {}, {})
...
def test_rule10_not_data_class():
    rule = Rule10RemoveDataClass()
    df = create_df(
        {"wmc": [2, 1], "rfc": [1, 0], "totalFieldsQty": [1, 0], "id": [1, 2]}
    )
    # Row 1: wmc > 1 -> keep
    # Row 2: fields = 0 -> keep
    result = rule.apply(df, {}, {})
...
def test_rule_cluster_large_commit_below_avg(mock_kmeans):
    ...
    result = rule.apply(
        df, params, {"feature_columns": ["x1"], "target_column": "is_buggy"}
    )
    assert (
        len(result) == 2
    )  # Should keep both rows as large commit group size (1) <= avg_count (1)
    mock_kmeans.assert_not_called()
...
def test_rule_cluster_performs_clustering_and_agg(mock_kmeans_cls):
    ...
    df = create_df(
        {
            ...
            + [True, False, True, False, True],  # Target
    ...
    config = {
        "feature_columns": ["x1", "changed_file_count"],
...
def test_rule_cluster_handles_agg_error(mock_kmeans_cls, caplog):
    ...
    with patch(
        ...
        side_effect=TypeError("Agg failed"),
    ):
        result = rule.apply(df, params, config)
