import pytest
from unittest.mock import AsyncMock, MagicMock, patch # Use AsyncMock for async functions
...
async def test_get_repository_found(mock_db_session):
    ...
    repo_id = 1
    mock_repo = Repository(id=repo_id, name="test-repo", git_url="http://test.com/repo.git")

    # Configure mock result
    mock_db_session.execute.return_value.scalars.return_value.first.return_value = mock_repo
...
async def test_get_repository_by_git_url_found(mock_db_session):
    ...
    result = await crud_repository.get_repository_by_git_url(mock_db_session, git_url=git_url)
...
async def test_get_repositories(mock_db_session):
    ...
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = [mock_repo1, mock_repo2]
...
async def test_create_repository(mock_db_session):
    ...
    created_repo = await crud_repository.create_repository(db=mock_db_session, obj_in=repo_in)
...
async def test_update_repository(mock_db_session):
    ...
    updated_repo = await crud_repository.update_repository(db=mock_db_session, db_obj=db_repo, obj_in=repo_in)

    # Check that the original object was modified
    assert db_repo.name == "new-name"
    assert str(db_repo.git_url) == "http://new.com/repo.git" # Compare string representation
...
async def test_delete_repository_found(mock_db_session):
    ...
    db_repo = Repository(id=repo_id, name="to-delete", git_url="http://delete.me/repo.git")
    ...
    with patch('backend.app.crud.crud_repository.get_repository', new_callable=AsyncMock) as mock_get:
        mock_get.return_value = db_repo

        deleted_repo = await crud_repository.delete_repository(db=mock_db_session, repo_id=repo_id)
