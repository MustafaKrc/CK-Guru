...
from shared.exceptions import NotFoundError, ConflictError, InternalError 
...
class InferenceService:
    ...
    async def trigger_inference(
        ...
        """
        logger.info(f"Service: Starting inference trigger process for Repo={repo_id}, Commit={commit_hash[:7]}, Model={model_id}")

        # --- Validation ---
        try:
            repo = await crud.crud_repository.get_repository(self.db, repo_id=repo_id)
            if not repo: raise NotFoundError(f"Repository {repo_id} not found.")

            model = await crud.crud_ml_model.get_ml_model(self.db, model_id=model_id)
            if not model: raise NotFoundError(f"ML Model {model_id} not found.")
            if not model.s3_artifact_path:
                raise ConflictError(f"ML Model {model_id} is not ready (missing artifact path).")
            ...
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(e)) from e
             ...
             raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Validation error.") from e
        ...
        try:
            ...
            db_job = await crud.crud_inference_job.create_inference_job(db=self.db, obj_in=job_schema)
            job_id = db_job.id
            await self.db.flush() # Ensure ID is available
            logger.info(f"Service: Created InferenceJob ID {job_id}")
        except Exception as e:
             logger.error(f"Service: Failed to create InferenceJob record: {e}", exc_info=True)
             await self.db.rollback()
             raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create job record.") from e
        ...
        try:
            ...
            task_id = task.id
            logger.info(f"Service: Dispatched feature extraction task {task_id} for job {job_id}")
        except Exception as e:
            logger.error(f"Service: Failed to dispatch Celery task '{task_name}': {e}", exc_info=True)
            # Attempt to mark job as failed immediately
            try:
                fail_update = schemas.InferenceJobUpdate(status=JobStatusEnum.FAILED, status_message=f"Feature extraction task dispatch failed: {e}")
                await crud.crud_inference_job.update_inference_job(db=self.db, db_obj=db_job, obj_in=fail_update)
                await self.db.commit()
            except Exception as db_fail_err:
                logger.error(f"Failed to mark job {job_id} as failed after dispatch error: {db_fail_err}")
                await self.db.rollback()
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to dispatch background task.") from e

        # --- Update Job with Task ID ---
        try:
            update_schema = schemas.InferenceJobUpdate(celery_task_id=task_id)
            await crud.crud_inference_job.update_inference_job(db=self.db, db_obj=db_job, obj_in=update_schema)
            await self.db.commit()
            logger.info(f"Service: Updated job {job_id} with task {task_id}")
        except Exception as e:
            logger.error(f"Service: Failed to update job {job_id} with task ID {task_id}: {e}", exc_info=True)
            await self.db.rollback()
            # Difficult state: task dispatched but DB record doesn't reflect it.
            # Consider queuing a cleanup task or raising an alert.
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to link task ID to job.") from e
