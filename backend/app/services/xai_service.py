...
from shared.schemas.enums import JobStatusEnum, XAIStatusEnum, XAITypeEnum # Import XAITypeEnum
...
class XAIService:
    ...
    async def trigger_xai_orchestration(self, inference_job_id: int) -> str:
        ...
        """
        logger.info(f"Service: Triggering XAI orchestration for InferenceJob ID: {inference_job_id}")

        # --- Validation ---
        try:
            inference_job = await crud.crud_inference_job.get_inference_job(self.db, job_id=inference_job_id)
            ...
            if inference_job.status != JobStatusEnum.SUCCESS:
                raise ConflictError(f"Inference job {inference_job_id} is not in SUCCESS state (current: {inference_job.status.value}). Cannot trigger XAI.")
        except NotFoundError as e:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e)) from e
        except ConflictError as e:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(e)) from e
        except Exception as e:
             logger.error(f"Unexpected validation error for XAI trigger: {e}", exc_info=True)
             raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="XAI trigger validation error.") from e

        # --- Dispatch Orchestration Task ---
        task_name = "tasks.orchestrate_xai" # Task in ML worker
        args = [inference_job_id]
        try:
            task = self.celery_app.send_task(task_name, args=args, queue="ml_queue") # Use ML queue for orchestrator
            if not task or not task.id:
                raise InternalError("Celery dispatch returned invalid task object for XAI orchestration.")
            task_id = task.id
            logger.info(f"Service: Dispatched XAI orchestration task {task_id} for job {inference_job_id}")
            return task_id
        except Exception as e:
            logger.error(f"Service: Failed to dispatch Celery task '{task_name}': {e}", exc_info=True)
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to dispatch XAI orchestration task.") from e
    ...
    async def get_explanation_status(self, xai_result_id: int) -> schemas.XAIResultRead:
        ...
        db_xai = await crud.crud_xai_result.get_xai_result(self.db, xai_result_id=xai_result_id)
        if not db_xai:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"XAI Result {xai_result_id} not found.")
    ...
    async def get_all_explanations_for_job(
        ...
        logger.debug(f"Service: Getting all explanations for InferenceJob {inference_job_id}")
        # Ensure inference job exists (optional, but good practice)
        inference_job = await crud.crud_inference_job.get_inference_job(self.db, job_id=inference_job_id)
        if not inference_job:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Inference Job {inference_job_id} not found.")

        results_db = await crud.crud_xai_result.get_xai_results_by_job_id(
            db=self.db, inference_job_id=inference_job_id, xai_type=xai_type, status=status
        )
