...
async def create_xai_result(db: AsyncSession, *, obj_in: XAIResultCreate) -> XAIResult:
    ...
    logger.info(f"Created XAIResult ID {db_obj.id} for Job {db_obj.inference_job_id}, Type: {db_obj.xai_type.value}")
    return db_obj
...
async def delete_xai_result(db: AsyncSession, *, xai_result_id: int) -> Optional[XAIResult]:
    ...
    if db_obj:
        ...
        await db.commit()
        logger.info(f"Deleted XAIResult ID {xai_result_id} (Job: {job_id}, Type: {xai_type})")
    return db_obj