...
async def get_bot_patterns(
    ...
    *,
    ...
    stmt_total = (
        select(func.count()).select_from(BotPattern).where(*filters)
        if filters
        else select(func.count(BotPattern.id))
    )
...
async def update_bot_pattern(
    db: AsyncSession, *, db_obj: BotPattern, obj_in: BotPatternUpdate
) -> BotPattern:
...
async def delete_bot_pattern(
    ...
    if db_obj:
        ...
        await db.commit()
        logger.info(
            f"Deleted bot pattern ID {pattern_id} for repo {repo_id_log or 'Global'}"
        )
    return db_obj
