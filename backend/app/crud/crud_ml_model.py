...
async def create_ml_model(db: AsyncSession, *, obj_in: MLModelCreate) -> MLModel:
    ...
    logger.info(f"Created ML Model ID {db_obj.id} (Name: {db_obj.name}, Version: {db_obj.version})")
    return db_obj
...
async def delete_ml_model(db: AsyncSession, *, model_id: int) -> Optional[MLModel]:
    ...
    if db_obj:
        ...
        await db.commit()
        logger.info(f"Deleted ML Model ID {model_id} (Name: {model_name}, Version: {model_version})")
