...
async def get_inference_job_by_task_id(db: AsyncSession, celery_task_id: str) -> Optional[InferenceJob]:
...
async def get_inference_jobs(
    ...
    model_id: Optional[int] = None, status: Optional[JobStatusEnum] = None
...
async def update_inference_job(
    db: AsyncSession, *, db_obj: InferenceJob, obj_in: InferenceJobUpdate | Dict[str, Any]
) -> InferenceJob:
...
async def delete_inference_job(db: AsyncSession, *, job_id: int) -> Optional[InferenceJob]:
