...
def upgrade() -> None:
    ...
    commit_ingestion_status_enum = sa.Enum('PENDING', 'RUNNING', 'COMPLETE', 'FAILED', name='commit_ingestion_status_enum')
    file_change_type_enum = sa.Enum('A', 'M', 'D', 'R', 'C', 'T', 'U', 'X', 'B', name='file_change_type_enum')

    # Create the commit_details table
    op.create_table('commit_details',
        ...
        sa.Column('ingestion_status', commit_ingestion_status_enum, server_default='PENDING', nullable=False),
    ...
    op.create_index(op.f('ix_commit_details_celery_ingestion_task_id'), 'commit_details', ['celery_ingestion_task_id'], unique=False)
    op.create_index(op.f('ix_commit_details_commit_hash'), 'commit_details', ['commit_hash'], unique=False)
    op.create_index(op.f('ix_commit_details_id'), 'commit_details', ['id'], unique=False)
    op.create_index(op.f('ix_commit_details_ingestion_status'), 'commit_details', ['ingestion_status'], unique=False)
    op.create_index(op.f('ix_commit_details_repository_id'), 'commit_details', ['repository_id'], unique=False)

    # Create the commit_file_diffs table
    op.create_table('commit_file_diffs',
        sa.Column('id', sa.Integer(), nullable=False),
        ...
        sa.ForeignKeyConstraint(['commit_detail_id'], ['commit_details.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_commit_file_diffs_commit_detail_id'), 'commit_file_diffs', ['commit_detail_id'], unique=False)
    op.create_index(op.f('ix_commit_file_diffs_id'), 'commit_file_diffs', ['id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_commit_file_diffs_commit_detail_id'), table_name='commit_file_diffs')
    op.drop_table('commit_file_diffs')
    
    op.drop_index(op.f('ix_commit_details_repository_id'), table_name='commit_details')
    op.drop_index(op.f('ix_commit_details_ingestion_status'), table_name='commit_details')
    ...
    op.drop_index(op.f('ix_commit_details_celery_ingestion_task_id'), table_name='commit_details')
