...
def upgrade() -> None:
    ...
    pattern_type_enum = postgresql.ENUM(
        "REGEX", "WILDCARD", "EXACT", name="pattern_type_enum"
    )
...
def downgrade() -> None:
    ...
    pattern_type_enum = postgresql.ENUM(
        "REGEX", "WILDCARD", "EXACT", name="pattern_type_enum", create_type=False
    )
    ...
    op.add_column(
        "bot_patterns",
        sa.Column(
            "pattern_type", pattern_type_enum, nullable=False, server_default="REGEX"
        ),
    )
