...
def upgrade() -> None:
    ...
    sa.Column('xai_type', sa.Enum('SHAP', 'LIME', 'FEATURE_IMPORTANCE', 'DECISION_PATH', 'COUNTERFACTUALS', name='xai_type_enum'), nullable=False),
    sa.Column('status', sa.Enum('PENDING', 'RUNNING', 'SUCCESS', 'FAILED', name='xai_status_enum'), nullable=False),
    ...
    sa.ForeignKeyConstraint(['inference_job_id'], ['inference_jobs.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('inference_job_id', 'xai_type', name='uq_inference_job_xai_type')
    )
    op.create_index(op.f('ix_xai_results_celery_task_id'), 'xai_results', ['celery_task_id'], unique=False)
    op.create_index(op.f('ix_xai_results_id'), 'xai_results', ['id'], unique=False)
    op.create_index(op.f('ix_xai_results_inference_job_id'), 'xai_results', ['inference_job_id'], unique=False)
    op.create_index('ix_xai_results_job_id_type', 'xai_results', ['inference_job_id', 'xai_type'], unique=False)
    op.create_index(op.f('ix_xai_results_status'), 'xai_results', ['status'], unique=False)
    op.create_index(op.f('ix_xai_results_xai_type'), 'xai_results', ['xai_type'], unique=False)
    op.alter_column('inference_jobs', 'input_reference',
    ...
    op.alter_column('inference_jobs', 'prediction_result',
               existing_type=postgresql.JSON(astext_type=sa.Text()),
    ...
    op.create_index(op.f('ix_inference_jobs_celery_task_id'), 'inference_jobs', ['celery_task_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.create_index('ix_inference_jobs_celery_task_id', 'inference_jobs', ['celery_task_id'], unique=True)
    op.alter_column('inference_jobs', 'prediction_result',
               existing_type=postgresql.JSON(astext_type=sa.Text()),
